...
sfdc_lead_jan <- read_csv("number_check/report1707777600884.csv")
...
sfdc_lead_jan <- sfdc_lead_jan |>
  ...
  filter(str_detect(`Lead Channel`, "DM") &
           `Created Date` >= mdy("12/27/2023") & `Created Date` <= mdy("1/31/2024")) |> 
...
january_dm |> 
  ...
  select(flow, types, mql, sql, cw, list_source, everything())-> test2

write_csv(test2, "number_check/jan_dm.csv", na = "")
