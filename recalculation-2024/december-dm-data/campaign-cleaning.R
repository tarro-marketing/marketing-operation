...
sfdc_lead <- final_campaign_sfdc_lead <- read_csv("clean_data/final_campaign_sfdc_lead.csv")
...
december_dm_lead <- december_dm_lead %>%
  mutate(drop_group = case_when(
    `Latest Campaign` %in% c(
      "Mkt_DM_Snowflake_2023ChristmasUSDrop1",
    ...
    `Latest Campaign` %in% c(
      ...
      "Mkt_DM_Snowflake_2023ChristmasV6"
    ) ~ "Drop 1 - Canada",
    `Latest Campaign` %in% c(
      "Mkt_DM_Snowflake_2023ChristmasUSDrop2",
      "Mkt_DM_Snowflake_2023ChristmasV8",
      "Mkt_DM_Snowflake_2023ChristmasV9",
      "Mkt_DM_Snowflake_2023ChristmasV10"
    ) ~ "Drop 2 - US",
    TRUE ~ NA_character_
  )) |>
  ...
  mutate(flow = case_when(
    `Created By` %in% c("JotForm Integration User", "Carlito Academia") ~ "webflow",
    TRUE ~ "inbound call"
  )) |>
  mutate(types = case_when(
    `Created By` == "JotForm Integration User" ~ "jotform",
...
december_dm_lead <- december_dm_lead |>
  ...
  mutate(cw = case_when(
    Stage == "Closed Won" | Stage == "Onboarded" ~ "TRUE",
    TRUE ~ "FALSE"
  ))
