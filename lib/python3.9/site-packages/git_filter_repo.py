...
def string_to_date(datestring):
    ...
    return datetime.fromtimestamp(
        int(unix_timestamp), FixedTimeZone(tz_offset)
    )
...
def glob_to_regex(glob_bytestr):
    ...
    if decode(glob_bytestr).encode() != glob_bytestr:  # pragma: no cover
        raise SystemExit(
            _("Error: Cannot handle glob %s").format(glob_bytestr)
        )
    ...
    if regex.endswith(r"\Z(?ms)"):  # pragma: no cover
        regex = regex[0:-7]
    elif regex.startswith(r"(?s:") and regex.endswith(
        r")\Z"
    ):  # pragma: no cover
        regex = regex[4:-3]
...
class PathQuoting:
    ...
    _unescape_re = re.compile(rb'\\([a-z"\\]|[0-9]{3})')
    ...
    def unescape_sequence(orig):
        seq = orig.group(1)
        return (
            PathQuoting._unescape[seq]
            if len(seq) == 1
            else bytes([int(seq, 8)])
        )
...
class MailmapInfo(object):
    ...
    def _parse_file(self, filename):
        name_and_email_re = re.compile(rb"(.*?)\s*<([^>]*)>\s*")
        comment_re = re.compile(rb"\s*#.*")
        ...
        with open(filename, "br") as f:
            ...
            for line in f:
                ...
                if len(line) == m.end():
                    self.changes[(None, proper_email)] = (
                        proper_name,
                        proper_email,
                    )
                    continue
                ...
                self.changes[(commit_name, commit_email)] = (
                    proper_name,
                    proper_email,
                )
    ...
    def __str__(self):
        ...
        for k in sorted(self._reverse_translation):
            rv += (
                ...
                + str(k)
                + " -> "
                + str(self._reverse_translation[k])
                + "\n"
            )
...
class FileChange(_GitElement):
    ...
    def __init__(self, type_, filename=None, id_=None, mode=None):
        ...
        if type_ == b"M":
            ...
            if id_ is None:
                raise SystemExit(
                    _("new name needed for rename of %s") % filename
                )
...
class Commit(_GitElementWithId):
    ...
    def dump(self, file_):
        ...
        if self.message.endswith(b"\n") or not (
            self.parents or self.file_changes
        ):
            extra_newline = b""
...
class Tag(_GitElementWithId):
    ...
    def dump(self, file_):
        ...
        if self.tagger_name:
            file_.write(
                b"tagger %s <%s> " % (self.tagger_name, self.tagger_email)
            )
...
class FastExportParser(object):
    ...
    def __init__(
        ...
        self._mark_re = re.compile(rb"mark :(\d+)\n$")
        self._parent_regexes = {}
        parent_regex_rules = (rb" :(\d+)\n$", rb" ([0-9a-f]{40})\n")
        ...
        self._quoted_string_re = re.compile(rb'"(?:[^"\\]|\\.)*"')
        self._refline_regexes = {}
        for refline_name in (b"reset", b"commit", b"tag", b"progress"):
            self._refline_regexes[refline_name] = re.compile(
                refline_name + b" (.*)\n$"
            )
        self._user_regexes = {}
        for user in (b"author", b"committer", b"tagger"):
            self._user_regexes[user] = re.compile(
                user + b" (.*?) <(.*?)> (.*)\n$"
            )
    ...
    def _parse_optional_filechange(self):
        ...
        if changetype == b"M":
            ...
            changetype == b"R"
        ):  # pragma: no cover (now avoid fast-export renames)
    ...
    def _parse_commit(self):
        ...
        if self._currentline.startswith(b"author"):
            (author_name, author_email, author_date) = self._parse_user(
                b"author"
            )
    ...
    def _parse_tag(self):
        ...
        if self._currentline.startswith(b"tagger"):
            (tagger_name, tagger_email, tagger_date) = self._parse_user(
                b"tagger"
            )
    ...
    def run(self, input, output):
        ...
        while self._currentline:
            if self._currentline.startswith(b"blob"):
                ...
                raise SystemExit(
                    _("Unsupported command: '%s'") % self._currentline
                )
            else:
                raise SystemExit(
                    _("Could not parse line: '%s'") % self._currentline
                )
...
class SubprocessWrapper(object):
    ...
    def check_output(*args, **kwargs):
        ...
        return subprocess.check_output(
            SubprocessWrapper.decodify(*args), **kwargs
        )

    @staticmethod
    def check_call(
        *args, **kwargs
    ):  # pragma: no cover  # used by filter-lamely
        if "cwd" in kwargs:
            kwargs["cwd"] = decode(kwargs["cwd"])
...
class GitUtils(object):
    ...
    def get_commit_count(repo, *args):
        ...
        if p.wait() != 0:
            raise SystemExit(
                _("%s does not appear to be a valid git repository")
                % decode(repo)
            )
    ...
    def get_refs(repo_working_dir):
        try:
            output = subproc.check_output(
                "git show-ref".split(), cwd=repo_working_dir
            )
    ...
    def get_file_changes(repo, parent_hash, commit_hash):
        ...
        for line in output.splitlines():
            ...
            if changetype == b"D":
                ...
                raise SystemExit(
                    "Unknown change type for line {}".format(line)
                )
    ...
    def print_my_version():
        ...
        contents = re.sub(rb"\A#\!.*", rb"#!/usr/bin/env python3", contents)
        contents = re.sub(
            rb'(\("GIT_TEXTDOMAINDIR"\) or ").*"',
            rb'\1@@LOCALEDIR@@"',
            contents,
        )
...
class FilteringOptions(object):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            ...
            if suffix.startswith("rename"):
                ...
                if len(values) != 2:
                    raise SystemExit(
                        _(
                            "Error: --path-rename expects one colon in its"
                ...
                if any(v.startswith(b"/") for v in values):
                    raise SystemExit(
                        _("Error: Pathnames cannot begin with a '/'")
                    )
            ...
            for illegal_path in [b".", b".."]:
                if illegal_path in components:
                    raise SystemExit(
                        _("Error: Invalid path component '%s' found in '%s'")
    ...
    class HelperFilter(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            af = FilteringOptions.AppendFilter(
                dest="path_changes", option_strings=None
            )
    ...
    class FileWithPathsFilter(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            ...
            namespace.path_changes += FilteringOptions.get_paths_from_file(
                values
            )
...
EXAMPLES
    ...
    For more detailed examples and explanations, see
      https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html#EXAMPLES"""
        ...
        analyze.add_argument(
            ...
            dest="report_dir",
            help=_(
                "Directory to write report, defaults to GIT_DIR/filter_repo/analysis,"
        ...
        helpers.add_argument(
            "--paths", help=argparse.SUPPRESS, metavar="IGNORE"
        )
        helpers.add_argument(
            "--paths-from-file",
            metavar="FILENAME",
        ...
        contents.add_argument(
            ...
            help=_(
                ...
                "choose a replacement choice other than the default of '{}'.".format(
                    decode(FilteringOptions.default_replace_text)
                )
            ),
        )
        contents.add_argument(
        ...
        messages.add_argument(
            ...
            metavar="EXPRESSIONS_FILE",
            help=_(
                "A file with expressions that, if found in commit messages, "
        ...
        parents.add_argument(
            ...
            help=_(
                "How to handle replace refs (see git-replace(1)).  Replace "
        ...
        callback = parser.add_argument_group(
            title=_("Generic callback code snippets")
        )
        ...
        misc.add_argument(
            ...
            action="store_true",
            help=_(
                "Rewrite repository history even if the current repo does not "
    ...
    def sanity_check_args(args):
        ...
        if args.analyze and args.stdin:
            raise SystemExit(
                _("Error: --analyze is incompatible with --stdin.")
            )
        ...
        if not any(
            x in output for x in [b"--mark-tags", b"--[no-]mark-tags"]
        ):  # pragma: no cover
            global write_marks
            write_marks = False
        ...
        if not any(
            x in output for x in [b"--reencode", b"--[no-]reencode"]
        ):  # pragma: no cover
        ...
        if args.max_blob_size:
            ...
            if suffix not in "1234567890":
                ...
                args.max_blob_size = (
                    int(args.max_blob_size[0:-1]) * mult[suffix]
                )
            else:
    ...
    def get_paths_from_file(filename):
        ...
        with open(filename, "br") as f:
            for line in f:
                ...
                if repl is not None:
                    new_path_changes.append(
                        ["rename", match_type, (match, repl)]
                    )
                else:
                    ...
                    if match_type == "glob" and not match.endswith(b"*"):
                        ...
                        new_path_changes.append(
                            ["filter", match_type, match + extension]
                        )
    ...
    def parse_args(input_args, error_on_empty=True):
        ...
        if args.replace_text:
            args.replace_text = FilteringOptions.get_replace_text(
                args.replace_text
            )
        if args.replace_message:
            args.replace_message = FilteringOptions.get_replace_text(
...
class RepoAnalyze(object):
    ...
    def handle_renames(stats, commit, change_types, filenames):
        for index, change_type in enumerate(change_types):
            if change_type == ord(b"R"):
                ...
                RepoAnalyze.setup_equivalence_for_rename(
                    stats, oldname, newname
                )
        ...
        for change in file_changes:
            ...
            if modes[-1] == b"160000":
                ...
                RepoAnalyze.handle_file(
                    stats, graph, commit, modes, shas, filenames
                )
                ...
                RepoAnalyze.handle_renames(
                    stats, commit, change_types, filenames
                )
            else:
                raise SystemExit(
                    _(
    ...
    def gather_data(args):
        ...
        num_commits = 0
        cmd = (
            "git rev-list --topo-order --reverse {}".format(
                " ".join(args.refs)
            )
        ...
        dtp = subproc.Popen(
            cmd, shell=True, bufsize=-1, stdout=subprocess.PIPE
        )
        f = dtp.stdout
        line = f.readline()
        if not line:
        ...
        while cont:
            ...
            if args.refs:
                graph.record_external_commits(
                    [p for p in parents if not p in graph.value]
                )
    ...
    def write_report(reportdir, stats):
        ...
        with open(os.path.join(reportdir, b"README"), "bw") as f:
            ...
            f.write(
                (
                    "  %s: %d\n"
                    % (_("Number of commits"), stats["num_commits"])
                ).encode()
            )
            f.write(
                (
        ...
        with open(
            os.path.join(reportdir, b"directories-deleted-sizes.txt"), "bw"
        ) as f:
        ...
        with open(
            os.path.join(reportdir, b"directories-all-sizes.txt"), "bw"
        ) as f:
            f.write(
                (
        ...
        with open(
            os.path.join(reportdir, b"extensions-deleted-sizes.txt"), "bw"
        ) as f:
        ...
        with open(
            os.path.join(reportdir, b"extensions-all-sizes.txt"), "bw"
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(reportdir, b"path-deleted-sizes.txt"), "bw"
        ) as f:
            msg = "=== %s ===\n" % _(
        ...
        with open(
            os.path.join(reportdir, b"blob-shas-and-paths.txt"), "bw"
        ) as f:
            f.write(
                (
                    "=== %s ===\n"
            ...
            for sha, size in sorted(
                ...
                if len(names_with_sha) == 1:
                    ...
                    names_with_sha = (
                        b"[" + b", ".join(sorted(names_with_sha)) + b"]"
                    )
                f.write(
                    b"  %s %10d %10d %s\n"
    ...
    def run(args):
        ...
        if os.path.isdir(reportdir):
            if args.force:
                sys.stdout.write(
                    _('Warning: Removing recursively: "%s"')
                    % decode(reportdir)
                )
                ...
                sys.stdout.write(
                    _(
                        'Error: dir already exists (use --force to delete): "%s"\n'
                    )
                    % decode(reportdir)
                )
                sys.exit(1)
...
class RepoFilter(object):
    def __init__(
        ...
        self._fe_orig = (
            None  # Path to where original fast-export output stored
        )
        self._fe_filt = (
            None  # Path to where filtered fast-export output stored
        )
        ...
        # Compile some regexes and cache those
        self._hash_re = re.compile(rb"(\b[0-9a-f]{7,40}\b)")
        ...
        def handle(type):
            ...
            if code_string:
                ...
                if "return " not in code_string and type not in (
                    ...
                    raise SystemExit(
                        _(
                            "Error: --%s-callback should have a return statement"
                        )
                        % type
                    )
    ...
    def sanity_check(refs, is_bare):
        ...
        if (
            num_packs > 1
            or (num_packs == 1 and num_loose_objects > 0)
            or num_loose_objects >= 100
        ):
            ...
            for filename in files:
                ...
                with open(pathname, "br") as f:
                    if len(f.read().splitlines()) > 1:
                        ...
                        abort(
                            _(
                                "expected at most one entry in the reflog for %s"
                            )
                            % decode(shortpath)
                        )
        ...
        if not is_bare:
            ...
            for refname, rev in refs.items():
                if not refname.startswith(b"refs/heads/"):
                    continue
                origin_ref = refname.replace(
                    b"refs/heads/", b"refs/remotes/origin/"
                )
    ...
    def cleanup(repo, repack, reset, run_quietly=False, show_debuginfo=False):
        ...
        if repack and not run_quietly and not show_debuginfo:
            print(
                _("Repacking your repo and cleaning out old unneeded objects")
            )
        ...
        if reset:
            cleanup_cmds.insert(
                0, "git reset {} --hard".format(quiet_flags).split()
            )
        location_info = " (in {})".format(decode(repo)) if repo != b"." else ""
        for cmd in cleanup_cmds:
            if show_debuginfo:
                print(
                    "[DEBUG] Running{}: {}".format(
                        location_info, " ".join(cmd)
                    )
                )
    ...
    def _prunable(
        self, commit, new_1st_parent, had_file_changes, orig_parents
    ):
        ...
        if len(parents) < 2:
            ...
            if not had_file_changes and not always_prune:
                had_parents_pruned = len(parents) < len(orig_parents) or (
                    len(orig_parents) == 1
                    and orig_parents[0] in _SKIPPED_COMMITS
                )
        ...
        if not _IDS.has_renames():
            ...
            # the parent.
            changed_files = set(
                change.filename for change in commit.file_changes
            )
        ...
        for change in commit.file_changes:
            parent = (
                new_1st_parent or commit.parents[0]
            )  # exists due to above checks
    ...
    def _tweak_blob(self, blob):
        if (
            ...
            and len(blob.data) > self._args.max_blob_size
        ):
            blob.skip()
    ...
    def _filter_files(self, commit):
        ...
        def newname(
            path_changes, pathname, use_base_name, filtering_is_inclusive
        ):
            ...
            for mod_type, match_type, path_exp in path_changes:
                if mod_type == "filter" and not wanted:
                    ...
                    if match_type == "match" and filename_matches(
                        path_exp, pathname
                    ):
                        wanted = True
                    if match_type == "glob" and fnmatch.fnmatch(
                        pathname, path_exp
                    ):
                        wanted = True
            ...
            return (
                full_pathname if (wanted == filtering_is_inclusive) else None
            )

        args = self._args
        new_file_changes = (
            {}
        )  # Assumes no renames or copies, otherwise collisions
        for change in commit.file_changes:
    ...
    def _tweak_commit(self, commit, aux_info):
        ...
        if self._email_callback:
            ...
            commit.committer_email = self._email_callback(
        ...
        if self._args.tag_rename:
            commit.branch = RepoFilter._do_tag_rename(
                args.tag_rename, commit.branch
            )
        ...
        parents, new_1st_parent = self._trim_extra_parents(
        ...
        if self._args.replace_text or self._blob_callback:
            ...
            differences = orig_file_changes.symmetric_difference(
                final_file_changes
            )
        ...
        if not self._args.quiet:
            self._progress_writer.show(
                self._parsed_message % self._num_commits
            )
    ...
    def _tweak_reset(self, reset):
        if self._args.tag_rename:
            reset.ref = RepoFilter._do_tag_rename(
                self._args.tag_rename, reset.ref
            )
        if self._refname_callback:
    ...
    def _load_marks_file(self, marks_basename):
        ...
        if subproc.call(cmd, stdout=subprocess.DEVNULL) == 0:
            ...
            try:
                ...
                raise SystemExit(
                    _("Failed loading %s from %s")
                    % (decode(marks_basename), full_branch)
                )
        if contents:
            biggest_id = max(
                int(x.split()[0][1:]) for x in contents.splitlines()
            )
    ...
    def _save_marks_files(self):
        ...
        subproc.call(
            ["git", "-C", working_dir, "update-ref", full_branch, commit]
        )
        ...
        if self._args.stdin:
            ...
            if skip_blobs:
                ...
                if self._args.max_blob_size:
                    self._unpacked_size, packed_size = (
                        GitUtils.get_blob_sizes()
                    )
            if use_done_feature:
            ...
            if (
                self._args.preserve_commit_encoding is not None
            ):  # pragma: no cover
                reencode = (
                    "no" if self._args.preserve_commit_encoding else "yes"
                )
            ...
            self._fep = subproc.Popen(
                fep_cmd, bufsize=-1, stdout=subprocess.PIPE
            )
            ...
            if self._args.dry_run or self._args.debug:
                ...
                if self._args.debug:
                    tmp = [
                        decode(x) if isinstance(x, bytes) else x
                        for x in fep_cmd
                    ]
    ...
    def _migrate_origin_to_heads(self):
        ...
        if p.wait():
            raise SystemExit(
                _("git update-ref failed; see above")
            )  # pragma: no cover
    ...
    def _ref_update(self, target_working_dir):
        ...
        replace_refs = {
            k: v
            for k, v in self._orig_refs.items()
        ...
        self._flush_renames()
        actual_renames = {
            k: v for k, v in self._commit_renames.items() if k != v
        }
            ...
            p.stdin.write(
                b"".join(
                    [
                        ...
                        if new
                        and not (update_only and old in reverse_replace_refs)
                    ]
                )
            )
    ...
    def _record_metadata(self, metadata_dir, orig_refs):
        ...
        batch_check_output_re = re.compile(
            b"^([0-9a-f]{40}) ([a-z]+) ([0-9]+)$"
        )
        with open(os.path.join(metadata_dir, b"ref-map"), "bw") as f:
            ...
            if self._args.source or self._args.target:
                ...
                for ref, new_hash in new_refs.items():
                    if ref not in orig_refs and not ref.startswith(
                        b"refs/replace/"
                    ):
        ...
        with open(os.path.join(metadata_dir, b"suboptimal-issues"), "bw") as f:
          ...
          """
                ...
                for (
                    bad_commit_reference
                ) in self._commits_referenced_but_removed:
    ...
    def run(self):
        ...
        print(
            _("Completely finished after {:.2f} seconds.").format(
                time.time() - start
            )
        )
