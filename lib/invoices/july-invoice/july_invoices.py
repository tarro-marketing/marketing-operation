...
def extract_info(text, file_name):
    ...
    amount_pattern = (
        r"(?i)(amount due|balance due|\$\(USD\)|net amount \(usd\)|total \(usd\)"
        r"|total amount|amount due \(usd\)\:|total at checkout|amount due \(usd\)"
        r"|grand total|total|total cost|payment currency/payment amount|amount paid)"
        r"[:\s]*[\$]?\(?([\d,]+\.\d{2})\)?\s*(usd)?"
    )
    ...
    vendor_names = [
        ...
        "Hotjar Ltd",
        "Chihuo Inc",
        "North America Asian Food Industry Association \\(NAAFIA\\)",
        "TAO Marketing Inc.",
    ]

    # Company names to exclude (case-insensitive)
    company_names_to_exclude = [
        "Wonders Technologies Corp.",
        "SKT Technologies Inc.",
        "SKT TECHNOLOGIES, INC.",
    ]

    # Compile regex pattern for vendor names with case insensitivity
    vendor_pattern = r"(?i)" + r"|".join(
        [re.escape(name) for name in vendor_names]
    )

    # Compile regex pattern to exclude company names with case insensitivity
    exclude_pattern = r"(?i)" + r"|".join(
        [re.escape(name) for name in company_names_to_exclude]
    )
    ...
    vendor_name = (
        ...
        if vendor_match and not exclude_match
        else "Not Found"
    )
...
output_excel = (
    "/Users/yukachen/marketing-operation/invoices/july_invoice_summary.xlsx"
)
