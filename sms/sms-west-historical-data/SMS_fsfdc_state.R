...
ggplot(map_data, aes(x = long, y = lat, group = group, fill = MEL)) +
  ...
  labs(title = "MEL Heat Map by State (SMS)", fill = "MEL") +
  theme_minimal()
ggsave("sms/sms-west-historical-data/SMS_MEL_Heat_Map.png", width = 10, height = 8)
...
ggplot(map_data, aes(x = long, y = lat, group = group, fill = MQL)) +
  ...
  coord_fixed(1.3) +
  scale_fill_gradient(low = "#D3D3D3", high = "#6317cb", na.value = "white") +
  labs(title = "MQL Heat Map by State (SMS)", fill = "MQL") +
  theme_minimal()
ggsave("sms/sms-west-historical-data/SMS_MQL_Heat_Map.png", width = 10, height = 8)
...
ggplot(map_data, aes(x = long, y = lat, group = group, fill = SQL)) +
  ...
  coord_fixed(1.3) +
  scale_fill_gradient(low = "#D3D3D3", high = "#6317cb", na.value = "white") +
  labs(title = "SQL Heat Map by State (SMS)", fill = "SQL") +
  theme_minimal()
ggsave("sms/sms-west-historical-data/SMS_SQL_Heat_Map.png", width = 10, height = 8)
...
ggplot(map_data, aes(x = long, y = lat, group = group, fill = Onboarded)) +
  ...
  coord_fixed(1.3) +
  scale_fill_gradient(low = "#D3D3D3", high = "#6317cb", na.value = "white") +
  labs(title = "Onboarded Heat Map by State (SMS)", fill = "Onboarded") +
  theme_minimal()
ggsave("sms/sms-west-historical-data/SMS_Onboarded_Heat_Map.png", width = 10, height = 8)
