...
class VisualPrologBaseLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"\s+", Whitespace),
            (
                words(minorendkw, prefix=r"\bend\s+", suffix=r"\b"),
                Keyword.Minor,
            ),
            (r"end", Keyword),
...
class VisualPrologLexer(VisualPrologBaseLexer):
    ...
    def analyse_text(text):
        ...
        if re.search(
            r"^\s*(end\s+(interface|class|implement)|(clauses|predicates|domains|facts|constants|properties)\s*$)",
            text,
        ):
            return 0.98
        else:
...
class VisualPrologGrammarLexer(VisualPrologBaseLexer):
    ...
    def analyse_text(text):
        ...
        if re.search(
            r"^\s*(end\s+grammar|(nonterminals|startsymbols|terminals|rules|precedence)\s*$)",
            text,
        ):
            return 0.98
        else:
