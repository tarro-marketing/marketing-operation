    ...
    def __init__(
        ...
        if isinstance(cmd_or_spawn, basestring):
            self.child = pexpect.spawn(
                cmd_or_spawn, echo=False, encoding="utf-8"
            )
        else:
...
def python(command=sys.executable):
    ...
    return REPLWrapper(
        command, ">>> ", "import sys; sys.ps1={0!r}; sys.ps2={1!r}"
    )
...
def _repl_sh(command, args, non_printable_insert):
    ...
    return REPLWrapper(
        child, "\\$", prompt_change, extra_init_cmd="export PAGER=cat"
    )
...
def bash(command="bash"):
    ...
    return _repl_sh(
        command, ["--rcfile", bashrc], non_printable_insert="\\[\\]"
    )
