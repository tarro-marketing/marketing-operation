...
def _is_tuple_constant(node):  # type: (ast.AST) -> bool
    return isinstance(node, ast.Tuple) and all(
        _is_constant(elt) for elt in node.elts
    )
...
def _is_typing_helper(node, is_name_match_fn, scope_stack):
    ...
    def _module_scope_is_typing(name):
        for scope in reversed(scope_stack):
            if name in scope:
                return (
                    isinstance(scope[name], Importation)
                    and scope[name].fullName in TYPING_MODULES
                )
...
def is_typing_overload(value, scope_stack):
    return isinstance(
        value.source, (ast.FunctionDef, ast.AsyncFunctionDef)
    ) and any(
...
class Checker:
        ...
        for scope in self.deadScopes:
            ...
            if isinstance(scope, FunctionScope):
                ...
                for name, binding in scope.unused_annotations():
                    self.report(
                        messages.UnusedAnnotation, binding.source, name
                    )
            ...
            if all_binding:
                ...
                undefined = [
                    name for name in all_binding.names if name not in scope
                ]
            else:
                all_names = undefined = []
    ...
    def handleNodeLoad(self, node, parent):
        ...
        for scope in self.scopeStack[-1::-1]:
            ...
            if (
                ...
                and not self._in_postponed_annotation
            ):
        ...
        if name in DetectClassScopedMagic.names and isinstance(
    ...
    def handleNodeStore(self, node):
        ...
        if (
            isinstance(parent_stmt, ast.AnnAssign)
            and parent_stmt.value is None
        ):
    ...
    def _in_postponed_annotation(self):
        return (
            self._in_annotation == AnnotationState.STRING
    ...
    # "stmt" type nodes
    DELETE = FOR = ASYNCFOR = WHILE = WITH = WITHITEM = ASYNCWITH = EXPR = (
        ASSIGN
    ) = handleChildren
    ...
    # "expr" type nodes
    BOOLOP = UNARYOP = SET = ATTRIBUTE = STARRED = NAMECONSTANT = NAMEDEXPR = (
        handleChildren
    )
    ...
    def CALL(self, node):
        ...
        if (
            _is_typing(node.func, "cast", self.scopeStack)
            and len(node.args) >= 1
        ):
            with self._enter_annotation():
            ...
            if (
                ...
                not_annotated += [
                    (elt.elts[0], None) for elt in node.args[1].elts
                ]
                not_annotated += [
    ...
    # same for operators
    AND = OR = ADD = SUB = MULT = DIV = MOD = POW = LSHIFT = RSHIFT = BITOR = (
        BITXOR
    ) = BITAND = FLOORDIV = INVERT = NOT = UADD = USUB = EQ = NOTEQ = LT = (
        LTE
    ) = GT = GTE = IS = ISNOT = IN = NOTIN = MATMULT = ignore
    ...
    def NAME(self, node):
        ...
        if isinstance(node.ctx, ast.Load):
            ...
            # Unknown context
            raise RuntimeError(
                f"Got impossible expression context: {node.ctx!r}"
            )
        ...
        for alias in node.names:
            ...
            if node.module == "__future__":
                ...
                if alias.name not in __future__.all_feature_names:
                    self.report(
                        messages.FutureFeatureNotDefined, node, alias.name
                    )
