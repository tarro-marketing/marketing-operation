...
class Evaluator:
    ...
    def __getitem__(self, node: ast.expr) -> Any:
        ...
        if not isinstance(node, ast.expr):
            raise TypeError(
                "node should be an ast.expr, not {!r}".format(
                    type(node).__name__
                )
            )
    ...
    def _handle_call(self, node):
        ...
        if (
            ...
            or len(args) == 0
            and is_any(
                func, set, dict, str, frozenset, bytes, bytearray, object
            )
            or len(args) >= 2
        ...
        if len(args) == 1:
            ...
            if is_any(func, all, any, sum):
                of_type(
                    arg, tuple, frozenset, list, set, dict, OrderedDict, deque
                )
                for x in arg:
    ...
    def _handle_container(
        self, node: Union[ast.List, ast.Tuple, ast.Set, ast.Dict]
        ...
        # Set and Dict
        if not all(
            is_standard_types(elt, check_dict_values=False, deep=True)
            for elt in elts
        ):
            raise CannotEval
    ...
    def interesting_expressions_grouped(
        self, root: ast.AST
