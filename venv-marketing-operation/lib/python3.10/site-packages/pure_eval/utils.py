...
def _is_standard_types_deep(x, check_dict_values: bool, deep: bool):
    ...
    if is_any(
        typ, tuple, frozenset, list, set, dict, OrderedDict, deque, slice
    ):
        if typ in [slice]:
            length = 0
        else:
...
safe_name_types = tuple({type(f) for f in safe_name_samples.values()})


typing_annotation_types = tuple(
    {type(f) for f in typing_annotation_samples.values()}
)
...
def safe_name(value):
    ...
    if is_any(typ, *safe_name_types):
        ...
        return getattr(value, "__name__", None) or getattr(
            value, "_name", None
        )
    else:
        return None
