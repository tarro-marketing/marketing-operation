...
def _skip_bad_config(
    ...
    if dist is None or (
        ...
        # It seems that the build is not getting any configuration from other
        # places
        return False

    if setuptools_cfg:
        # If `[tool.setuptools]` is set, then `pyproject.toml` config is
        # intentional
        return False
...
class _ConfigExpander:
    ...
    def _expand_all_dynamic(self, dist: "Distribution",
    ...
    def _obtain(self, dist: "Distribution", field: str,
                package_dir: Mapping[str, str]):
    ...
    def _obtain_version(self, dist: "Distribution",
                        package_dir: Mapping[str, str]):
        # Since plugins can set version, let's silently skip if it cannot be
        # obtained
...
class _EnsurePackagesDiscovered(_expand.EnsurePackagesDiscovered):
    ...
    def __enter__(self):
        ...
        # auto-discovery, but avoid overwriting empty lists purposefully set by
        # users.
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        """
        # If anything was discovered set them back, so they count in the final
        # config.
