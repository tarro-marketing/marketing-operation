...
def parse_notifier_name(
    ...
    if names is All or isinstance(names, str):
        ...
        raise TypeError(
            "`names` must be either `All`, a str, or a list of strs."
        )
    else:
        ...
        for n in names:
            if not isinstance(n, str):
                raise TypeError(
                    f"names must be strings, not {type(n).__name__}({n!r})"
                )
        return names
...
def _validate_link(*tuples: t.Any) -> None:
    ...
    for tup in tuples:
        ...
        if not isinstance(obj, HasTraits):
            raise TypeError(
                "Each object must be HasTraits, not %r" % type(obj)
            )
...
class link:
    ...
    def _update_target(self, change: t.Any) -> None:
        ...
        with self._busy_updating():
            setattr(
                self.target[0], self.target[1], self._transform(change.new)
            )
    ...
    def _update_source(self, change: t.Any) -> None:
        ...
        with self._busy_updating():
            setattr(
                self.source[0], self.source[1], self._transform_inv(change.new)
            )
...
class TraitType(BaseDescriptor, t.Generic[G, S]):
    ...
    def __get__(self, obj: HasTraits | None, cls: type[t.Any]) -> Self | G:
        ...
        if obj is None:
            ...
            return t.cast(
                G, self.get(obj, cls)
            )  # the G should encode the Optional
    ...
    def get_metadata(self, key: str, default: t.Any = None) -> t.Any:
        ...
        warn(
            "Deprecated in traitlets 4.1, " + msg,
            DeprecationWarning,
            stacklevel=2,
        )
...
class _CallbackWrapper:
    ...
    def __init__(self, cb: t.Any) -> None:
        ...
        if self.nargs > 4:
            raise TraitError(
                "a trait changed callback must have 0-4 arguments."
            )
            ...
            if isinstance(value, TraitType):
                ...
                for c in mro_trait:
                    if default_method_name in c.__dict__:
                        cls._all_trait_default_generators[name] = c.__dict__[
                            default_method_name
                        ]
                        break
                    ...
                    if (
                        ...
                        cls._static_immutable_initial_values[name] = (
                            trait.default_value
                        )
                    elif (
                        ...
                        isinstance(
                            trait.default_value, (str, int, float, bool)
                        )
                        or none_ok
                    ):
                        cls._static_immutable_initial_values[name] = (
...
class HasTraits(HasDescriptors, metaclass=MetaHasTraits):
    ...
    def __init__(self, *args: t.Any, **kwargs: t.Any) -> None:
        ...
        if kwargs:
            ...
            for key in changed:
                value = self._traits[key]._cross_validate(
    ...
    def hold_trait_notifications(self) -> t.Any:
        ...
        if self._cross_validation_lock:
            ...
            try:
                ...
                self.notify_change = (
                    lambda x: None
                )  # type:ignore[method-assign, assignment]  # noqa: ARG005
    ...
    def set_trait(self, name: str, value: t.Any) -> None:
        ...
        if not self.has_trait(name):
            raise TraitError(
                f"Class {cls.__name__} does not have a trait named {name}"
            )
        getattr(cls, name).set(self, value)

    @classmethod
    def class_trait_names(
    ...
    def trait_values(self, **metadata: t.Any) -> dict[str, t.Any]:
        ...
        """
        return {
            name: getattr(self, name) for name in self.trait_names(**metadata)
        }
    ...
    def trait_defaults(
        ...
        for n in names:
            if not self.has_trait(n):
                raise TraitError(
                    f"'{n}' is not a trait of '{type(self).__name__}' instances"
                )

        if len(names) == 1 and len(metadata) == 0:
            return t.cast(
                Sentinel, self._get_trait_default_generator(names[0])(self)
            )
    ...
    def trait_metadata(
        self, traitname: str, key: str, default: t.Any = None
    ) -> t.Any:
        ...
        if hasattr(self, metadata_name) and key in getattr(
            self, metadata_name
        ):
    ...
    @classmethod
    def class_own_trait_events(
        cls: type[HasTraits], name: str
    ...
    @classmethod
    def trait_events(
        cls: type[HasTraits], name: str | None = None
    ) -> dict[str, EventHandler]:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (klass is not None) and (
            inspect.isclass(klass) or isinstance(klass, str)
        ):
            self.klass = klass
        else:
            raise TraitError(
        ...
        super().__init__(
            allow_none=allow_none, read_only=read_only, help=help, **kwargs
        )
...
class Any(TraitType[t.Optional[t.Any], t.Optional[t.Any]]):
    ...
    if t.TYPE_CHECKING:
        ...
        def __get__(
            self, obj: t.Any | None, cls: type[t.Any]
        ) -> t.Any | Any: ...
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        if isinstance(value, bytes):
            try:
                ...
                raise TraitError(
                    msg.format(value, self.name, class_of(obj))
                ) from e
        self.error(obj, value)
...
class DottedObjectName(ObjectName):
    ...
    def validate(self, obj: t.Any, value: t.Any) -> str:
        ...
        if isinstance(value, str) and all(
            isidentifier(a) for a in value.split(".")
        ):
            return value
        self.error(obj, value)
...
class FuzzyEnum(Enum[G]):
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        conv_func = (
            (lambda c: c) if self.case_sensitive else lambda c: c.lower()
        )
        ...
        choices = self.values or []
        matches = [
            match_func(value, conv_func(c)) for c in choices
...
class Container(Instance[T]):
    ...
    def __init__(
        ...
        **kwargs : any
        ...
        if (
            trait is not None
            and default_value is Undefined
            and not is_trait(trait)
        ):
            default_value = trait
            trait = None
...
class List(Container[t.List[T]]):
    ...
    def __init__(
        ...
        default_value: (
            t.List[T] | t.Tuple[T] | t.Set[T] | Sentinel | None
        ) = Undefined,
        minlen: int = 0,
...
class Tuple(Container[t.Tuple[t.Any, ...]]):
    ...
    def __init__(self, *traits: t.Any, **kwargs: t.Any) -> None:
        ...
        **kwargs
        ...
        if self._traits and (
            default_value is None or default_value is Undefined
        ):
    ...
    def item_from_string(
        self, s: str, index: int
    ) -> t.Any:  # type:ignore[override]
        """Cast a single item from a string
...
class Dict(Instance["dict[K, V]"]):
    ...
    def __init__(
        self,
        value_trait: (
            TraitType[t.Any, t.Any] | dict[K, V] | Sentinel | None
        ) = None,
        ...
        **kwargs: t.Any,
        ...
        if is_trait(value_trait):
            ...
            raise TypeError(
                "`value_trait` must be a Trait or None, got %s"
                % repr_type(value_trait)
            )
        ...
        if is_trait(key_trait):
            ...
            raise TypeError(
                "`key_trait` must be a Trait or None, got %s"
                % repr_type(key_trait)
            )
    ...
    def from_string(self, s: str) -> dict[K, V] | None:
        ...
        if not isinstance(s, str):
            raise TypeError(
                f"from_string expects a string, got {s!r} of type {type(s)}"
            )
        try:
...
class TCPAddress(TraitType[G, S]):
    ...
    if t.TYPE_CHECKING:
        ...
        def __init__(
            self: (
                TCPAddress[tuple[str, int] | None, tuple[str, int] | None]
...
class CRegExp(
    TraitType["re.Pattern[t.Any]", t.Union["re.Pattern[t.Any]", str]]
):
...
class UseEnum(TraitType[t.Any, t.Any]):
    ...
    def select_by_number(
        self, value: int, default: t.Any = Undefined
    ) -> t.Any:
    ...
    def _choices_str(self, as_rst: bool = False) -> str:
        ...
        if as_rst:
            ...
            return repr(
                list(choices)
            )  # Listify because py3.4- prints odict-class
