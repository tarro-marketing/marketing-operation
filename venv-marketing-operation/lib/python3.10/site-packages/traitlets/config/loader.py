        ...
        for section in self:
            ...
            for key in mine:
                if key in theirs and mine[key] != theirs[key]:
                    ...
                    collisions[section][
                        key
                    ] = f"{mine[key]!r} ignored, using {theirs[key]!r}"
        return collisions
...
class PyFileConfigLoader(FileConfigLoader):
    ...
    def _read_file_as_dict(self) -> None:
        ...
        with open(conf_filename, "rb") as f:
            exec(
                compile(f.read(), conf_filename, "exec"), namespace, namespace
            )  # noqa: S102
...
class CommandLineConfigLoader(ConfigLoader):
    ...
    def _exec_config_str(
        ...
        lhs: t.Any,
        rhs: t.Any,
        trait: TraitType[t.Any, t.Any] | None = None,
    ) -> None:
...
class KVArgParseConfigLoader(ArgParseConfigLoader):
        ...
        for keys, traitname in aliases.items():
            ...
            for key in keys:
                ...
                if traitname in argparse_traits:
                    ...
                    argcompleter = trait.metadata.get(
                        "argcompleter"
                    ) or getattr(trait, "argcompleter", None)
                ...
                if argcompleter is not None:
                    ...
                    action.completer = (
                        functools.partial(  # type:ignore[attr-defined]
    ...
    def _convert_to_config(self) -> None:
        ...
        for lhs, rhs in vars(self.parsed_data).items():
            if lhs == "extra_args":
                self.extra_args = [
                    "-" if a == _DASH_REPLACEMENT else a for a in rhs
                ] + extra_args
                continue
            if lhs == "_flags":
            ...
            try:
                ...
                if isinstance(rhs, DeferredConfig):
                    rhs = rhs._super_repr()
                raise ArgumentError(
                    f"Error loading argument {lhs}={rhs}, {e}"
                ) from e
    ...
    def _handle_unrecognized_alias(self, arg: str) -> None:
        ...
        """
        self.log.warning(
            "Unrecognized alias: '%s', it will have no effect.", arg
        )
    ...
    def _argcomplete(
        ...
        finder = (
            argcomplete_config.ExtendedCompletionFinder()
