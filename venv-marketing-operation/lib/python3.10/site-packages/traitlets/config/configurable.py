...
class Configurable(HasTraits):
    ...
    def _config_changed(self, change: Bunch) -> None:
        ...
        section_names = self.section_names()
        self._load_config(
            change.new, traits=traits, section_names=section_names
        )
    ...
    def class_get_trait_help(
        ...
        if inst is not None:
            lines.append(
                indent(f"Current: {getattr(inst, trait.name or '')!r}")
            )
        else:
            try:
    ...
    def _defining_class(
        cls,
        trait: TraitType[t.Any, t.Any],
        classes: t.Sequence[type[HasTraits]],
    ) -> type[Configurable]:
        ...
        for parent in cls.mro():
            if (
                ...
                and parent.class_own_traits(config=True).get(trait.name, None)
                is trait
            ):
                defining_cls = parent
        return defining_cls
    ...
    @classmethod
    def class_config_section(
        cls, classes: t.Sequence[type[HasTraits]] | None = None
    ) -> str:
        ...
        breaker = "#" + "-" * 78
        parent_classes = ", ".join(
            p.__name__ for p in cls.__bases__ if issubclass(p, Configurable)
        )
...
class LoggingConfigurable(Configurable):
    ...
    def _validate_log(self, proposal: Bunch) -> LoggerType:
        if not isinstance(
            proposal.value, (logging.Logger, logging.LoggerAdapter)
        ):
