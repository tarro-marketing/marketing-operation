    ...
    class StubModule:
        def __getattr__(self, attr: str) -> t.Any:
            ...
            raise AttributeError(
                f"argcomplete stub module has no attribute '{attr}'"
            )
...
class ExtendedCompletionFinder(CompletionFinder):
    ...
    def match_class_completions(
        self, cword_prefix: str
    ) -> t.List[t.Tuple[t.Any, str]]:
        ...
        """
        class_completions = [
            (cls, f"--{cls.__name__}.") for cls in self.config_classes
        ]
        matched_completions = class_completions
        if "." in cword_prefix:
            cword_prefix = cword_prefix[: cword_prefix.index(".") + 1]
            matched_completions = [
                (cls, c) for (cls, c) in class_completions if c == cword_prefix
            ]
        elif len(cword_prefix) > 0:
            matched_completions = [
                (cls, c)
                for (cls, c) in class_completions
                if c.startswith(cword_prefix)
            ]
    ...
    def _get_completions(
        ...
        if is_option:
            ...
            for prev_word in comp_words[::-1]:
                if len(prev_word) > 0 and prev_word[0] in prefix_chars:
                    matched_completions = self.match_class_completions(
                        prev_word
                    )
        ...
        if self.subcommands and len(comp_words) == 1:
            argcomplete.debug(
                "Adding subcommands for", cword_prefix
            )  # type:ignore[attr-defined,no-untyped-call]
            completions.extend(
                subc
                for subc in self.subcommands
                if subc.startswith(cword_prefix)
            )
    ...
    def _get_option_completions(
        ...
        completions: t.List[str]
        completions = super()._get_option_completions(
            parser, cword_prefix
        )  # type:ignore[no-untyped-call]
