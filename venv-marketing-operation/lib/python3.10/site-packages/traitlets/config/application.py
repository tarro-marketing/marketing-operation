...
def catch_config_error(method: T) -> T:
    ...
    def inner(app: Application, *args: t.Any, **kwargs: t.Any) -> t.Any:
        try:
            ...
            app.log.fatal(
                "Bad config encountered during initialization: %s", e
            )
...
class Application(SingletonConfigurable):
    ...
    # of the help.
    description: str | Unicode[str, str | bytes] = Unicode(
        "This is an application."
    )
    # default section descriptions
    option_description: str | Unicode[str, str | bytes] = Unicode(
        option_description
    )
    keyvalue_description: str | Unicode[str, str | bytes] = Unicode(
        keyvalue_description
    )
    subcommand_description: str | Unicode[str, str | bytes] = Unicode(
        subcommand_description
    )
    ...
    subapp = Instance(
        "traitlets.config.application.Application", allow_none=True
    )
    ...
    def start_show_config(self) -> None:
        ...
        if self.show_config_json:
            json.dump(
                config, sys.stdout, indent=1, sort_keys=True, default=repr
            )
    ...
    def emit_alias_help(self) -> t.Generator[str, None, None]:
        ...
        for alias, longname in self.aliases.items():
            try:
                ...
                fhelp_lines = cls.class_get_trait_help(
                    trait, helptext=fhelp
                ).splitlines()
    ...
    def document_config_options(self) -> str:
        ...
        """
        return "\n".join(
            c.class_config_rst_doc() for c in self._classes_inc_parents()
        )
    ...
    def initialize_subcommand(self, subc: str, argv: ArgvType = None) -> None:
        ...
        if isinstance(subapp, type) and issubclass(subapp, Application):
            ...
            raise AssertionError(
                "Invalid mappings for subcommand '%s'!" % subc
            )
    ...
    def parse_command_line(self, argv: ArgvType = None) -> None:
        ...
        if argv is None:
            argv = self._get_sys_argv(
                check_argcomplete=bool(self.subcommands)
            )[1:]
    ...
    def _load_config_files(
        ...
        for current in reversed(path):
            ...
            pyloader = cls.python_config_loader_class(
                basefilename + ".py", path=current, log=log
            )
            if log:
                log.debug(
                    "Looking for %s in %s",
                    basefilename,
                    current or os.getcwd(),
                )
            jsonloader = cls.json_config_loader_class(
                basefilename + ".json", path=current, log=log
            )
            loaded: list[t.Any] = []
            filenames: list[str] = []
            for loader in [pyloader, jsonloader]:
                ...
                try:
                    ...
                    if log:
                        log.debug(
                            "Loaded config file: %s", loader.full_filename
                        )
                if config:
    ...
    def load_config_environ(self) -> None:
        ...
        self.log.debug(
            'Looping through config variables with prefix "%s"', PREFIX
        )
    ...
    def _classes_with_config_traits(
        ...
        def is_any_parent_included(cls: t.Any) -> bool:
            return any(
                b in cls_to_config and cls_to_config[b] for b in cls.__bases__
            )
...
def boolean_flag(
    name: str, configurable: str, set_help: str = "", unset_help: str = ""
) -> StrDict:
