...
class Token(
    collections.namedtuple(
        "Token", "type string start end line index startpos endpos"
    )
):
    """
...
def match_token(token, tok_type, tok_str=None):
    ...
    return token.type == tok_type and (
        tok_str is None or token.string == tok_str
    )
...
def generate_tokens(text):
    ...
    return tokenize.generate_tokens(
        cast(Callable[[], str], io.StringIO(text).readline)
    )
...
def iter_children_func(node):
    ...
    """
    return (
        iter_children_astroid
        if hasattr(node, "get_children")
        else iter_children_ast
    )
...
SINGLETONS = {
    c
    for n, c in iteritems(ast.__dict__)
    if isinstance(c, type)
    and issubclass(
        c, (ast.expr_context, ast.boolop, ast.operator, ast.unaryop, ast.cmpop)
    )
}
...
def is_slice(node):
    ...
    return node.__class__.__name__ in ("Slice", "ExtSlice") or (
...
def visit_tree(node, previsit, postvisit):
    ...
    * ``postvisit(node, par_value, value)`` - should return ``value``
    ...
    stack = [
        (node, None, _PREVISIT)
    ]  # type: List[Tuple[AstNode, Optional[Token], Union[Optional[Token], object]]]
    while stack:
        current, par_value, value = stack.pop()
        if value is _PREVISIT:
            assert (
                current not in done
            )  # protect againt infinite loop in case of a bad tree.
            done.add(current)
...
class NodeMethods(object):
    ...
    def __init__(self):
        ...
        self._cache = (
            {}
        )  # type: Dict[Union[ABCMeta, type], Callable[[AstNode, Token, Token], Tuple[Token, Token]]]
...
if sys.version_info[0] == 2:
    ...
    def patched_generate_tokens(original_tokens):
        ...
        for tok in original_tokens:
            if (
                tok.type
                in (tokenize.NAME, tokenize.ERRORTOKEN, tokenize.NUMBER)
    ...
    def combine_tokens(group):
        ...
        if (
            not any(tok.type == tokenize.ERRORTOKEN for tok in group)
            or len({tok.line for tok in group}) != 1
        ):
            return group
        return [
            tokenize.TokenInfo(
...
def last_stmt(node):
    ...
    child_stmts = [
        child
        for child in iter_children_func(node)(node)
        if is_stmt(child)
...
if sys.version_info[:2] >= (3, 8):
    ...
    def fstring_positions_work():
      ...
      '''
        ...
        name_positions = [
            (node.lineno, node.col_offset) for node in name_nodes
        ]
    ...
    def annotate_fstring_nodes(tree):
        ...
        for joinedstr in walk(tree, include_joined_str=True):
            ...
            for part in joinedstr.values:
                ...
                setattr(
                    part, "_broken_positions", True
                )  # use setattr for mypy
