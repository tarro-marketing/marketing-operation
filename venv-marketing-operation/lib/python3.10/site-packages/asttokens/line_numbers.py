...
class LineNumbers(object):
    ...
    def __init__(self, text):
        ...
        self._line_offsets = [
            m.start(0) for m in _line_start_re.finditer(text)
        ]
        self._text = text
        self._text_len = len(text)
        self._utf8_offset_cache = (
            {}
        )  # type: Dict[int, List[int]] # maps line num to list of char offset for each byte in line
    ...
    def from_utf8_col(self, line, utf8_column):
        ...
        if offsets is None:
            ...
            offsets = [
                i for i, c in enumerate(line_text) for byte in c.encode("utf8")
            ]
    ...
    def line_to_offset(self, line, column):
        ...
        if line >= len(self._line_offsets):
            ...
            return min(
                self._line_offsets[line] + max(0, column), self._text_len
            )
