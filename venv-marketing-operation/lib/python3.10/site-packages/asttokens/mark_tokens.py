...
from .astroid_compat import (
    astroid_node_classes as nc,
    BaseContainer as AstroidBaseContainer,
)
...
class MarkTokens(object):
    ...
    def visit_tree(self, node):
        ...
        util.visit_tree(
            node, self._visit_before_children, self._visit_after_children
        )
    ...
    def _visit_before_children(self, node, parent_token):
        ...
        token = (
            self._code.get_token_from_utf8(node.lineno, col)
            if col is not None
            else None
        )
    ...
    def _visit_after_children(self, node, parent_token, token):
        ...
        first, last = self._expand_to_matching_pairs(
            cast(util.Token, first), cast(util.Token, last), node
        )

        # Give a chance to node-specific methods to adjust.
        nfirst, nlast = self._methods.get(self, node.__class__)(
            node, first, last
        )
    ...
    def _find_last_in_stmt(self, start_token):
        ...
        t = start_token
        while (
            not util.match_token(t, token.NEWLINE)
    ...
    def visit_comprehension(
        ...
        first = self._code.find_token(
            first_token, token.NAME, "for", reverse=True
        )
    ...
    def handle_following_brackets(self, node, last_token, opening_bracket):
        ...
        call_start = self._code.find_token(
            first_child.last_token, token.OP, opening_bracket
        )
    ...
    if sys.version_info >= (3, 8):
        ...
        def handle_tuple_nonempty(self, node, first_token, last_token):
            # type: (AstNode, util.Token, util.Token) -> Tuple[util.Token, util.Token]
            assert isinstance(node, ast.Tuple) or isinstance(
                node, AstroidBaseContainer
            )
            ...
            child_first, child_last = self._gobble_parens(
                child.first_token, child.last_token, True
            )
        ...
        def handle_tuple_nonempty(self, node, first_token, last_token):
            # type: (AstNode, util.Token, util.Token) -> Tuple[util.Token, util.Token]
            (first_token, last_token) = self.handle_bare_tuple(
    ...
    def _gobble_parens(self, first_token, last_token, include_all=False):
        ...
        while first_token.index > 0:
            ...
            if util.match_token(prev, token.OP, "(") and util.match_token(
                next, token.OP, ")"
            ):
    ...
    def visit_num(self, node, first_token, last_token):
        ...
        return self.handle_num(
            node, cast(ast.Num, node).n, first_token, last_token
        )
    ...
    def visit_keyword(self, node, first_token, last_token):
        ...
        if node.arg is not None and getattr(node, "lineno", None) is None:
            equals = self._code.find_token(
                first_token, token.OP, "=", reverse=True
            )
    ...
    if six.PY2:
        ...
        def visit_with(self, node, first_token, last_token):
            # type: (AstNode, util.Token, util.Token) -> Tuple[util.Token, util.Token]
            first = self._code.find_token(
                first_token, token.NAME, "with", reverse=True
            )
            return (first, last_token)
