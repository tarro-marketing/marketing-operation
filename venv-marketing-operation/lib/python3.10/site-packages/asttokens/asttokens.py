...
class ASTTokens(ASTTextBase, object):
    ...
    def get_token(self, lineno, col_offset):
        ...
        # but isn't explicit.
        return self.get_token_from_offset(
            self._line_numbers.line_to_offset(lineno, col_offset)
        )
    ...
    def get_token_from_utf8(self, lineno, col_offset):
        ...
        return self.get_token(
            lineno, self._line_numbers.from_utf8_col(lineno, col_offset)
        )
    ...
    def get_tokens(self, node, include_extra=False):
        ...
        return self.token_range(
            node.first_token, node.last_token, include_extra=include_extra
        )
    ...
    def get_text_positions(self, node, padded):
        ...
        if padded and any(
            match_token(t, token.NEWLINE) for t in self.get_tokens(node)
        ):
    ...
    def _get_text_positions_tokenless(self, node, padded):
        ...
        if sys.version_info[:2] < (3, 8):  # pragma: no cover
            ...
            raise AssertionError(
                "This method should only be called internally after checking supports_tokenless()"
            )
        ...
        if padded and (
            ...
            start_col_offset = self._line_numbers.from_utf8_col(
                start_lineno, start_node.col_offset
            )
        ...
        end_col_offset = self._line_numbers.from_utf8_col(
        ...
        and not (
            ...
            and (
                (
                    ...
                    and type(node.parent).__name__
                    in ("Arguments", "ExceptHandler")
                )
            )
        )
