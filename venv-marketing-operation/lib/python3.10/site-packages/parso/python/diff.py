...
def _assert_valid_graph(node):
    ...
    try:
        ...
        if (
            node.type == "error_leaf"
            and node.token_type in _INDENTATION_TOKENS
        ):
...
def _skip_dedent_error_leaves(leaf):
    while (
        leaf is not None
        and leaf.type == "error_leaf"
        and leaf.token_type == "DEDENT"
    ):
        leaf = leaf.get_previous_leaf()
    return leaf
...
def _func_or_class_has_suite(node):
    ...
    if node.type in ("async_funcdef", "async_stmt"):
        node = node.children[-1]
    return (
        node.type in ("classdef", "funcdef")
        and node.children[-1].type == "suite"
    )
...
class DiffParser:
    ...
    def update(self, old_lines, new_lines):
        ...
        LOG.debug(
            "line_lengths old: %s; new: %s" % (len(old_lines), line_length)
        )

        for operation, i1, i2, j1, j2 in opcodes:
            LOG.debug(
                "-> code[%s] old[%s:%s] new[%s:%s]",
                operation,
                i1 + 1,
                i2,
                j1 + 1,
        ...
        if DEBUG_DIFF_PARSER:
            ...
            try:
                ...
                without_diff_parser_module = Parser(
                    self._pgen_grammar, error_recovery=True
                ).parse(self._tokenizer(new_lines))
                _assert_nodes_are_equal(
                    self._module, without_diff_parser_module
                )
            except AssertionError:
                print(
                    _get_debug_error_message(
                        self._module, old_lines, new_lines
                    )
                )
                raise
    ...
    def _copy_from_old_parser(
        self, line_offset, start_line_old, until_line_old, until_line_new
    ):
        last_until_line = -1
        while until_line_new > self._nodes_tree.parsed_until_line:
            parsed_until_line_old = (
                self._nodes_tree.parsed_until_line - line_offset
            )
            ...
            if line_stmt is None:
                ...
                if start_line_old == 1 and p_children[
                    0
                ].get_first_leaf().prefix.startswith(BOM_UTF8_STRING):
                    ...
                    copied_nodes = self._nodes_tree.copy_nodes(
                        p_children[index:], until_line_old, line_offset
                    )
                ...
                if copied_nodes:
                    ...
                    LOG.debug(
                        "copy old[%s:%s] new[%s:%s]",
                        copied_nodes[0].start_pos[0],
                        copied_nodes[-1].end_pos[0] - 1,
                        from_,
                        to,
                    )
                else:
    ...
    def _get_old_line_stmt(self, old_line):
        leaf = self._module.get_leaf_for_position(
            (old_line, 0), include_prefixes=True
        )
    ...
    def _parse(self, until_line):
        ...
        while until_line > self._nodes_tree.parsed_until_line:
            ...
            self._nodes_tree.add_parsed_nodes(
                nodes, self._keyword_token_indents
            )
    ...
    def _try_parse_part(self, until_line):
        ...
        tokens = self._diff_tokenize(
            lines_after, until_line, line_offset=parsed_until_line
        )
        ...
        for token in tokens:
            ...
            if typ == DEDENT:
                ...
                if len(indents) == initial_indentation_count:
                    ...
                    if _suite_or_file_input_is_valid(
                        self._pgen_grammar, stack
                    ):
...
class _NodesTreeNode:
    _ChildrenGroup = namedtuple(
        "_ChildrenGroup", "prefix children line_offset last_line_offset_leaf"
    )
    ...
    def add_tree_nodes(
        self, prefix, children, line_offset=0, last_line_offset_leaf=None
    ):
...
class _NodesTree:
    ...
    def _update_insertion_node(self, indentation):
        for node in reversed(list(self._working_stack)):
            if (
                ...
                or node is self._working_stack[0]
            ):
                return node
    ...
    def _update_parsed_node_tos(self, tree_node, keyword_token_indents):
        if tree_node.type == "suite":
            ...
            self._update_parsed_node_tos(
                tree_node.children[-1], keyword_token_indents
            )
    ...
    def copy_nodes(self, tree_nodes, until_line, line_offset):
        ...
        new_nodes, self._working_stack, self.prefix, added_indents = (
            self._copy_nodes(
    ...
    def _copy_nodes(
        ...
        if _func_or_class_has_suite(last_node):
            ...
            suite_tos = _NodesTreeNode(
                suite, indentation=_get_indentation(last_node)
            )
        ...
        if new_nodes:
            if (
                not _ends_with_newline(new_nodes[-1].get_last_leaf())
    ...
    def close(self):
        ...
        endmarker = EndMarker(
            "", tuple(end_pos), self.prefix + self._prefix_remainder
        )
