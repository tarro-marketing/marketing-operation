...
def tokenize(
    ...
    *,
    ...
    return tokenize_lines(
        lines, version_info=version_info, start_pos=start_pos
    )
    ...
    for line in lines:  # loop over lines in stream
        ...
        if contstr:  # continued string
            ...
            if endmatch:
                pos = endmatch.end(0)
                yield PythonToken(
                    STRING, contstr + line[:pos], contstr_start, prefix
                )  # noqa: F821
                contstr = ""
                contline = ""
            else:
        ...
        while pos < max_:
            ...
            if fstring_stack:
                ...
                for fstring_stack_node in fstring_stack:
                    ...
                    if end_match is not None:
                        ...
                        if len(end_match_string) - len(quote) + pos < len(
                            string_line
                        ):
                            string_line = (
                                line[:pos] + end_match_string[: -len(quote)]
                            )
            ...
            if initial in numchars or (  # ordinary number
                ...
                if token in always_break_tokens and (
                    fstring_stack or paren_level
                ):
                    fstring_stack[:] = []
                    paren_level = 0
...
def _split_illegal_unicode_name(token, start_pos, prefix):
    def create_token():
        return PythonToken(
            ERRORTOKEN if is_illegal else NAME, found, pos, prefix
        )
