...
class _Context:
    ...
    def is_async_funcdef(self):
        ...
        return self.is_function() and self.node.parent.type in (
            "async_funcdef",
            "async_stmt",
        )
    ...
    def finalize(self):
        ...
        for nonlocal_name in self._nonlocal_names_in_subscopes:
            ...
            if search in global_name_strs or self.parent_context is None:
                message = (
                    "no binding for nonlocal '%s' found" % nonlocal_name.value
                )
    ...
    def _analyze_names(self, globals_or_nonlocals, type_):
        def raise_(message):
            self._add_syntax_error(
                base_name, message % (base_name.value, type_)
            )
        ...
        for base_name in globals_or_nonlocals:
            ...
            for name in reversed(
                self._used_name_dict.get(base_name.value, [])
            ):
    ...
    def is_issue(self, node):
        ...
        if check_delete_starred(node):
            if self._normalizer.version >= (3, 9):
                ...
                self.add_issue(
                    node, message="can't use starred expression here"
                )
            return
    ...
    def is_issue(self, node):
        ...
        for argument in node.children:
            ...
            if argument.type == "argument":
                ...
                if (
                    _is_argument_comprehension(argument)
    ...
    def _check_assignment(
        ...
        if error is not None:
            if is_namedexpr:
                ...
                cannot = (
                    "can't" if self._normalizer.version < (3, 8) else "cannot"
                )
                message = " ".join(
    ...
    def is_issue(self, node):
        ...
        if self._normalizer.version <= (3, 8) or not is_aug_assign:
            for before_equal in node.children[:-2:2]:
                self._check_assignment(
                    before_equal, is_aug_assign=is_aug_assign
                )
        ...
        def search_namedexpr_in_comp_for(node):
            while True:
                ...
                if parent is None:
                    return parent
                if (
                    parent.type == "sync_comp_for"
                    and parent.children[3] == node
                ):
                    return parent
                node = parent
