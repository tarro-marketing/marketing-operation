...
class PEP8Normalizer(ErrorFinder):
    ...
    def _visit_node(self, node):
        ...
        if typ in "import_name":
            ...
            for child in node.children:
                ...
                if (
                    ...
                    self.add_issue(
                        node, 721, "Do not compare types, use 'isinstance()"
                    )
                    break
        elif typ == "file_input":
        ...
        if typ == "suite":
            ...
            if typ in ("classdef", "funcdef"):
                self._wanted_newline_count = (
                    self._get_wanted_blank_lines_count()
                )
    ...
    def _visit_part(self, part, spacing, leaf):
        ...
        if type_ == "comment":
            if value.startswith("##"):
                ...
                if value.lstrip("#"):
                    self.add_issue(
                        part, 266, "Too many leading '#' for block comment."
                    )
                ...
                if (
                    ...
                    self.add_issue(
                        part, 265, "Block comment should start with '# '"
                    )
            else:
                if not re.match(r"#:? [^ ]", value):
                    self.add_issue(
                        part, 262, "Inline comment should start with '# '"
                    )
            ...
            if self._newline_count > self._get_wanted_blank_lines_count():
                self.add_issue(
                    part,
                    303,
                    "Too many blank lines (%s)" % self._newline_count,
                )
            elif (
                leaf in ("def", "class")
                and leaf.parent.parent.type == "decorated"
                ...
                self.add_issue(
                    part, 304, "Blank lines found after function decorator"
                )
                ...
                if self._new_statement:
                    ...
                    if (
                        ...
                        self.add_issue(
                            part,
                            129,
                            "Line with same indent as next logical block",
                        )
                    elif indentation != should_be_indentation:
                        if not self._check_tabs_spaces(
                            spacing
                        ) and part.value not in {
                            "\n",
                            "\r\n",
                            "\r",
                        }:
                            if value in "])}":
                                ...
                                if len(indentation) < len(
                                    ...
                                    if (
                                        ...
                                        node.type == IndentationTypes.IMPLICIT
                                    ):
    ...
    def _check_spacing(self, part, spacing):
        ...
        if (
            prev is not None
            and prev.type == "error_leaf"
            or part.type == "error_leaf"
        ):
            return
        ...
        if "\t" in spaces:
            ...
            if prev in _OPENING_BRACKETS:
                ...
                if (
                    ...
                    if param.type == "param" and param.annotation:
                        add_not_spaces(
                            spacing,
                            252,
                            "Expected spaces around annotation equals",
                        )
                    else:
                        add_if_spaces(
                            spacing,
                ...
                add_not_spaces(
                    spacing, 275, "Missing whitespace around keyword"
                )
            else:
                ...
                if (
                    spaces
                    and part not in _ALLOW_SPACE
                    and prev not in _ALLOW_SPACE
                ):
                    message_225 = "Missing whitespace between tokens"
        ...
        if typ == "name" and leaf.value in ("l", "O", "I"):
            if leaf.is_definition():
                ...
                if leaf.parent.type == "class" and leaf.parent.name == leaf:
                    ...
                    leaf.parent.type == "function" and leaf.parent.name == leaf
                ):
            ...
            if (
                isinstance(leaf.parent, (Flow, Scope))
                and leaf.parent.type != "lambdef"
            ):
                next_leaf = leaf.get_next_leaf()
            ...
            if leaf.get_next_leaf().type in ("newline", "endmarker"):
                ...
                self.add_issue(
                    leaf, 702, "Multiple statements on one line (semicolon)"
                )
        elif leaf.value in ("==", "!="):
            ...
            if (
                ...
                and comparison.parent.type == "not_test"
            ):
                if leaf.value == "in":
                    self.add_issue(
                        leaf, 713, "test for membership should be 'not in'"
                    )
                else:
                    self.add_issue(
                        leaf,
