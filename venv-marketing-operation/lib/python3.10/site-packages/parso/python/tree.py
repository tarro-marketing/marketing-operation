...
_FUNC_CONTAINERS = (
    set(["suite", "simple_stmt", "decorated", "async_funcdef"])
    | _FLOW_CONTAINERS
)
...
class String(Literal):
    ...
    def _get_payload(self):
        match = re.search(
            r"""('{3}|"{3}|'|")(.*)$""", self.value, flags=re.DOTALL
        )
...
class ImportFrom(Import):
    ...
    def _aliases(self):
        ...
        return dict(
            (alias, name)
            for name, alias in self._as_name_tuples()
...
class Param(PythonBaseNode):
    ...
    def get_code(self, include_prefix=True, include_comma=True):
        ...
        return self._get_code_for_children(
            children, include_prefix=include_prefix
        )

    def __repr__(self):
        default = (
            "" if self.default is None else "=%s" % self.default.get_code()
        )
        return "<%s: %s>" % (
            type(self).__name__,
