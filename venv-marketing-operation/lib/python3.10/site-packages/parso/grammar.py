...
class Grammar(Generic[_NodeT]):
    ...
    def __init__(
        self, text: str, *, tokenizer, parser=BaseParser, diff_parser=None
    ):
    ...
    def parse(
        ...
        *,
        ...
        if cache and file_io.path is not None:
            module_node = load_module(
                self._hashed, file_io, cache_path=cache_path
            )
        ...
        if diff_cache:
            if self._diff_parser is None:
                raise TypeError(
                    "You have to define a diff parser to be able "
                    "to use this option."
                )
            try:
