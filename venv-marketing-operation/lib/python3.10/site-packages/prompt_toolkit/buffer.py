...
class CompletionState:
    ...
    def __init__(
        ...
        self.complete_index = (
            complete_index  # Position in the `_completions` array.
        )
    ...
    def new_text_and_position(self) -> tuple[str, int]:
        ...
        if self.complete_index is None:
            ...
            original_text_before_cursor = (
                self.original_document.text_before_cursor
            )
            original_text_after_cursor = (
                self.original_document.text_after_cursor
            )
...
class Buffer:
    ...
    def reset(
        ...
        self.yank_nth_arg_state: YankNthArgState | None = (
            None  # for yank-nth-arg.
        )
    ...
    def load_history_if_not_yet_loaded(self) -> None:
        ...
        if self._load_history_task is None:
            ...
            self._load_history_task = get_app().create_background_task(
                load_history()
            )
    ...
    def set_document(
        self, value: Document, bypass_readonly: bool = False
    ) -> None:
        """
        ...
        cursor_position_changed = self._set_cursor_position(
    ...
    def cursor_up(self, count: int = 1) -> None:
        ...
        original_column = (
            self.preferred_column or self.document.cursor_position_col
        )
    ...
    def auto_up(
        ...
        if self.complete_state:
            ...
            if go_to_start_of_line_if_history_changes:
                self.cursor_position += (
                    self.document.get_start_of_line_position()
                )
    ...
    def delete_before_cursor(self, count: int = 1) -> str:
        ...
        if self.cursor_position > 0:
            deleted = self.text[
                self.cursor_position - count : self.cursor_position
            ]
    ...
    def join_selected_lines(self, separator: str = " ") -> None:
        ...
        from_, to = sorted(
            [
                self.cursor_position,
                self.selection_state.original_cursor_position,
            ]
        )
    ...
    def complete_next(
        ...
        if self.complete_state:
            ...
            if self.complete_state.complete_index is None:
                ...
                index = min(
                    completions_count - 1,
                    self.complete_state.complete_index + count,
                )
    ...
    def _history_matches(self, i: int) -> bool:
        ...
        """
        return self.history_search_text is None or self._working_lines[
            i
        ].startswith(self.history_search_text)
    ...
    def start_selection(
        ...
        self.selection_state = SelectionState(
            self.cursor_position, selection_type
        )
        ...
        if overwrite:
            ...
            if "\n" in overwritten_text:
                overwritten_text = overwritten_text[
                    : overwritten_text.find("\n")
                ]

            text = (
                otext[:ocpos] + data + otext[ocpos + len(overwritten_text) :]
            )
        else:
        ...
        def search_once(
            ...
            if direction == SearchDirection.FORWARD:
                ...
                if new_index is not None:
                    ...
                    for i in range(
                        working_index + 1, len(self._working_lines) + 1
                    ):
    ...
    def document_for_search(self, search_state: SearchState) -> Document:
        ...
        if search_result is None:
            ...
            return Document(
                self._working_lines[working_index],
                cursor_position,
                selection=selection,
            )
        ...
        async def run() -> None:
            try:
                ...
                success = await run_in_terminal(
                    lambda: self._open_file_in_editor(filename),
                    in_executor=True,
                )
    ...
    def _create_completer_coroutine(
        ...
        def completion_does_nothing(
            document: Document, completion: Completion
        ) -> bool:
            """
        ...
        async def async_completer(
            ...
            document = self.document
            complete_event = complete_event or CompleteEvent(
                text_inserted=True
            )
            ...
            if proceed():
                ...
                if select_first:
                    ...
                    common_part = get_common_complete_suffix(
                    ...
                    if common_part:
                        ...
                        if len(completions) > 1:
                            ...
                            completions[:] = [
                                c.new_completion_from_position(
                                    len(common_part)
                                )
    ...
    def _create_auto_suggest_coroutine(
        ...
        async def async_suggestor() -> None:
            ...
            suggestion = await self.auto_suggest.get_suggestion_async(
                self, document
            )
...
def unindent(
    buffer: Buffer, from_row: int, to_row: int, count: int = 1
) -> None:
    """
