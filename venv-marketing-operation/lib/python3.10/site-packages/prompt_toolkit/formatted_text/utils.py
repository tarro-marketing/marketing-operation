...
def fragment_list_len(fragments: StyleAndTextTuples) -> int:
    ...
    return sum(
        len(item[1]) for item in fragments if ZeroWidthEscape not in item[0]
    )
...
def split_lines(
    ...
    for style, string, *mouse_handler in fragments:
        ...
        for part in parts[:-1]:
            if part:
                line.append(
                    cast(OneStyleAndTextTuple, (style, part, *mouse_handler))
                )
            yield line
            line = []

        line.append(
            cast(OneStyleAndTextTuple, (style, parts[-1], *mouse_handler))
        )
