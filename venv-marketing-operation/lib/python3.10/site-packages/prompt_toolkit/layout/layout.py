...
class Layout:
        ...
        if isinstance(value, str):
            for control in self.find_all_controls():
                if (
                    ...
                    and control.buffer.name == value
                ):
                    self.focus(control)
                    return
            raise ValueError(
                f"Couldn't find Buffer in the current layout: {value!r}."
            )
    ...
    def get_visible_focusable_windows(self) -> list[Window]:
        ...
        return [
            w for w in self.get_focusable_windows() if w in visible_windows
        ]
