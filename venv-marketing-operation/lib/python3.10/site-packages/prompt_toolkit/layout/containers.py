...
def _window_too_small() -> Window:
    ...
    return Window(
        FormattedTextControl(
            text=[("class:window-too-small", " Window too small... ")]
        )
    )
...
class HSplit(_Split):
    ...
    def write_to_screen(
        ...
        if sizes is None:
            ...
            remaining_height = (
                write_position.ypos + write_position.height - ypos
            )
...
class VSplit(_Split):
    ...
    def write_to_screen(
        ...
        height = max(
            write_position.height, min(write_position.height, max(heights))
        )
...
class FloatContainer(Container):
    ...
    def _draw_float(
        ...
        # Near x position of cursor.
            ...
            if fl_width is None:
                width = fl.content.preferred_width(
        ...
        if height > 0 and width > 0:
            ...
            if not fl.hide_when_covering_content or self._area_is_empty(
                screen, wp
            ):
...
class WindowRenderInfo:
    ...
    def applied_scroll_offsets(self) -> ScrollOffsets:
        ...
        if self.displayed_lines[0] == 0:
            ...
            y = self.input_line_to_visible_line[
                self.ui_content.cursor_position.y
            ]
    ...
    def displayed_lines(self) -> list[int]:
        ...
        """
        return sorted(
            row for row, col in self.visible_line_to_row_col.values()
        )
    ...
    def last_visible_line(self, before_scroll_offset: bool = False) -> int:
        ...
        if before_scroll_offset:
            return self.displayed_lines[
                -1 - self.applied_scroll_offsets.bottom
            ]
        else:
...
class ColorColumn:
    ...
    def __init__(
        self, position: int, style: str = "class:color-column"
    ) -> None:
...
class Window(Container):
    ...
    def _write_to_screen_at_index(
        ...
        def render_margin(m: Margin, width: int) -> UIContent:
            ...
            fragments = m.create_margin(
                render_info, width, write_position.height
            )
        ...
        for m, width in zip(self.left_margins, left_margin_widths):
            if (
                ...
                # Copy and shift X.
                self._copy_margin(
                    margin_content, screen, write_position, move_x, width
                )
                move_x += width
    ...
    def _copy_body(
                ...
                for c in text:
                    ...
                    if wrap_lines and x + char_width > width:
                        ...
                        if is_input and get_line_prefix:
                            ...
                            x, y = copy_line(
                                prompt, lineno, x, y, is_input=False
                            )
                    ...
                    if x >= 0 and y >= 0 and x < width:
                        ...
                        # Merge it in the previous cell.
                            ...
                            for pw in [2, 1]:  # Previous character width.
                                if (
                                    x - pw >= 0
                                    and new_buffer_row[x + xpos - pw].width
                                    == pw
                                ):
        ...
        new_screen.height = max(
            new_screen.height, ypos + write_position.height
        )
        ...
        if self.cursorcolumn():
            for y2 in range(y, y + height):
                ...
                row[cpos.x] = _CHAR_CACHE[
                    original_char.char,
                    original_char.style + cursor_column_style,
                ]
            ...
            if column < x + width:  # Only draw when visible.
                ...
                for y2 in range(y, y + height):
                    ...
                    row[column + x] = _CHAR_CACHE[
                        original_char.char,
                        original_char.style + color_column_style,
                    ]
    ...
    def _copy_margin(
        ...
        margin_write_position = WritePosition(
            xpos, ypos, width, write_position.height
        )
        self._copy_body(
            margin_content, new_screen, margin_write_position, 0, width
        )
    ...
    def _scroll_when_linewrapping(
        ...
        self.vertical_scroll = max(
            self.vertical_scroll,
            min(topmost_visible, get_min_vertical_scroll()),
        )
        self.vertical_scroll = min(
            self.vertical_scroll, get_max_vertical_scroll()
        )
    ...
    def _scroll_without_linewrapping(
        ...
        def do_scroll(
            ...
            if (
                current_scroll
                < (cursor_pos + 1) - window_size + scroll_offset_end
            ):
                current_scroll = (
                    (cursor_pos + 1) - window_size + scroll_offset_end
                )
...
class DynamicContainer(Container):
    ...
    def preferred_height(
        ...
        return self._get_container().preferred_height(
