    ...
    def get_height_for_line(
        ...
        try:
            ...
            if width == 0:
                ...
                line = fragment_list_to_text(self.get_line(lineno))[
                    :slice_stop
                ]
...
class BufferControl(UIControl):
    ...
    def __init__(
        ...
        self._last_get_processed_line: (
            Callable[[int], _ProcessedLine] | None
        ) = None
    ...
    def _create_get_processed_line_func(
        ...
        def transform(
            lineno: int, fragments: StyleAndTextTuples
        ) -> _ProcessedLine:
    ...
    def create_content(
        ...
        def translate_rowcol(row: int, col: int) -> Point:
            ...
            return Point(
                x=get_processed_line(row).source_to_display(col), y=row
            )
        ...
        if get_app().layout.current_control == self:
            menu_position = (
                self.menu_position() if self.menu_position else None
            )
            if menu_position is not None:
                assert isinstance(menu_position, int)
                menu_row, menu_col = (
                    buffer.document.translate_index_to_position(menu_position)
                )
                ...
                menu_row, menu_col = (
                    buffer.document.translate_index_to_position(
                        min(
        ...
        if get_app().layout.current_control == self:
            if self._last_get_processed_line:
                ...
                if mouse_event.event_type == MouseEventType.MOUSE_DOWN:
                    ...
                    if double_click:
                        start, end = (
                            buffer.document.find_boundaries_of_current_word()
                        )
