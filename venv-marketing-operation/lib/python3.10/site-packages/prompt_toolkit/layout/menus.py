...
class MultiColumnCompletionMenuControl(UIControl):
    ...
    def create_content(self, width: int, height: int) -> UIContent:
        ...
        visible_columns = max(
            1, (width - self._required_margin) // column_width
        )
    ...
    def _get_column_width(self, completion_state: CompletionState) -> int:
        ...
        """
        try:
            count, width = self._column_width_for_completion_state[
                completion_state
            ]
            ...
            result = (
                max(
                    get_cwidth(c.display_text)
                    for c in completion_state.completions
                )
                + 1
            )
    ...
    def get_key_bindings(self) -> KeyBindings:
        ...
        def filter() -> bool:
            ...
            return any(
                window.content == self for window in app.layout.visible_windows
            )
...
class _SelectedCompletionMetaControl(UIControl):
    ...
    def preferred_width(self, max_available_width: int) -> int | None:
        ...
        if app.current_buffer.complete_state:
            ...
            return 2 + max(
                get_cwidth(c.display_meta_text)
                for c in state.completions[:100]
            )
        else:
            return 0
