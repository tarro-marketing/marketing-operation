    ...
    def apply_transformation(
        ...
        if search_text and not get_app().is_done:
            ...
            for match in re.finditer(
                re.escape(search_text), line_text, flags=flags
            ):
            ...
            if from_ == 0 and to == 0 and len(fragments) == 0:
                ...
                for i in range(from_, to):
                    if i < len(fragments):
                        ...
                        fragments[i] = (
                            old_fragment + selected_fragment,
                            old_text,
                        )
...
class AppendAutoSuggestion(Processor):
    ...
    def apply_transformation(self, ti: TransformationInput) -> Transformation:
        ...
        if ti.lineno == ti.document.line_count - 1:
            ...
            return Transformation(
                fragments=ti.fragments + [(self.style, suggestion)]
            )
        else:
            return Transformation(fragments=ti.fragments)
    ...
    def apply_transformation(self, ti: TransformationInput) -> Transformation:
        ...
        def display_to_source(display_pos: int) -> int:
            ...
            position_mappings_reversed = {
                v: k for k, v in position_mappings.items()
            }
...
class ReverseSearchProcessor(Processor):
    ...
    def _content(
        ...
        return buffer_control.create_content(
            ti.width, ti.height, preview_search=True
        )
    ...
    def apply_transformation(self, ti: TransformationInput) -> Transformation:
        ...
        if ti.lineno == 0 and main_control:
            ...
            fragments = (
                ...
                + [
                    (
                        "class:prompt.search.text",
                        fragment_list_to_text(ti.fragments),
                    ),
                    ("", "': "),
                ]
                + line_fragments
