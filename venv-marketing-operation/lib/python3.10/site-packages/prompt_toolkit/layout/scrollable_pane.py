...
class ScrollablePane(Container):
    ...
    def preferred_height(
        ...
        dimension = self.content.preferred_height(
            width, self.max_available_height
        )
    ...
    def write_to_screen(
        ...
        try:
            visible_win_write_pos = (
                temp_screen.visible_windows_to_write_positions[focused_window]
            )
        except KeyError:
        ...
        self._copy_over_screen(
            screen, temp_screen, write_position, virtual_width
        )
    ...
    def _clip_point_to_visible_area(
        ...
        if point.y >= write_position.ypos + write_position.height:
            point = point._replace(
                y=write_position.ypos + write_position.height - 1
            )
    ...
    def _copy_over_mouse_handlers(
        ...
        for y in range(write_position.height):
            if y in temp_mouse_handlers_dict:
                temp_mouse_row = temp_mouse_handlers_dict[
                    y + self.vertical_scroll
                ]
                mouse_row = mouse_handlers_dict[y + ypos]
                for x in range(virtual_width):
                    if x in temp_mouse_row:
                        mouse_row[x + xpos] = wrap_mouse_handler(
                            temp_mouse_row[x]
                        )
    ...
    def _copy_over_write_positions(
        ...
        for (
            win,
            write_pos,
        ) in temp_screen.visible_windows_to_write_positions.items():
