...
class FuzzyCompleter(Completer):
    ...
    def _get_fuzzy_completions(
        ...
        document2 = Document(
            text=document.text[
                : document.cursor_position - len(word_before_cursor)
            ],
        ...
        if word_before_cursor == "":
            ...
            # pattern.
            fuzzy_matches = [
                _FuzzyMatch(0, 0, compl) for compl in inner_completions
            ]
        else:
            pat = ".*?".join(map(re.escape, word_before_cursor))
            pat = (
                f"(?=({pat}))"  # lookahead regex to manage overlapping matches
            )
            ...
            for compl in inner_completions:
                ...
                if matches:
                    ...
                    best = min(
                        matches, key=lambda m: (m.start(), len(m.group(1)))
                    )
    ...
    def _get_display(
        ...
        def get_display() -> AnyFormattedText:
            ...
            result.append(
                (
                    "class:fuzzymatch.outside",
                    word[m.start_pos + m.match_length :],
                )
            )
