...
class PygmentsLexer(Lexer):
    ...
    def from_filename(
        ...
        try:
            ...
            return cls(
                pygments_lexer.__class__, sync_from_start=sync_from_start
            )
    ...
    def lex_document(
        ...
        def find_closest_generator(i: int) -> LineGenerator | None:
            ...
            for generator, lineno in line_generators.items():
                if (
                    lineno < i
                    and i - lineno < self.REUSE_GENERATOR_MAX_DISTANCE
                ):
                    return generator
            return None

        def create_line_generator(
            start_lineno: int, column: int = 0
        ) -> LineGenerator:
            """
            ...
            def get_text_fragments() -> Iterable[tuple[str, str]]:
                ...
                # have given as input.)
                for _, t, v in self.pygments_lexer.get_tokens_unprocessed(
                    text
                ):
            ...
            yield from enumerate(
                split_lines(list(get_text_fragments())), start_lineno
            )
        ...
        def get_generator(i: int) -> LineGenerator:
            ...
            if i == 0:
                ...
                row, column = get_syntax_sync().get_sync_start_position(
                    document, i
                )
