...
def _get_closest_ansi_color(
    r: int, g: int, b: int, exclude: Sequence[str] = ()
) -> str:
    """
    ...
    if saturation > 30:
        exclude.extend(
            ["ansilightgray", "ansidarkgray", "ansiwhite", "ansiblack"]
        )
                ...
                if self.color_depth == ColorDepth.DEPTH_4_BIT:
                    if bg:  # Background.
                        if fg_color != bg_color:
                            ...
                            exclude = []
                        code, name = _16_bg_colors.get_code(
                            rgb, exclude=exclude
                        )
                        return [code]
                    else:  # Foreground.
...
class Vt100_Output(Output):
    ...
    def from_pty(
        ...
        if not stdout.isatty() and (
            fd is None or fd not in cls._fds_not_a_terminal
        ):
