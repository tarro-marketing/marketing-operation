...
class Progress(Formatter):
    ...
    template = HTML(
        "<current>{current:>3}</current>/<total>{total:>3}</total>"
    )
...
class TimeElapsed(Formatter):
    ...
    def get_width(self, progress_bar: ProgressBar) -> AnyDimension:
        all_values = [
            len(_format_timedelta(c.time_elapsed))
...
class TimeLeft(Formatter):
    ...
    def get_width(self, progress_bar: ProgressBar) -> AnyDimension:
        all_values = [
            (
                len(_format_timedelta(c.time_left))
...
class IterationsPerSecond(Formatter):
    ...
    def format(
        ...
        value = (
            progress.items_completed / progress.time_elapsed.total_seconds()
        )
