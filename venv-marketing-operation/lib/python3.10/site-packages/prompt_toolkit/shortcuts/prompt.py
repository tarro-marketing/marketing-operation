...
class PromptSession(Generic[_T]):
    ...
    def _create_layout(self) -> Layout:
        ...
        def display_placeholder() -> bool:
            return (
                self.placeholder is not None and self.default_buffer.text == ""
            )
        ...
        layout = HSplit(
            [
                ...
                ConditionalContainer(
                    search_toolbar, dyncond("multiline") & ~is_done
                ),
                bottom_toolbar,
            ]
        )
    ...
    def _create_prompt_bindings(self) -> KeyBindings:
        ...
        def _keyboard_interrupt(event: E) -> None:
            ...
            event.app.exit(
                exception=self.interrupt_exception(), style="class:aborting"
            )
        ...
        def _eof(event: E) -> None:
            ...
            event.app.exit(
                exception=self.eof_exception(), style="class:exiting"
            )
        ...
        *,
        ...
        if self._output is None and is_dumb_terminal():
            with self._dumb_prompt(self.message) as dump_app:
                return dump_app.run(
                    in_thread=in_thread, handle_sigint=handle_sigint
                )
    ...
    @contextmanager
    def _dumb_prompt(
        self, message: AnyFormattedText = ""
    ) -> Iterator[Application[_T]]:
        """
        ...
        self.output.write(
            fragment_list_to_text(to_formatted_text(self.message))
        )
        self.output.flush()
        ...
        def on_text_changed(_: object) -> None:
            self.output.write(
                self.default_buffer.document.text_before_cursor[-1:]
            )
            self.output.flush()
    ...
    def _get_line_prefix(
        ...
        if line_number == 0 and wrap_count == 0:
            if (
                not is_true(self.multiline)
                and get_app().key_processor.arg is not None
            ):
