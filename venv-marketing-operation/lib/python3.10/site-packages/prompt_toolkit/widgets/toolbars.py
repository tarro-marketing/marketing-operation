...
class FormattedTextToolbar(Window):
    def __init__(
        self, text: AnyFormattedText, style: str = "", **kw: Any
    ) -> None:
...
class SystemToolbar:
    ...
    def _build_key_bindings(self) -> KeyBindingsBase:
        ...
        @handle(
            "!", filter=~focused & vi_mode & vi_navigation_mode, is_global=True
        )
        ...
        return merge_key_bindings(
            [
                ...
                ConditionalKeyBindings(
                    global_bindings, self.enable_global_bindings
                ),
            ]
        )
...
class ArgToolbar:
    def __init__(self) -> None:
        ...
        self.window = Window(
            FormattedTextControl(get_formatted_text), height=1
        )

        self.container = ConditionalContainer(
            content=self.window, filter=has_arg
        )
    ...
    def __init__(
        ...
        def get_before_input() -> AnyFormattedText:
            if not is_searching():
                ...
                self.control.searcher_search_state.direction
        ...
        self.control = SearchBufferControl(
            ...
            input_processors=[
                BeforeInput(
                    get_before_input, style="class:search-toolbar.prompt"
                )
            ],
    ...
    def create_content(self, width: int, height: int) -> UIContent:
        ...
        if complete_state:
            ...
            for i, c in enumerate(completions):
                ...
                if (
                    fragment_list_len(fragments) + len(c.display_text)
                    >= content_width
                ):
