...
class _CompiledGrammar:
    ...
    def __init__(
        ...
        self._re_prefix = [
            re.compile(t, flags) for t in self._re_prefix_patterns
        ]
        ...
        self._re_prefix_with_trailing_input = [
            re.compile(
                r"(?:{})(?P<{}>.*?)$".format(
                    t.rstrip("$"), _INVALID_TRAILING_INPUT
                ),
                flags,
            )
    ...
    def _transform(
        ...
        def transform(node: Node) -> str:
            ...
            if isinstance(node, AnyNode):
                return "(?:{})".format(
                    "|".join(transform(c) for c in node.children)
                )
        ...
        def transform(node: Node) -> Iterable[str]:
            ...
            if isinstance(node, AnyNode):
                ...
                if children_without_variable:
                    yield "|".join(
                        r
                        for c in children_without_variable
...
class Match:
    ...
    def variables(self) -> Variables:
        ...
        return Variables(
            [
                (k, self._unescape(k, v), sl)
                for k, v, sl in self._nodes_to_values()
            ]
        )
...
class MatchVariable:
    ...
    def __init__(
        self, varname: str, value: str, slice: tuple[int, int]
    ) -> None:
