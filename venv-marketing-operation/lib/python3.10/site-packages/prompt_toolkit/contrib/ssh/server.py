...
class PromptToolkitSSHSession(asyncssh.SSHServerSession):  # type: ignore
    def __init__(
        self,
        interact: Callable[
            [PromptToolkitSSHSession], Coroutine[Any, Any, None]
        ],
        *,
        enable_cpr: bool,
    ) -> None:
    ...
    async def _interact(self) -> None:
        ...
        if (
            hasattr(self._chan, "set_line_mode")
            and self._chan._editor is not None
        ):
...
class PromptToolkitSSHServer(asyncssh.SSHServer):
    ...
    def session_requested(self) -> PromptToolkitSSHSession:
        return PromptToolkitSSHSession(
            self.interact, enable_cpr=self.enable_cpr
        )
