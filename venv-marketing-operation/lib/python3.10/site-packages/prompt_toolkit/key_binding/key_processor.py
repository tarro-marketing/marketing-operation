...
class KeyProcessor:
    ...
    def _get_matches(self, key_presses: list[KeyPress]) -> list[Binding]:
        ...
        return [
            b for b in self._bindings.get_bindings_for_keys(keys) if b.filter()
        ]
            ...
            if buffer:
                ...
                if flush:
                    ...
                    is_prefix_of_longer_match = (
                        self._is_prefix_of_longer_match(buffer)
                    )
                ...
                # No match found.
                    ...
                    for i in range(len(buffer), 0, -1):
                        ...
                        if matches:
                            self._call_handler(
                                matches[-1], key_sequence=buffer[:i]
                            )
                            del buffer[:i]
                            found = True
                            break
    ...
    def _call_handler(
        self, handler: Binding, key_sequence: list[KeyPress]
    ) -> None:
        app = get_app()
