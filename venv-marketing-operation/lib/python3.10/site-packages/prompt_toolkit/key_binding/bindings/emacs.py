...
def load_emacs_bindings() -> KeyBindingsBase:
    ...
    handle("escape", "backspace", filter=insert_mode)(
        get_by_name("backward-kill-word")
    )
    handle("escape", "\\", filter=insert_mode)(
        get_by_name("delete-horizontal-space")
    )
    ...
    handle("escape", "<", filter=~has_selection)(
        get_by_name("beginning-of-history")
    )
    ...
    def _insert_all_completions(event: E) -> None:
        ...
        complete_event = CompleteEvent(
            text_inserted=False, completion_requested=True
        )
        completions = list(
    ...
    def _toggle_start_end(event: E) -> None:
        ...
        if buffer.document.is_cursor_at_the_end_of_line:
            buffer.cursor_position += (
                buffer.document.get_start_of_line_position(
                    after_whitespace=False
                )
            )
        else:
            buffer.cursor_position += (
                buffer.document.get_end_of_line_position()
            )
