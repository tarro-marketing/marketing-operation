...
class TextObject:
    ...
    def operator_range(self, document: Document) -> tuple[int, int]:
        ...
        if (
            ...
            and doc.translate_index_to_position(end + doc.cursor_position)[1]
            == 0
        ):
        ...
        if self.type == TextObjectType.LINEWISE:
            # Select whole lines
            row, col = doc.translate_index_to_position(
                start + doc.cursor_position
            )
            start = (
                doc.translate_row_col_to_index(row, 0) - doc.cursor_position
            )
            row, col = doc.translate_index_to_position(
                end + doc.cursor_position
            )
            end = (
    ...
    def text_object_decorator(
        *keys: Keys | str,
        ...
        def decorator(text_object_func: _TOF) -> _TOF:
            @key_bindings.add(
                *keys,
                filter=vi_waiting_for_text_object_mode & filter,
                eager=eager,
            )
            def _apply_operator_to_text_object(event: E) -> None:
                ...
                event._arg = str(
                    (vi_state.operator_arg or 1) * (event.arg or 1)
                )
...
def in_block_selection() -> bool:
    ...
    return bool(
        buff.selection_state
        and buff.selection_state.type == SelectionType.BLOCK
    )
...
def load_vi_bindings() -> KeyBindingsBase:
    ...
    def _delete_line(event: E) -> None:
        ...
        after = "\n".join(
            lines[buffer.document.cursor_position_row + event.arg :]
        )
    ...
    def _prev_occurrence(event: E) -> None:
        ...
        b.apply_search(
            search_state, count=event.arg, include_current_position=False
        )
    ...
    def create_delete_and_change_operators(
        ...
        def delete_or_change_operator(
            event: E, text_object: TextObject
        ) -> None:
            clipboard_data = None
            buff = event.current_buffer
            ...
            if clipboard_data and clipboard_data.text:
                if with_register:
                    ...
                    if reg_name in vi_register_names:
                        event.app.vi_state.named_registers[reg_name] = (
                            clipboard_data
                        )
                else:
                    event.app.clipboard.set_data(clipboard_data)
    ...
    def _inner_word(event: E) -> TextObject:
        ...
        """
        start, end = (
            event.current_buffer.document.find_boundaries_of_current_word()
        )
    ...
    def create_ci_ca_handles(
        ...
        def handler(event: E) -> TextObject:
            if ci_start == ci_end:
                ...
                end = event.current_buffer.document.find(
                    ci_end, in_current_line=False
                )
            else:
                # Brackets
                start = (
                    event.current_buffer.document.find_enclosing_bracket_left(
                        ci_start, ci_end
                    )
                )
                end = (
                    event.current_buffer.document.find_enclosing_bracket_right(
                        ci_start, ci_end
                    )
                )
    ...
    def _find_next_occurrence(event: E) -> TextObject:
        ...
        """
        event.app.vi_state.last_character_find = CharacterFind(
            event.data, False
        )
        ...
        if w and w.render_info:
            ...
            pos = (
                b.document.translate_row_col_to_index(
                    w.render_info.first_visible_line(after_scroll_offset=True),
                    0,
                )
                - b.cursor_position
            )
    ...
    def _end_of_screen(event: E) -> TextObject:
        ...
        if w and w.render_info:
            ...
            pos = (
                b.document.translate_row_col_to_index(
                    w.render_info.last_visible_line(before_scroll_offset=True),
                    0,
                )
                - b.cursor_position
            )
    ...
    def _scroll_top(event: E) -> None:
        ...
        """
        b = event.current_buffer
        event.app.layout.current_window.vertical_scroll = (
            b.document.cursor_position_row
        )
    ...
    def _goto_first_line(event: E) -> TextObject:
        ...
        if event._arg:
            ...
            return TextObject(
                d.translate_row_col_to_index(event.arg - 1, 0)
                - d.cursor_position,
                type=TextObjectType.LINEWISE,
            )
        else:
    ...
    def _ge(event: E) -> TextObject:
        ...
        return TextObject(
            prev_end - 1 if prev_end is not None else 0,
            type=TextObjectType.INCLUSIVE,
        )
    ...
    def _gm(event: E) -> TextObject:
        ...
        if w and w.render_info:
            ...
            start = buff.document.get_start_of_line_position(
                after_whitespace=False
            )
    ...
    def _last_line(event: E) -> TextObject:
        ...
        return TextObject(
            buf.document.translate_row_col_to_index(
                buf.document.line_count - 1, 0
            )
            - buf.cursor_position,
            type=TextObjectType.LINEWISE,
        )
    ...
    def _stop_macro(event: E) -> None:
        ...
        if vi_state.recording_register:
            vi_state.named_registers[vi_state.recording_register] = (
                ClipboardData(vi_state.current_recording)
            )
...
def load_vi_search_bindings() -> KeyBindingsBase:
    ...
    handle(
        "/",
        filter=(vi_navigation_mode | vi_selection_mode)
        & ~vi_search_direction_reversed,
