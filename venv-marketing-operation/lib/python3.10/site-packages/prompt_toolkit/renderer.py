...
def _output_screen_diff(
    ...
    def output_char(char: Char) -> None:
        ...
        if last_style == char.style:
            ...
            if (
                not last_style
                or new_attrs != attrs_for_style_string[last_style]
            ):
    ...
    for y in range(row_count):
        ...
        previous_max_line_len = min(
            width - 1, get_max_column_index(previous_row)
        )
        ...
        while c <= new_max_line_len:
            ...
            if (
                new_char.char != old_char.char
                or new_char.style != old_char.style
            ):
                ...
                current_pos = Point(
                    x=current_pos.x + char_width, y=current_pos.y
                )
...
class Renderer:
    ...
    def height_is_known(self) -> bool:
        ...
        try:
            self._min_available_height = (
                self.output.get_rows_below_cursor_position()
            )
            return True
    ...
    def rows_above_layout(self) -> int:
        ...
        if self._in_alternate_screen:
            ...
            last_screen_height = (
                self._last_screen.height if self._last_screen else 0
            )
            return total_rows - max(
                self._min_available_height, last_screen_height
            )
        else:
