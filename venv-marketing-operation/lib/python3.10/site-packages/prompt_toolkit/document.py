...
class Document:
    ...
    def __init__(
        ...
        # insert text.)
        assert cursor_position is None or cursor_position <= len(
            text
        ), AssertionError(
    ...
    def has_match_at_current_position(self, sub: str) -> bool:
        ...
        """
        return (
            self.text.find(sub, self.cursor_position) == self.cursor_position
        )
    ...
    def find_all(self, sub: str, ignore_case: bool = False) -> list[int]:
        ...
        flags = re.IGNORECASE if ignore_case else 0
        return [
            a.start() for a in re.finditer(re.escape(sub), self.text, flags)
        ]
    ...
    def get_word_before_cursor(
        ...
        start = (
            self.find_start_of_previous_word(WORD=WORD, pattern=pattern) or 0
        )
    ...
    def _is_word_before_cursor_complete(
        ...
        if pattern:
            return (
                self.find_start_of_previous_word(WORD=WORD, pattern=pattern)
                is None
            )
        else:
            return (
                self.text_before_cursor == ""
                or self.text_before_cursor[-1:].isspace()
            )
        ...
        def get_regex(include_whitespace: bool) -> Pattern[str]:
            return {
                ...
                (
                    False,
                    True,
                ): _FIND_CURRENT_WORD_INCLUDE_TRAILING_WHITESPACE_RE,
                (True, False): _FIND_CURRENT_BIG_WORD_RE,
                (
                    True,
                    True,
                ): _FIND_CURRENT_BIG_WORD_INCLUDE_TRAILING_WHITESPACE_RE,
            }[(WORD, include_whitespace)]

        match_before = get_regex(include_leading_whitespace).search(
            text_before_cursor
        )
        match_after = get_regex(include_trailing_whitespace).search(
            text_after_cursor
        )
    ...
    def get_word_under_cursor(self, WORD: bool = False) -> str:
        ...
        return self.text[
            self.cursor_position + start : self.cursor_position + end
        ]
    ...
    def find_previous_word_ending(
        ...
        text_before_cursor = (
            self.text_after_cursor[:1] + self.text_before_cursor[::-1]
        )
    ...
    def find_next_matching_line(
        ...
        for index, line in enumerate(
            self.lines[self.cursor_position_row + 1 :]
        ):
            if match_func(line):
                result = 1 + index
                count -= 1
    ...
    def find_matching_bracket_position(
        ...
        for pair in "()", "[]", "{}", "<>":
            ...
            if self.current_char == A:
                return (
                    self.find_enclosing_bracket_right(A, B, end_pos=end_pos)
                    or 0
                )
            elif self.current_char == B:
                return (
                    self.find_enclosing_bracket_left(A, B, start_pos=start_pos)
                    or 0
                )
    ...
    def get_start_of_line_position(
        self, after_whitespace: bool = False
    ) -> int:
    ...
    def selection_ranges(self) -> Iterable[tuple[int, int]]:
        ...
        if self.selection:
            ...
            if self.selection.type == SelectionType.BLOCK:
                from_line, from_column = self.translate_index_to_position(
                    from_
                )
        ...
        if self.selection:
            ...
            if intersection_start <= intersection_end:
                if self.selection.type == SelectionType.LINES:
                    ...
                    intersection_start = self.translate_row_col_to_index(
                        row, col1
                    )
                    intersection_end = self.translate_row_col_to_index(
                        row, col2
                    )

                _, from_column = self.translate_index_to_position(
                    intersection_start
                )
                _, to_column = self.translate_index_to_position(
                    intersection_end
                )
    ...
    def cut_selection(self) -> tuple[Document, ClipboardData]:
        ...
        if self.selection:
            ...
            if (
                self.selection.type == SelectionType.LINES
                and cut_text.endswith("\n")
            ):
                cut_text = cut_text[:-1]

            return (
                Document(
                    text=remaining_text, cursor_position=new_cursor_position
                ),
    ...
    def end_of_paragraph(self, count: int = 1, after: bool = False) -> int:
        ...
        line_index = self.find_next_matching_line(
            match_func=match_func, count=count
        )
        ...
        if line_index:
            ...
            return max(
                0, self.get_cursor_down_position(count=line_index) - add
            )
        else:
