...
if tuple(map(int, pytest.__version__.split(".")[:2])) < (
    4,
    6,
):  # pragma: no cover
    import warnings
    ...
    def pytest_report_header(config):
        ...
        if (
            config.option.verbose < 1
            and settings.default.verbosity < Verbosity.verbose
        ):
            return None
        ...
        return (
            f"hypothesis profile {settings._current_profile!r}{settings_str}"
        )

    def pytest_configure(config):
        config.addinivalue_line(
            "markers", "hypothesis: Tests which use hypothesis."
        )
        ...
        if (
            verbosity_name
            and verbosity_name != settings.default.verbosity.name
        ):
            verbosity_value = Verbosity[verbosity_name]
            name = (
                f"{settings._current_profile}-with-{verbosity_name}-verbosity"
            )
    ...
    def pytest_runtest_makereport(item, call):
        ...
        if hasattr(item, "hypothesis_report_information"):
            report.sections.append(
                ("Hypothesis", item.hypothesis_report_information)
            )
        if report.when != "teardown":
            return

        terminalreporter = item.config.pluginmanager.getplugin(
            "terminalreporter"
        )
        ...
        if failing_examples and terminalreporter is not None:
            ...
            triple = get_patch_for(
                item.obj, [(x, FAIL_MSG) for x in failing_examples]
            )
    ...
    def pytest_terminal_summary(terminalreporter):
        failing_examples = []
        print_stats = terminalreporter.config.getoption(
            PRINT_STATISTICS_OPTION
        )
        if print_stats:
