...
def identify_ftz_culprits():
    ...
    always_enables_ftz, always_imports = run_in_process(
        always_imported_modules
    )
    if always_enables_ftz:
        raise RuntimeError(
            "Python is always in FTZ mode, even without imports!"
        )
    ...
    KNOWN_FTZ = min(
        result
    )  # Cache the result - it's likely this will trigger again!
    return KNOWN_FTZ
