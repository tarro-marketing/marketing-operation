...
def decode_failure(blob):
    try:
        ...
        raise InvalidArgument(
            f"Invalid base64 encoded string: {blob!r}"
        ) from None
    prefix = buffer[:1]
    if prefix == b"\0":
        return buffer[1:]
...
def _invalid(message, *, exc=InvalidArgument, test, given_kwargs):
    ...
    def wrapped_test(
        *arguments, **kwargs
    ):  # pragma: no cover  # coverage limitation
        raise exc(message)
...
def is_invalid_test(test, original_sig, given_arguments, given_kwargs):
    ...
    extra_kwargs = [
        ...
        if k not in {p.name for p in pos_params + kwonly_params}
    ]
    if extra_kwargs and (
        params == [] or params[-1].kind is not params[-1].VAR_KEYWORD
    ):
        arg = extra_kwargs[0]
        return invalid(
    ...
    if empty:
        ...
        return invalid(
            f"Cannot generate examples from empty {strats}: "
            + ", ".join(empty),
            exc=Unsatisfiable,
        )
...
def execute_explicit_examples(
    ...
    for example in reversed(
        ...
        if example.args:
            ...
            example_kwargs = dict(
                zip(posargs[-len(example.args) :], example.args)
            )
        else:
                    ...
                    if example.raises is None:
                        ...
                        try:
                            ...
                            state.xfail_example_reprs.add(
                                repr_call(
                                    state.test, arguments, example_kwargs
                                )
                            )
                            ...
                            if example.raises is BaseException:
                                name = (
                                    "exception"  # special-case no raises= arg
                                )
...
def process_arguments_to_given(
    wrapped_test, arguments, kwargs, given_kwargs, params
):
    selfy = None
    ...
    stuff = Stuff(
        selfy=selfy, args=arguments, kwargs=kwargs, given_kwargs=given_kwargs
    )
...
def get_executor(runner):
    ...
    if hasattr(runner, "setup_example") or hasattr(runner, "teardown_example"):
        ...
        teardown = getattr(runner, "teardown_example", None) or (
            lambda ex: None
        )
...
class StateForActualGivenExecution:
    ...
    def run_engine(self):
        ...
        if TESTCASE_CALLBACKS:
            ...
            for msg in (
                p
                if isinstance(p := runner.provider, PrimitiveProvider)
                else p(None)
        ...
        if not self.falsifying_examples:
            ...
            self.settings.report_multiple_bugs and pytest_shows_exceptiongroups
        ):
        ...
        for falsifying_example in self.falsifying_examples:
            ...
            try:
                ...
                err = FlakyFailure(
                    ...
                    [
                        info._expected_exception or e
                    ],  # (note: e is a BaseException)
                )
                errors_to_report.append((fragments, err))
            except (
                UnsatisfiedAssumption
            ) as e:  # pragma: no cover  # ironically flaky
                err = FlakyFailure(
                ...
                fragments.extend(
                    explanations[falsifying_example.interesting_origin]
                )
...
def given(
    ...
    **_given_kwargs: Union[SearchStrategy[Any], EllipsisType],
    ...
    def run_test_as_given(test):
        ...
        for name in [
            name for name, value in given_kwargs.items() if value is ...
        ]:
        ...
        def wrapped_test(*arguments, **kwargs):
            ...
            runner = stuff.selfy
            if isinstance(stuff.selfy, TestCase) and test.__name__ in dir(
                TestCase
            ):
                msg = (
            ...
            if settings.database is not None:
                ...
                cur_self = (
                    stuff.selfy
                    if getattr(type(stuff.selfy), test.__name__, None)
                    is wrapped_test
                    else None
                )
                if prev_self is Unset:
                    ...
                    fail_health_check(
                        settings, msg, HealthCheck.differing_executors
                    )
        ...
        def _get_fuzz_target() -> Callable[
            [Union[bytes, bytearray, memoryview, BinaryIO]],
            Optional[bytes],
        ]:
        ...
        wrapped_test.hypothesis = HypothesisHandle(
            test, _get_fuzz_target, given_kwargs
        )
        return wrapped_test
