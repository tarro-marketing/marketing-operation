...
def run_state_machine_as_test(
    state_machine_factory, *, settings=None, _min_steps=0
):
    ...
    def run_state_machine(factory, data):
        ...
        machine._observability_predicates = (
            cd._observability_predicates
        )  # alias
        ...
        try:
            ...
            while True:
                ...
                cd.draw_times[draw_label] += (
                    perf_counter() - start_draw - in_gctime
                )
                ...
                machine.check_invariants(
                    settings, output, cd._stateful_run_times
                )
                cd.stop_example()
        finally:
        ...
        if rule.targets:
            if isinstance(result, MultipleResults):
                if len(result.values) == 1:
                    ...
                    number_of_last_names = len(rule.targets) * len(
                        result.values
                    )
...
class BundleReferenceStrategy(SearchStrategy):
    ...
    def do_draw(self, data):
        ...
        position = data.draw_integer(
            0, len(bundle) - 1, shrink_towards=len(bundle)
        )
        if self.consume:
            return bundle.pop(
                position
...
def invariant(
    *, check_during_init: bool = False
    ...
    def accept(f):
        if getattr(f, RULE_MARKER, None) or getattr(
            f, INITIALIZE_RULE_MARKER, None
        ):
            raise InvalidDefinition(
                "A function cannot be used for both a rule and an invariant.",
                Settings.default,
...
class RuleStrategy(SearchStrategy):
    def __init__(self, machine):
        ...
        self.enabled_rules_strategy = st.shared(
            FeatureStrategy(
                at_least_one_of={r.function.__name__ for r in self.rules}
            ),
    ...
    def do_draw(self, data):
        ...
        def rule_is_enabled(r):
            ...
            return self.is_valid(r) and feature_flags.is_enabled(
                r.function.__name__
            )
        ...
        for k, strat in rule.arguments_strategies.items():
            try:
                ...
                add_note(
                    err,
                    f"while generating {k!r} from {strat!r} for rule {rname}",
                )
                raise
    ...
    def is_valid(self, rule):
        ...
        for pred in rule.preconditions:
            ...
            where = (
                f"{desc} precondition {get_pretty_function_description(pred)}"
            )
            predicates[where][
                "satisfied" if meets_precond else "unsatisfied"
            ] += 1
