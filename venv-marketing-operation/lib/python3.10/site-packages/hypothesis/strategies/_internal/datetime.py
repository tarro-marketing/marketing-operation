...
def datetime_does_not_exist(value):
    ...
    assert (
        value.tzinfo is roundtrip.tzinfo
    ), "so only the naive portions are compared"
...
class DatetimeStrategy(SearchStrategy):
    ...
    def do_draw(self, data):
        ...
        if (not self.allow_imaginary) and datetime_does_not_exist(result):
            data.mark_invalid(
                f"{result} does not exist (usually a DST transition)"
            )
        return result
...
class TimeStrategy(SearchStrategy):
    ...
    def do_draw(self, data):
        result = draw_capped_multipart(
            data, self.min_value, self.max_value, TIMENAMES
        )
        tz = data.draw(self.tz_strat)
    ...
    def do_draw(self, data):
        return dt.date(
            **draw_capped_multipart(
                data, self.min_value, self.max_value, DATENAMES
            )
        )
...
def _valid_key_cacheable(tzpath, key):
    assert isinstance(
        tzpath, tuple
    )  # zoneinfo changed, better update this function!
    for root in tzpath:
...
def timezones(
    *, no_cache: bool = False
) -> SearchStrategy["zoneinfo.ZoneInfo"]:
