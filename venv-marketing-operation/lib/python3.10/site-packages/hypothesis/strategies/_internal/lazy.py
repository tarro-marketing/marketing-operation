...
unwrap_cache: MutableMapping[SearchStrategy, SearchStrategy] = (
    WeakKeyDictionary()
)
unwrap_depth = 0
...
class LazyStrategy(SearchStrategy):
    ...
    def __init__(
        self, function, args, kwargs, *, transforms=(), force_repr=None
    ):
        super().__init__()
    ...
    def wrapped_strategy(self):
        if self.__wrapped_strategy is None:
            ...
            if (
                unwrapped_args == self.__args
                and unwrapped_kwargs == self.__kwargs
            ):
            ...
            for method, fn in self._transformations:
                self.__wrapped_strategy = getattr(
                    self.__wrapped_strategy, method
                )(fn)
    ...
    def do_validate(self):
        ...
        assert isinstance(
            w, SearchStrategy
        ), f"{self!r} returned non-strategy {w!r}"
        w.validate()
