...
class IntegersStrategy(SearchStrategy):
    ...
    def filter(self, condition):
        ...
        if "min_value" in kwargs:
            start = max(
                kwargs["min_value"], -math.inf if start is None else start
            )
...
class FloatStrategy(SearchStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        ), "programmer error if this is negative"
    ...
    def filter(self, condition):
        ...
        if (
            ...
            or (
                self.allow_nan
                and (-math.inf < min_bound or max_bound < math.inf)
            )
        ):
            self = type(self)(
                min_value=min_bound,
...
def floats(
    ...
    *,
    ...
    if allow_nan is None:
        ...
        raise InvalidArgument(
            f"Cannot have {allow_nan=}, with min_value or max_value"
        )
    ...
    if allow_subnormal is None:
        if min_value is not None and max_value is not None:
            if min_value == max_value:
                allow_subnormal = (
                    -smallest_normal < min_value < smallest_normal
                )
            else:
                allow_subnormal = (
                    min_value < smallest_normal
