        ...
        for _ in range(3):
            ...
            if i not in known_bad_indices:
                ...
                if not known_bad_indices:
                    data.events[
                        f"Retried draw from {self!r} to satisfy filter"
                    ] = ""
...
class OneOfStrategy(SearchStrategy[Ex]):
    ...
    def filter(self, condition):
        return FilteredStrategy(
            OneOfStrategy(
                [s.filter(condition) for s in self.original_strategies]
            ),
            conditions=(),
        )
        ...
        with warnings.catch_warnings():
            ...
            for _ in range(3):
                try:
                    ...
                    data.stop_example()
                    _current_build_context.value.record_call(
                        result, self.pack, [x], {}
                    )
                    return result
...
class FilteredStrategy(SearchStrategy[Ex]):
    ...
    def branches(self) -> List[SearchStrategy[Ex]]:
        return [
            FilteredStrategy(
                strategy=strategy, conditions=self.flat_conditions
            )
