...
from hypothesis.strategies._internal.misc import (
    BooleansStrategy,
    just,
    none,
    nothing,
...
def sampled_from(
    elements: Sequence[T],
) -> SearchStrategy[T]:  # pragma: no cover
    ...


@overload
def sampled_from(
    elements: Type[enum.Enum],
    ...
    *,
    ...
    if unique_by is not None:
        ...
        for i, f in enumerate(unique_by):
            if not callable(f):
                raise InvalidArgument(
                    f"unique_by[{i}]={f!r} is not a callable"
                )
...
def characters(
    *,
    ...
    check_valid_interval(
        min_codepoint, max_codepoint, "min_codepoint", "max_codepoint"
    )
    ...
    has_old_arg = any(
        v is not None for k, v in locals().items() if "list" in k
    )
    has_new_arg = any(
        v is not None for k, v in locals().items() if "lude" in k
    )
    ...
    if codec is not None:
        try:
            ...
            raise InvalidArgument(
                f"{codec=} is not valid on this system"
            ) from None
        except Exception:
...
@defines_strategy(force_reusable_values=True)
def text(
    alphabet: Union[Collection[str], SearchStrategy[str]] = characters(
        codec="utf-8"
    ),
    *,
    min_size: int = 0,
    ...
    if isinstance(alphabet, SearchStrategy):
        ...
        if isinstance(char_strategy, SampledFromStrategy):
            ...
            return text(
                char_strategy.elements, min_size=min_size, max_size=max_size
            )
...
def _from_type(thing: Type[Ex]) -> SearchStrategy[Ex]:
    ...
    def as_strategy(strat_or_callable, thing):
        ...
        if not isinstance(strat_or_callable, SearchStrategy):
            assert callable(
                strat_or_callable
            )  # Validated in register_type_strategy
        ...
        if strategy.is_empty:
            raise ResolutionFailed(
                f"Error: {thing!r} resolved to an empty strategy"
            )
        return strategy
    ...
    strategies = [
        ...
        for s in (
            ...
            and sum(
                types.try_issubclass(k, typ)
                for typ in types._global_type_lookup
            )
            == 1
        )
    ...
    if not isabstract(thing):
        ...
        if required and not (
            ...
            or attr.has(thing)
            or is_typed_named_tuple(
                thing
            )  # weird enough that we have a specific check
        ):
            raise ResolutionFailed(
    ...
    *,
    ...
    if places is not None:
        ...
        def fraction_to_decimal(val):
            precision = (
                ceil(
                    math.log10(abs(val.numerator) or 1)
                    + math.log10(val.denominator)
                )
                + 1
            )
...
class DrawFn(Protocol):
    ...
    # parameter.
    def __call__(
        self, strategy: SearchStrategy[Ex], label: object = None
    ) -> Ex:
...
def _composite(f):
    ...
    if not (
        f is typing._overload_dummy or is_first_param_referenced_in_function(f)
    ):
        note_deprecation(
    ...
    if get_origin(sig.return_annotation) is SearchStrategy:
        ret_repr = repr(sig.return_annotation).replace(
...
def complex_numbers(
    *,
    ...
    check_valid_interval(
        min_magnitude, max_magnitude, "min_magnitude", "max_magnitude"
    )
    ...
    if allow_infinity is None:
        ...
        raise InvalidArgument(
            f"Cannot have {allow_infinity=} with {max_magnitude=}"
        )
    ...
    def constrained_complex(draw):
        ...
        if min_magnitude == 0 or math.fabs(zi) >= min_magnitude:
            zr = draw(
                floats(None if rmax is None else -rmax, rmax, **allow_kw)
            )
        else:
...
def uuids(
    *,
    version: Optional[Literal[1, 2, 3, 4, 5]] = None,
    ...
    random_uuids = shared(
        ...
        key="hypothesis.strategies.uuids.generator",
...
class DataObject:
    ...
    def draw(self, strategy: SearchStrategy[Ex], label: Any = None) -> Ex:
        ...
        if TESTCASE_CALLBACKS:
            self.conjecture_data._observability_args[desc] = to_jsonable(
                result
            )
...
def register_type_strategy(
    custom_type: Type[Ex],
    strategy: Union[
        SearchStrategy[Ex], Callable[[Type[Ex]], SearchStrategy[Ex]]
    ],
) -> None:
    ...
    if (
        ...
        and issubclass(
            custom_type, sys.modules["pydantic.generics"].GenericModel
        )
...
@defines_strategy(never_lazy=True)
def deferred(
    definition: Callable[[], SearchStrategy[Ex]]
) -> SearchStrategy[Ex]:
