...
def try_issubclass(thing, superclass):
    try:
        ...
        if issubclass(
            get_origin(thing) or thing, get_origin(superclass) or superclass
        ):
            ...
            if getattr(thing, "__args__", None) is not None:
                return (
                    True  # pragma: no cover  # only possible on Python <= 3.9
                )
...
def is_a_new_type(thing):
    if not isinstance(typing.NewType, type):
        ...
        return (  # pragma: no cover  # Python <= 3.9 only
            ...
            and getattr(thing, "__module__", None)
...
def _get_constraints(args: Tuple[Any, ...]) -> Iterator["at.BaseMetadata"]:
    ...
    for arg in args:
        if at and isinstance(arg, at.BaseMetadata):
            ...
            for subarg in arg:
                if getattr(
                    subarg, "__is_annotated_types_grouped_metadata__", False
                ):
...
def find_annotated_strategy(annotated_type):
    ...
    if any(is_annotated_type(arg) for arg in metadata):
        ...
        bits = ", ".join(
            [" | ".join(dict.fromkeys(ts or "?")), *dict.fromkeys(ms)]
        )
...
def is_generic_type(type_):
    ...
    return isinstance(type_, (*typing_root_type, GenericAlias)) or (
        ...
        and (
            typing.Generic in type_.__mro__
            or hasattr(type_, "__class_getitem__")
        )
    )
...
def from_typing_type(thing):
    ...
    if {collections.deque, typing.Deque}.intersection(mapping) and len(
        mapping
    ) > 1:
    ...
    strategies = [
        ...
        for s in (
            ...
            for k, v in sorted(
                mapping.items(), key=lambda kv: type_sorting_key(kv[0])
            )
...
def _networks(bits):
    return st.tuples(
        st.integers(0, 2**bits - 1), st.integers(-bits, 0).map(abs)
    )
...
_global_type_lookup: typing.Dict[
    ...
    | st.builds(
        datetime.timezone, offset=utc_offsets, name=st.text(st.characters())
    ),
    uuid.UUID: st.uuids(),
    ...
    range: st.one_of(
        ...
        st.builds(
            range, st.integers(), st.integers(), st.integers().filter(bool)
        ),
    ),
    ...
    re.Match: st.text()
    .map(lambda c: re.match(".", c, flags=re.DOTALL))
    .filter(bool),
...
_global_type_lookup.update(
    {
        ...
        typing.SupportsInt: st.one_of(
            ...
            st.from_regex(r"\A-?\d+\Z").filter(
                functools.partial(can_cast, int)
            ),
        ),
...
@register(
    "DefaultDict", st.builds(collections.defaultdict), module=typing_extensions
)
...
def resolve_KeysView(thing):
    return st.dictionaries(
        _from_hashable_type(thing.__args__[0]), st.none()
    ).map(dict.keys)
...
class GeneratorStrategy(st.SearchStrategy):
    ...
    def do_draw(self, data):
        elements = conjecture_utils_many(
            data, min_size=0, max_size=100, average_size=5
        )
        while elements.more():
