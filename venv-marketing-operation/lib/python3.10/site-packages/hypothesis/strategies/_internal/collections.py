...
) -> SearchStrategy[Tuple[Ex]]:  # pragma: no cover
    ...
...
class ListStrategy(SearchStrategy):
    ...
    def filter(self, condition):
        if condition in self._nonempty_filters or is_identity_function(
            condition
        ):
            assert (
                self.max_size >= 1
            ), "Always-empty is special cased in st.lists()"
            if self.min_size >= 1:
                return self
            new = copy.copy(self)
            new.min_size = 1
        ...
        if kwargs.get("len") and (
            "min_value" in kwargs or "max_value" in kwargs
        ):
            new = copy.copy(self)
            new.min_size = max(
                self.min_size, kwargs.get("min_value", self.min_size)
            )
            new.max_size = min(
                self.max_size, kwargs.get("max_value", self.max_size)
            )
    ...
    def do_draw(self, data):
        ...
        while elements.more():
            ...
            if value is filter_not_satisfied:
                elements.reject(
                    f"Aborted test because unable to satisfy {filtered!r}"
                )
            else:
...
class FixedKeysDictStrategy(MappedStrategy):
    ...
    def __repr__(self):
        return (
            f"FixedKeysDictStrategy({self.keys!r}, {self.mapped_strategy!r})"
        )
