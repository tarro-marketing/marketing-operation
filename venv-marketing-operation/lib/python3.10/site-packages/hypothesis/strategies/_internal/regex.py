...
class CharactersBuilder:
    ...
    def strategy(self):
        ...
        return OneCharStringStrategy(intervals & self._alphabet.intervals) | (
            st.sampled_from(sorted(multi_chars))
            if multi_chars
            else st.nothing()
        )
    ...
    def add_category(self, category):
        ...
        if category == sre.CATEGORY_DIGIT:
            ...
            raise NotImplementedError(
                f"Unknown character category: {category}"
            )
    ...
    def add_char(self, c):
        ...
        if (
            self._ignorecase
            and re.match(re.escape(c), c.swapcase(), flags=re.IGNORECASE)
            is not None
        ):
...
def regex_strategy(
    ...
    if fullmatch:
        ...
        return base_regex_strategy(regex, parsed, alphabet).filter(
            regex.fullmatch
        )
    ...
    if parsed[0][0] == sre.AT:
        if parsed[0][1] == sre.AT_BEGINNING_STRING:
            ...
            if regex.flags & re.MULTILINE:
                left_pad = st.one_of(
                    empty, st.builds(operator.add, left_pad, newline)
                )
            else:
                left_pad = empty
...
def _strategy(codes, context, is_unicode, *, alphabet):
    ...
    if not isinstance(codes, tuple):
        ...
        while i < len(codes):
            if (
                codes[i][0] == sre.LITERAL
                and not context.flags & re.IGNORECASE
            ):
        ...
        if code == sre.LITERAL:
            ...
            if (
                ...
                and c != c.swapcase()
                and re.match(re.escape(c), c.swapcase(), re.IGNORECASE)
                is not None
            ...
            for charset_code, charset_value in value:
                if charset_code == sre.NEGATE:
                    ...
                    chars = empty.join(
                        map(builder.code_to_char, range(low, high + 1))
                    )
                    if len(chars) == len(
                    ...
                    raise NotImplementedError(
                        f"Unknown charset code: {charset_code}"
                    )
            ...
            strat = _strategy(
                value[-1], context, is_unicode, alphabet=alphabet
            )
            ...
            return st.lists(
                recurse(subregex), min_size=at_least, max_size=at_most
            ).map(empty.join)
