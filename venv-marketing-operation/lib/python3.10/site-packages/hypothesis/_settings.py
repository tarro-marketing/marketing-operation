...
class settings(metaclass=settingsMeta):
        ...
        if defaults is not None:
            for setting in all_settings.values():
                ...
                if value is not_set:
                    ...
                    object.__setattr__(
                        self, setting.name, setting.validator(value)
                    )
    ...
    def __repr__(self):
        ...
        bits = sorted(
            ...
            if (
                name != "backend" or len(AVAILABLE_PROVIDERS) > 1
            )  # experimental
        )
...
class Phase(IntEnum):
    ...
    explain = (
        5  #: controls whether Hypothesis attempts to explain test failures.
    )
...
def _validate_stateful_step_count(x):
    ...
    if x < 1:
        raise InvalidArgument(
            f"stateful_step_count={x!r} must be at least one."
        )
    return x
...
def _backend_validator(value):
    ...
    if value not in AVAILABLE_PROVIDERS:
        if value == "crosshair":  # pragma: no cover
            ...
            raise InvalidArgument(
                f"backend={value!r} is not available.  {install}"
            )
        raise InvalidArgument(
...
def note_deprecation(
    ...
    warnings.warn(
        HypothesisDeprecationWarning(message), stacklevel=2 + stacklevel
    )
