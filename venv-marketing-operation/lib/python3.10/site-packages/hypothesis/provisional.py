...
try:  # pragma: no cover
    traversable = (
        resources.files("hypothesis.vendor") / "tlds-alpha-by-domain.txt"
    )
    _comment, *_tlds = traversable.read_text(encoding="utf-8").splitlines()
except (
    AttributeError,
    ValueError,
):  # pragma: no cover  # .files() was added in 3.9
...
TOP_LEVEL_DOMAINS = [
    "COM",
    *sorted((d for d in _tlds if d != "ARPA"), key=len),
]
...
def urls() -> st.SearchStrategy[str]:
    ...
    def url_encode(s):
        return "".join(
            c if c in URL_SAFE_CHARACTERS else "%%%02X" % ord(c) for c in s
        )
