...
def comp_to_kwargs(
    ...
    b = convert(y, argname)
    num = (int, float)
    if not (a is ARG and isinstance(b, num)) and not (
        isinstance(a, num) and b is ARG
    ):
    ...
    of_len = (
        {"len": True}
        if isinstance(x, ast.Call) or isinstance(y, ast.Call)
        else {}
    )
...
def get_numeric_predicate_bounds(
    predicate: Predicate,
) -> ConstructivePredicate:
    ...
    if isinstance(tree, ast.Lambda) and len(tree.args.args) == 1:
        return numeric_bounds_from_ast(
            tree.body, tree.args.args[0].arg, unchanged
        )
...
def get_integer_predicate_bounds(
    predicate: Predicate,
) -> ConstructivePredicate:
    ...
    if "min_value" in kwargs:
        if kwargs["min_value"] == -math.inf:
            ...
            return ConstructivePredicate(
                {"min_value": 1, "max_value": -1}, None
            )
...
def get_float_predicate_bounds(predicate: Predicate) -> ConstructivePredicate:
    ...
    if "min_value" in kwargs:
        ...
        if min_value < kwargs["min_value"] or (
            min_value == kwargs["min_value"]
            and kwargs.get("exclude_min", False)
        ):
    ...
    if "max_value" in kwargs:
        ...
        if max_value > kwargs["max_value"] or (
            max_value == kwargs["max_value"]
            and kwargs.get("exclude_max", False)
        ):
