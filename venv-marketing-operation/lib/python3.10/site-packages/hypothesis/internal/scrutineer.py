    ...
    def __enter__(self):
        ...
        for event, callback_name in MONITORING_EVENTS.items():
            ...
            sys.monitoring.register_callback(
                MONITORING_TOOL_ID, event, callback
            )
...
def get_explaining_locations(traces):
    ...
    unions = {
        origin: set().union(*values) for origin, values in traces.items()
    }
    ...
    for origin, seen_arcs in unions.items():
        ...
        assert cf_graphs[origin][
            None
        ], "Expected start node with >=1 successor"
...
def make_report(explanations, cap_lines_at=5):
    ...
    for origin, locations in explanations.items():
        report_lines = [
            f"        {fname}:{lineno}" for fname, lineno in locations
        ]
        ...
        if len(report_lines) > cap_lines_at + 1:
            ...
            report_lines[cap_lines_at:] = [
                msg.format(len(report_lines[cap_lines_at:]))
            ]
        if (
            report_lines
        ):  # We might have filtered out every location as uninformative.
...
def tractable_coverage_report(trace: Trace) -> Dict[str, List[int]]:
    ...
    stdlib_fragment = (
        f"{os.sep}lib{os.sep}python3.{sys.version_info.minor}{os.sep}"
    )
    return {
        k: sorted(v)
