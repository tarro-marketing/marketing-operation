...
def _clean_source(src: str) -> bytes:
    ...
    return "\n".join(
        x.rstrip() for x in src.splitlines() if x.rstrip()
    ).encode()
...
def get_signature(
    ...
    if isinstance(patches, list) and all(
        isinstance(p, PatchType) for p in patches
    ):
        P = inspect.Parameter
    ...
    if isinstance(getattr(target, "__signature__", None), inspect.Signature):
        ...
        if sig.parameters and (
            inspect.isclass(target) or inspect.ismethod(target)
        ):
    ...
    for p in sig.parameters.values():
        if p.name in new_kwargs:
            if p.kind is p.POSITIONAL_ONLY:
                ...
                assert set(new_kwargs[p.name]).isdisjoint(
                    set(new_kwargs) - {p.name}
                )
...
def _extract_lambda_source(f):
    ...
    source = source.strip()
    if (
        "lambda" not in source and sys.platform == "emscripten"
    ):  # pragma: no cover
...
def get_pretty_function_description(f):
    ...
    if name == "<lambda>":
        ...
        if not (
            self is None or inspect.isclass(self) or inspect.ismodule(self)
        ):
...
def define_function_signature(name, docstring, signature):
    ...
    def accept(f):
        ...
        candidate_names = ["f"] + [
            f"f_{i}" for i in range(1, len(used_names) + 2)
        ]
...
def is_identity_function(f):
    ...
    return bool(
        re.fullmatch(r"lambda (\w+): \1", get_pretty_function_description(f))
    )
