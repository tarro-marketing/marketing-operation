...
class DFA:
    ...
    def canonicalise(self):
        ...
        transitions = [
            {c: state_map[s] for c, s in self.transitions(t)}
            for t in reverse_state_map
        ]
    ...
    def equivalent(self, other):
        ...
        while queue:
            ...
            # not be equivalent.
            if self.is_accepting(self_state) != other.is_accepting(
                other_state
            ):
                return False
            ...
            for c in alphabet:
                queue.append(
                    (
                        self.transition(self_state, c),
                        other.transition(other_state, c),
                    )
                )
        return True
        ...
        for i in range(len(self.__transitions)):
            ...
            for c, j in self.transitions(i):
                if not table or j != table[-1][-1] or c != table[-1][1] + 1:
                    ...
                    table[-1][1] = c
            transitions.append(
                [(u, j) if u == v else (u, v, j) for u, v, j in table]
            )
