...
class HealthCheckState:
    ...
    def timing_report(self) -> str:
        ...
        if not self.draw_times:
            return ""
        width = max(
            len(k[len("generate:") :].strip(": ")) for k in self.draw_times
        )
        out = [
            f"\n  {'':^{width}}   count | fraction |    slowest draws (seconds)"
        ]
        args_in_order = sorted(
            self.draw_times.items(), key=lambda kv: -sum(kv[1])
        )
        for i, (argname, times) in enumerate(
            args_in_order
        ):  # pragma: no branch
            ...
            arg = argname[len("generate:") :].strip(
                ": "
            )  # removeprefix in py3.9
            out.append(
...
class ConjectureRunner:
    def __init__(
        ...
        *,
        ...
        self.interesting_examples: Dict[
            InterestingOrigin, ConjectureResult
        ] = {}
        ...
        if (
            ...
            and self.settings.database is not None
        ):
    ...
    def debug_data(
        ...
        def go(ex: Example) -> None:
            ...
            if len(ex.children) == 0:
                stack[-1].append(
                    int_from_bytes(data.buffer[ex.start : ex.end])
                )
            else:
                node: Ls = []
                stack.append(node)
        ...
        self.debug(
            f"{data.index} bytes {stack[0]!r} -> {status}, {data.output}"
        )
    ...
    def has_existing_examples(self) -> bool:
        return (
            self.database is not None and Phase.reuse in self.settings.phases
        )
    ...
    def reuse_existing_examples(self) -> None:
        ...
        if self.has_existing_examples():
            ...
            if len(corpus) < desired_size and not self.interesting_examples:
                ...
                pareto_corpus = list(
                    self.settings.database.fetch(self.pareto_key)
                )
                if len(pareto_corpus) > desired_extra:
                    pareto_corpus = self.random.sample(
                        pareto_corpus, desired_extra
                    )
                pareto_corpus.sort(key=sort_key)

                for existing in pareto_corpus:
                    data = self.cached_test_function(
                        existing, extend=BUFFER_SIZE
                    )
                    if data not in self.pareto_front:
                        self.settings.database.delete(
                            self.pareto_key, existing
                        )
                    if data.status == Status.INTERESTING:
                        break
    ...
    def should_generate_more(self) -> bool:
        ...
        assert (
            self.first_bug_found_at
            <= self.last_bug_found_at
            <= self.call_count
        )
    ...
    def generate_new_examples(self) -> None:
        ...
        optimise_at = max(
            self.settings.max_examples // 2, small_example_cap + 1
        )
        ...
        while self.should_generate_more():
            ...
            if self.settings.backend != "hypothesis":
                data = self.new_conjecture_data(
                    prefix=b"", max_length=BUFFER_SIZE
                )
                self.test_function(data)
                continue
    ...
    def generate_mutations_from(
        ...
        if (
            ...
            while (
                ...
                (start1, end1), (start2, end2) = self.random.sample(
                    sorted(group), 2
                )
    ...
    def new_conjecture_data_ir(
        ...
        *,
        ...
        provider = (
            HypothesisProvider
            if self._switch_to_hypothesis_provider
            else self.provider
        )
    ...
    def shrink_interesting_examples(self) -> None:
        ...
        self.finish_shrinking_deadline = (
            time.perf_counter() + MAX_SHRINKING_SECONDS
        )
        ...
        for prev_data in sorted(
            ...
            data = self.new_conjecture_data_ir(
                prev_data.examples.ir_tree_nodes
            )
            self.test_function(data)
            if data.status != Status.INTERESTING:
        ...
        *,
        ...
        def check_result(
            ...
            assert result is Overrun or (
                isinstance(result, ConjectureResult)
        ...
        data = self.new_conjecture_data(
            prefix=max((buffer, dummy_data.buffer), key=len),
            max_length=max_length,
        )
        self.test_function(data)
    ...
    def passing_buffers(self, prefix: bytes = b"") -> FrozenSet[bytes]:
        ...
        return frozenset(
            ...
            and self.__data_cache[buf].status == Status.VALID
        )
