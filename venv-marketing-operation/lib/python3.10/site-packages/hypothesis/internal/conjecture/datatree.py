    ...
    def _repr_pretty_(self, p, cycle):
        ...
        for i, (value, child) in enumerate(self.children.items()):
            if i > 0:
                p.break_()
            p.text(
                _node_pretty(self.ir_type, value, self.kwargs, forced=False)
            )
            with p.indent(2):
                p.break_()
                p.pretty(child)
...
class Conclusion:
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        origin = (
            ""
            if o is None
            else f", {o.exc_type.__name__} at {o.filename}:{o.lineno}"
        )
...
def compute_max_children(ir_type, kwargs):
    if ir_type == "integer":
        ...
        if (
            len(intervals) == 1
            and max_size > MAX_CHILDREN_EFFECTIVELY_INFINITE
        ):
            return MAX_CHILDREN_EFFECTIVELY_INFINITE
        ...
        if flt.is_negative(min_value):
            if flt.is_negative(max_value):
                ...
                yield from floats_between(
                    -smallest_nonzero_magnitude, min_value
                )
                yield from floats_between(
                    smallest_nonzero_magnitude, max_value
                )
        else:
            # case: both positive.
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        for i, (ir_type, kwargs, value) in enumerate(
            ...
            with p.indent(indent):
                ...
                p.text(
                    _node_pretty(
                        ir_type, value, kwargs, forced=i in self.forced
                    )
                )
            indent += 2
...
class DataTree:
        ...
        while True:
            ...
            for i, (ir_type, kwargs, value) in enumerate(
                ...
                assert not isinstance(
                    current_node.transition, (Conclusion, Killed)
                )
                ...
                while True:
                    ...
                    assert (  # pragma: no cover
                        ...
                        or any(
                            not v.is_exhausted
                            for v in branch.children.values()
                        )
                    )
    ...
    def _draw(self, ir_type, kwargs, *, random, forced=None):
        ...
        (value, buf) = ir_to_buffer(
            ir_type, kwargs, forced=forced, random=random
        )
        ...
        *,
        ...
        if i < len(node.values):
            ...
            if trans is None:
                ...
                # children.
                if (
                    compute_max_children(ir_type, kwargs) == 1
                    and not was_forced
                ):
                    node.split_at(i)
                ...
                try:
                    ...
                    self.__current_node = trans.children.setdefault(
                        value, TreeNode()
                    )
                self.__index_in_current_node = 0
