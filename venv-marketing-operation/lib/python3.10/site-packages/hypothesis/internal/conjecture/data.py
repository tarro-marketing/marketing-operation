...
IRTypeName: TypeAlias = Literal[
    "integer", "string", "boolean", "float", "bytes"
]
# index, ir_type, kwargs, forced
MisalignedAt: TypeAlias = Tuple[
    int, IRTypeName, IRKWargsType, Optional[IRType]
]
...
def structural_coverage(label: int) -> StructuralCoverageTag:
    try:
        ...
        return STRUCTURAL_COVERAGE_CACHE.setdefault(
            label, StructuralCoverageTag(label)
        )
...
NASTY_FLOATS = sorted(
    ...
    ]  # minimum (sub)normals for float16,32
    + [
        float_info.min / n for n in (2, 10, 1000, 100_000)
    ]  # subnormal in float64
    + [math.inf, math.nan] * 5
    + [SIGNALING_NAN],
    key=float_to_lex,
)
...
class Examples:
    ...
    ir_starts_and_ends: "Tuple[IntList, IntList]" = (
        calculated_example_property(_ir_starts_and_ends)
    )
...
class Blocks:
    ...
    def __check_completion(self):
        ...
        """
        if self.__count == len(self) and isinstance(
...
class IRNode:
    ...
    def copy(
        ...
        *,
        ...
        assert (
            not self.was_forced
        ), "modifying a forced node doesn't make sense"
    ...
    def trivial(self):
        ...
        if self.ir_type == "bytes":
            ...
            return len(self.value) == self.kwargs["min_size"] and not any(
                self.value
            )
...
def ir_value_permitted(value, ir_type, kwargs):
    ...
    raise NotImplementedError(
        f"unhandled type {type(value)} of ir value {value}"
    )
    ...
    def draw_integer(
        ...
        *,
        ...
        if weights is not None:
            ...
            idx = sampler.sample(
                self._cd, forced=forced_idx, fake_forced=fake_forced
            )
        ...
        if min_value is None and max_value is None:
            return self._draw_unbounded_integer(
                forced=forced, fake_forced=fake_forced
            )
    ...
    def draw_float(
        ...
        *,
        ...
        while True:
            ...
            i = (
                sampler.sample(
                    self._cd, forced=forced_i, fake_forced=fake_forced
                )
                if sampler
                else 0
            )
        ...
        *,
        ...
        if (
            ...
            and self.draw_boolean(
                7 / 8,
                forced=None if forced is None else False,
                fake_forced=fake_forced,
            )
        ):
            ...
            forced = self._draw_bounded_integer(
                lower=(
                    center if above else max(lower, center - 2**force_bits - 1)
                ),
                upper=(
                    center
                    if not above
                    else min(upper, center + 2**force_bits - 1)
                ),
        ...
        assert forced is None or result == forced, (
            result,
            forced,
            center,
            above,
        ...
        *,
        ...
        def permitted(f):
            ...
            if 0 < abs(f) < smallest_nonzero_magnitude:
                return False
            return sign_aware_lte(min_value, f) and sign_aware_lte(
                f, max_value
            )
...
class ConjectureData:
    ...
    def __init__(
        ...
        *,
        ...
        self._stateful_run_times: "DefaultDict[str, float]" = defaultdict(
            float
        )
        self.max_depth = 0
    ...
    def draw_integer(
        ...
        *,
        ...
        if self.ir_tree_nodes is not None and observe:
            node_value = self._pop_ir_tree_node(
                "integer", kwargs, forced=forced
            )
            if forced is None:
        ...
        *,
        ...
        if forced is not None:
            ...
            assert math.isnan(forced) or (
                sign_aware_lte(min_value, forced)
                and sign_aware_lte(forced, max_value)
            )
    ...
    def draw_boolean(
        ...
        *,
        ...
        eps = (
            2 ** (-64) if isinstance(self.provider, HypothesisProvider) else 0
        )
        ...
        if node.ir_type != ir_type or not ir_value_permitted(
            ...
            try:
                value = buffer_to_ir(
                    ir_type,
                    kwargs,
                    buffer=buffer + bytes(BUFFER_SIZE - len(buffer)),
                )
            except StopTest:
    ...
    def draw(
        ...
        try:
            ...
            try:
                ...
                add_note(
                    err, f"while generating {key[9:]!r} from {strategy!r}"
                )
                raise
        finally:
            self.stop_example()
    ...
    def stop_example(self, *, discard: bool = False) -> None:
        ...
        if not discard:
            if self.labels_for_structure_stack:
                self.labels_for_structure_stack[-1].update(
                    labels_for_structure
                )
            else:
                self.tags.update(
                    [structural_coverage(l) for l in labels_for_structure]
                )
    ...
    def examples(self) -> Examples:
        ...
        if self.__examples is None:
            self.__examples = Examples(
                record=self.__example_record, blocks=self.blocks
            )
