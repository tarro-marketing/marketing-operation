...
def dominance(left, right):
    ...
    for target in set(left.target_observations) | set(
        right.target_observations
    ):
    ...
    def run(self):
        ...
        while i >= 0 and not self.__engine.interesting_examples:
            ...
            def allow_transition(source, destination):
                ...
                if (
                    dominance(destination, source)
                    == DominanceRelation.LEFT_DOMINATES
                ):
            ...
            shrunk = self.__engine.shrink(
                target, allow_transition=allow_transition
            )
