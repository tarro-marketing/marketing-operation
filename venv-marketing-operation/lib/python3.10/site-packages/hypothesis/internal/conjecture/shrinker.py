    ...
    def accept(run_step):
        ...
        def run(self):
            raise NotImplementedError(
                "Shrink passes should not be run directly"
            )
    ...
    * It's often worth trying one or two special minimal values
    ...
    def __init__(
        ...
        *,
        ...
        self.__allow_transition = allow_transition or (
            lambda source, destination: True
        )
    ...
    def consider_new_buffer(self, buffer):
        ...
        buffer = bytes(buffer)
        return buffer.startswith(self.buffer) or self.incorporate_new_buffer(
            buffer
        )
    ...
    def shrink(self):
        ...
        try:
            ...
            if self.engine.report_debug_info:
                ...
                misaligned = (
                    self.engine.misaligned_count - self.initial_misaligned
                )
        ...
        for start, end in sorted(
            ...
            for n_attempt in range(500):  # pragma: no branch
                ...
                result = self.engine.cached_test_function(
                    buf_attempt_fixed,
                    extend=BUFFER_SIZE - len(buf_attempt_fixed),
                )
                ...
                if not (
                    ...
                    for ex, res in zip(
                        shrink_target.examples, result.examples
                    ):
        ...
        def descendants():
            ...
            while lo + 1 < hi:
                ...
                if ls[mid].ir_start >= ancestor.ir_end:
                    ...
                    lo = mid
            return [
                t for t in ls[i + 1 : hi] if t.ir_length < ancestor.ir_length
            ]
    ...
    def try_shrinking_nodes(self, nodes, n):
        ...
        initial_attempt = replace_all(
            self.nodes,
            [
                (node.index, node.index + 1, [node.copy(with_value=n)])
                for node in nodes
            ],
        )
        ...
        for ex in self.examples:
            ...
            in_replaced = [
                c for c in replacement.children if c.ir_start >= end
            ]
            ...
            regions_to_delete.add(
                (
                    in_original[0].ir_start,
                    in_original[-len(in_replaced)].ir_start,
                )
            )
    ...
    def duplicated_nodes(self):
        ...
        for node in self.nodes:
            duplicates[
                (node.ir_type, ir_value_key(node.ir_type, node.value))
            ].append(node)
    ...
    def redistribute_block_pairs(self, chooser):
        ...
        node = chooser.choose(
            self.nodes,
            lambda node: node.ir_type == "integer" and not node.trivial,
        )
    ...
    def lower_blocks_together(self, chooser):
        ...
        def lower(k):
            ...
            attempt[block.start : block.end] = int_to_bytes(
                m - k, block.length
            )
    ...
    def minimize_nodes(self, nodes):
        ...
        assert all(
            ...
            and ir_value_equal(ir_type, node.value, value)
            for node in nodes
        )
    ...
    def minimize_individual_nodes(self, chooser):
        ...
        if chooser.choose([True, False]):
            ...
            self.consider_new_tree(
                lowered[: ex.ir_start] + lowered[ex.ir_end :]
            )
        else:
            node = self.nodes[
                chooser.choose(range(node.index + 1, len(self.nodes)))
            ]
            self.consider_new_tree(
                lowered[: node.index] + lowered[node.index + 1 :]
            )
    ...
    def run_node_program(self, i, description, original, repeats=1):
        ...
        """
        if (
            i + len(description) > len(original.examples.ir_tree_nodes)
            or i < 0
        ):
            return False
        ...
        for _ in range(repeats):
            for k, command in reversed(list(enumerate(description))):
                ...
                if command == "X":
                    ...
                    raise NotImplementedError(
                        f"Unrecognised command {command!r}"
                    )
