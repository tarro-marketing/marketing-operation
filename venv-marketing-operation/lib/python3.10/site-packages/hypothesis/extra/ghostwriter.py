...
from hypothesis.strategies._internal.lazy import (
    LazyStrategy,
    unwrap_strategies,
)
...
def _exceptions_from_docstring(doc: str) -> Tuple[Type[Exception], ...]:
    ...
    raises = []
    for excname in re.compile(r"\:raises\s+(\w+)\:", re.MULTILINE).findall(
        doc
    ):
...
def _type_from_doc_fragment(token: str) -> Optional[type]:
    ...
    if coll_match is not None:
        ...
        if elems is not None and coll_token in (
            "list",
            "sequence",
            "collection",
        ):
...
def _strategy_for(
    ...
    for pattern in (
        ...
        if elements or types:
            return (
                st.sampled_from(elements) if elements else st.nothing()
            ) | (
...
def _get_params_builtin_fn(func: Callable) -> List[inspect.Parameter]:
    if (
        ...
        for arg in args.split(", "):
            ...
            if (
                _iskeyword(arg.lstrip("*"))
                or not arg.lstrip("*").isidentifier()
            ):
...
def _get_params(func: Callable) -> Dict[str, inspect.Parameter]:
    ...
    try:
        ...
        if [(p.name, p.kind) for p in params] == placeholder:
            params = (
                _get_params_ufunc(func)
                or _get_params_builtin_fn(func)
                or params
            )
...
def _params_to_dict(
    ...
    return OrderedDict(
        (p.name, p) for p in params if p.kind not in var_param_kinds
    )
...
def _imports_for_object(obj):
    ...
    if type(obj) is getattr(types, "UnionType", object()):
        return {
            mod
            for mod, _ in set().union(*map(_imports_for_object, obj.__args__))
        }
...
def _imports_for_strategy(strategy):
    ...
    if isinstance(strategy, LazyStrategy):
        ...
        if re.match(r"from_(type|regex)\(", repr(strategy)):
            ...
            module = _get_module(strategy.function).replace(
                "._array_helpers", ".numpy"
            )
...
def _valid_syntax_repr(strategy):
    ...
    try:
        ...
        if isinstance(strategy, OneOfStrategy):
            ...
            for s in strategy.element_strategies:
                if isinstance(s, SampledFromStrategy) and s.elements == (
                    os.environ,
                ):
                    continue
...
def _write_call(
    ...
    *pass_variables: str,
    ...
    return SUPPRESS_BLOCK.format(
        ...
        exceptions=(
            "(" + ", ".join(exnames) + ")" if len(exnames) > 1 else exnames[0]
        ),
    )
...
def _make_test_body(
    *funcs: Callable,
    ...
    assertions: str = "",
    style: str,
    given_strategies: Optional[
        Mapping[str, Union[str, st.SearchStrategy]]
    ] = None,
    ...
    with _with_any_registered():
        ...
        reprs = [
            ((k, *_valid_syntax_repr(v))) for k, v in given_strategies.items()
        ]
...
def _annotate_args(
    ...
    for func in funcs:
        try:
            params = tuple(
                get_signature(func, eval_str=True).parameters.values()
            )
        except Exception:
...
def _join_generics(
    ...
    if (
        origin_type_data is not None
        and origin_type_data[0] == "typing.Optional"
    ):
        annotations = (
            annotation
    ...
    return _AnnotationData(
        "{}[{}]".format(origin_type, ", ".join(arg_types)), imports
    )
...
def _parameter_to_annotation(parameter: Any) -> Optional[_AnnotationData]:
    ...
    if isinstance(parameter, list):
        ...
        return _AnnotationData(
            "[{}]".format(", ".join(arg_type_names)), new_imports
        )
    ...
    if isinstance(parameter, type):
        if parameter.__module__ == "builtins":
            return _AnnotationData(
                (
                    "None"
                    if parameter.__name__ == "NoneType"
                    else parameter.__name__
                ),
                set(),
            )
    ...
    if type_name.startswith("hypothesis.strategies."):
        return _AnnotationData(
            type_name.replace("hypothesis.strategies", "st"), set()
        )
    ...
    if origin_type is None or origin_type == parameter:
        return _AnnotationData(
            type_name, set(type_name.rsplit(".", maxsplit=1)[:-1])
        )
...
def _are_annotations_used(*functions: Callable) -> bool:
    for function in functions:
        try:
            ...
            if any(
                param.annotation != inspect.Parameter.empty for param in params
            ):
                return True
    return False
...
def _make_test(imports: ImportSet, body: str) -> str:
    ...
    direct = {
        f"import {i}" for i in imports - do_not_import if isinstance(i, str)
    }
    from_imports = defaultdict(set)
    for module, name in {i for i in imports if isinstance(i, tuple)}:
        if not (
            module.startswith("hypothesis.strategies") and name in st.__all__
        ):
    ...
    if callable(thing):
        ...
        if hasattr(thing, "__all__"):
            ...
            funcs = [
                ...
                and not is_mock(v)
                and (
                    (not pkg) or getattr(v, "__module__", pkg).startswith(pkg)
                )
            ...
            if pkg and any(
                ...
                funcs = [
                    f for f in funcs if getattr(f, "__module__", pkg) == pkg
                ]
    else:
...
def magic(
    *modules_or_functions: Union[Callable, types.ModuleType],
    ...
    for group in names.values():
        if (
            len(group) >= 2
            and len({frozenset(_get_params(f)) for f in group}) == 1
        ):
            sentinel = object()
            returns = {
                get_type_hints(f).get("return", sentinel) for f in group
            }
    ...
    for name, func in sorted(by_name.items()):
        ...
        if (len(hints) == len(params) == 2) or (
            ...
            if a == b and len(arg1) == len(arg2) <= 3:
                ...
                distributes_over = getattr(
                    sys.modules[_get_module(func)], known, None
                )
                make_(
                    _make_binop_body,
                    func,
...
def _make_roundtrip_body(funcs, except_, style, annotate):
    ...
    test_lines = [
        ...
        *(
            _write_call(
                f, f"value{i}", assign=f"value{i + 1}", except_=except_
            )
...
def _make_equiv_errors_body(funcs, except_, style, annotate):
    ...
    test_lines = [
        EQUIV_FIRST_BLOCK.format(indent(first_call, prefix="    "), catch)
    ]
...
def equivalent(
    *funcs: Callable,
    ...
    if allow_same_errors and not any(
        issubclass(Exception, ex) for ex in except_
    ):
        imports, source_code = _make_equiv_errors_body(
            funcs, except_, style, annotate
        )
    else:
        imports, source_code = _make_equiv_body(
            funcs, except_, style, annotate
        )
    ...
    *,
    ...
    if identity is not None:
        ...
        if identity is ...:
            try:
                identity = find(
                    operands, lambda x: True, settings=_quietly_settings
                )
            except Exception:
...
def ufunc(
    ...
    *,
    ...
    return _make_test(
        *_make_ufunc_body(
            func, except_=except_, style=style, annotate=annotate
        )
    )
...
def _make_ufunc_body(func, *, except_, style, annotate):
    ...
    if all(obj_sigs) or not any(obj_sigs):
        ...
        types = (
            f"sampled_from([sig for sig in {qname}.types if 'O' not in sig])"
        )
