...
def range_indexes(
    ...
    * name is the name of the index. If st.none(), the index will have no name.
    ...
    return st.builds(
        pandas.RangeIndex, st.integers(min_size, max_size), name=name
    )
...
def indexes(
    ...
    * name is a strategy for strings or ``None``, which will be passed to
    ...
    return ValueIndexStrategy(
        elements, dtype, min_size, max_size, unique, name
    )
    ...
    def result(draw):
        ...
        if len(index) > 0:
            ...
            return pandas.Series(
                result_data, index=index, dtype=dtype, name=draw(name)
            )
        else:
            return pandas.Series(
                (),
            ...
            def rows_only(draw):
                ...
                if len(index) > 0:
                    return pandas.DataFrame(
                        [row() for _ in index], index=index
                    )
                else:
            ...
            result = pandas.DataFrame(
                OrderedDict(
                    (
                        c.name,
                        pandas.Series(
                            np.zeros(dtype=c.dtype, shape=len(index)),
                            dtype=c.dtype,
                        ),
                    )
                    for c in rewritten_columns
