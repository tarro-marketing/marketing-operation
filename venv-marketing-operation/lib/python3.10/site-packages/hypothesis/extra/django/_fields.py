...
def integers_for_field(min_value, max_value):
    def inner(field):
        return st.integers(
            *numeric_bounds_from_validators(field, min_value, max_value)
        )
...
_global_field_lookup: _FieldLookUpType = {
    ...
    df.FloatField: lambda field: st.floats(
        *numeric_bounds_from_validators(field),
        allow_nan=False,
...
def _for_text(field):
    ...
    regexes = [
        ...
        for v in field.validators
        if isinstance(v, django.core.validators.RegexValidator)
        and not v.inverse_match
    ]
    if regexes:
...
def _for_model_multiple_choice(field):
    ...
    return st.lists(
        _model_choice_strategy(field), min_size=min_size, unique=True
    )
...
def from_field(field: F) -> st.SearchStrategy[Union[F, None]]:
    ...
    if not isinstance(field, df.ModelChoiceField) and getattr(
        field, "choices", False
    ):
        choices: list = []
