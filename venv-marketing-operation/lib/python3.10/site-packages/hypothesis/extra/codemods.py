...
class HypothesisFixComplexMinMagnitude(VisitorBasedCodemodCommand):
    ...
    @m.call_if_inside(
        m.Call(
            metadata=match_qualname("hypothesis.strategies.complex_numbers")
        )
    )
    def leave_Arg(self, original_node, updated_node):
        if m.matches(
            updated_node,
            m.Arg(keyword=m.Name("min_magnitude"), value=m.Name("None")),
        ):
    ...
    def leave_Call(self, original_node, updated_node):
        ...
        newargs = [
            (
                ...
                if arg.keyword
                or arg.star
                or p.kind is not Parameter.KEYWORD_ONLY
                else arg.with_changes(
...
class HypothesisFixHealthCheckAll(VisitorBasedCodemodCommand):
    ...
    @m.leave(
        m.Call(func=m.Attribute(m.Name("HealthCheck"), m.Name("all")), args=[])
    )
...
class HypothesisFixCharactersArguments(VisitorBasedCodemodCommand):
    ...
    def fn(self, original_node, updated_node):
        ...
        if any(
            m.matches(arg, m.Arg(keyword=m.Name("categories")))
            for arg in newargs
        ):
