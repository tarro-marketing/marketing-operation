...
NP_FIXED_UNICODE = tuple(int(x) for x in np.__version__.split(".")[:2]) >= (
    1,
    19,
)
...
def from_dtype(
    ...
    *,
    ...
    kwargs = {
        k: v for k, v in locals().items() if k != "dtype" and v is not None
    }
    ...
    if dtype.names is not None and dtype.fields is not None:
        ...
        subs = [
            from_dtype(dtype.fields[name][0], **kwargs) for name in dtype.names
        ]
    ...
    if dtype.kind == "b":
        ...
        result = st.integers(
            **compat_kw(min_value=-overflow, max_value=overflow - 1)
        )
    elif dtype.kind == "U":
        ...
        result = st.text(
            **compat_kw("alphabet", "min_size", max_size=max_size)
...
class ArrayStrategy(st.SearchStrategy):
    ...
    def set_element(self, val, result, idx, *, fill=False):
        ...
        try:
            elem_changed = (
                self._check_elements and val != result[idx] and val == val
            )
        ...
        if self.fill.is_empty:
            ...
            if self.unique:
                ...
                for i in range(len(result)):
                    self.set_element(
                        data.draw(self.element_strategy), result, i
                    )
        else:
            ...
            if needs_fill.any():
                ...
                self.set_element(
                    data.draw(self.fill), one_element, 0, fill=True
                )
...
def arrays(
    ...
    *,
    elements: Optional[
        Union[st.SearchStrategy[Any], Mapping[str, Any]]
    ] = None,
    ...
    if elements is None or isinstance(elements, Mapping):
        if dtype.kind in ("m", "M") and "[" not in dtype.str:
            ...
            return (
                ...
                .flatmap(
                    lambda d: arrays(d, shape=shape, fill=fill, unique=unique)
                )
            )
...
def complex_number_dtypes(
    *,
    endianness: str = "?",
    sizes: Union[
        Literal[64, 128, 192, 256], Sequence[Literal[64, 128, 192, 256]]
    ] = (
        64,
        128,
    ),
...
def byte_string_dtypes(
    *, endianness: str = "?", min_len: int = 1, max_len: int = 16
    ...
    return dtype_factory(
        "S", list(range(min_len, max_len + 1)), None, endianness
    )
...
def unicode_string_dtypes(
    *, endianness: str = "?", min_len: int = 1, max_len: int = 16
    ...
    order_check("len", 1, min_len, max_len)
    return dtype_factory(
        "U", list(range(min_len, max_len + 1)), None, endianness
    )
...
def array_dtypes(
    ...
    *,
    ...
    elements: st.SearchStrategy[tuple] = st.tuples(
        name_titles, subtype_strategy
    )
    if allow_subarrays:
        elements |= st.tuples(
...
def _unpack_dtype(dtype):
    ...
    if dtype_args and type(dtype) not in (
        getattr(types, "UnionType", object()),
        Union,
    ):
