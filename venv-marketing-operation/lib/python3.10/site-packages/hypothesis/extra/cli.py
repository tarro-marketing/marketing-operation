...
try:
    ...
    def obj_name(s: str) -> object:
        ...
        def describe_close_matches(
            ...
            public_names = [
                name
                for name in vars(module_or_class)
        ...
        if classname is None:
            ...
            try:
                ...
                raise click.UsageError(
                    f"Found the {modulename!r} module, but it doesn't have a "
                    f"{classname!r} class."
                    + describe_close_matches(module, classname)
                ) from err
            try:
    ...
    def write(func, writer, except_, style, annotate):  # \b disables autowrap
        ...
        try:
            ...
            try:
                theme = (
                    "default"
                    if guess_background_color() == "light"
                    else "monokai"
                )
                code = Syntax(
                    code, "python", background_color="default", theme=theme
                )
                Console().print(code, soft_wrap=True)
            except Exception:
                print(
                    "# Error while syntax-highlighting code", file=sys.stderr
                )
                print(code)
