    ...
    * ``dtype`` may be a dtype object or the string name of a
    ...
    if builtin is bool:
        ...
        if allow_subnormal is not None:
            ...
            subnormal = next_down(
                float(finfo.smallest_normal), width=finfo.bits
            )
...
class ArrayStrategy(st.SearchStrategy):
    def __init__(
        ...
        self.finfo = (
            None if self.builtin is not float else xp.finfo(self.dtype)
        )
    ...
    def do_draw(self, data):
        ...
        if self.fill.is_empty:
            ...
            try:
                ...
                f_types = (
                    f"type {types[0]}" if len(types) == 1 else f"types {types}"
                )
                raise InvalidArgument(
            ...
            for i in range(self.array_size):
                self.check_set_value(
                    elems[i], result[i], self.elements_strategy
                )
        else:
            ...
            try:
                ...
                raise InvalidArgument(
                    f"Could not create array via {f_expr}"
                ) from e
...
def _arrays(
    ...
    * ``unique`` specifies if the elements of the array should all be distinct
    ...
    check_xp_attributes(
        xp,
        ["finfo", "asarray", "zeros", "all", "isnan", "isfinite", "reshape"],
    )
