...
class LarkStrategy(st.SearchStrategy):
    ...
    def __init__(self, grammar, start, explicit, alphabet):
        ...
        for t in terminals:
            ...
            s = st.from_regex(
                t.pattern.to_regexp(), fullmatch=True, alphabet=alphabet
            )
            try:
                s.validate()
        ...
        while dict(nonterminals) != (
            nonterminals := {
                ...
                if (
                    clean := [
                        x for x in v if all(r.name in allowed_rules for r in x)
                    ]
                )
            }
        ):
        ...
        self.nonterminal_strategies = {
            k: st.sampled_from(sorted(v, key=len))
            for k, v in nonterminals.items()
        }
