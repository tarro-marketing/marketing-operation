...
def order_check(name, floor, min_, max_):
    if floor > min_:
        raise InvalidArgument(
            f"min_{name} must be at least {floor} but was {min_}"
        )
    if min_ > max_:
        raise InvalidArgument(
            f"min_{name}={min_} is larger than max_{name}={max_}"
        )
...
def broadcastable_shapes(
    ...
    * ``max_side`` is the largest size that an unaligned dimension can possess,
    ...
    if not (
        min_side <= 1 <= max_side
        or all(s <= max_side for s in shape[::-1][:dims])
    ):
        raise InvalidArgument(
...
def _hypothesis_parse_gufunc_signature(signature):
    ...
    input_shapes, output_shapes = (
        tuple(
            tuple(re.findall(_DIMENSION, a))
            for a in re.findall(_SHAPE, arg_list)
        )
...
def mutually_broadcastable_shapes(
    ...
    * ``result_shape`` as the resulting shape produced by broadcasting the N shapes
    ...
    if num_shapes is not not_set:
        ...
        all_shapes = (
            *parsed_signature.input_shapes,
            parsed_signature.result_shape,
        )
    ...
    if not (
        min_side <= 1 <= max_side
        or all(s <= max_side for s in base_shape[::-1][:dims])
    ):
        raise InvalidArgument(
            f"Given base_shape={base_shape}, there are no broadcast-compatible "
            f"shapes that satisfy all of {bound_name}={dims}, "
...
class MutuallyBroadcastableShapesStrategy(st.SearchStrategy):
    ...
    def do_draw(self, data):
        ...
        return BroadcastableShapes(
            input_shapes=tuple(
                add_shape(l_in, c) for l_in, c in zip(loop_in, core_in)
            ),
    ...
    def _draw_loop_dimensions(self, data, use=None):
        ...
        p_keep_extending_shape = _calc_p_continue(
            desired_avg=_gap / 2, max_size=_gap
        )
    ...
    def do_draw(self, data):
        ...
        for dim_size in self.shape:
            if dim_size == 0:
                ...
                continue
            strategy = st.integers(-dim_size, dim_size - 1) | st.slices(
                dim_size
            )
        ...
        if self.allow_ellipsis and data.draw(st.booleans()):
            ...
            while result[-1:] == [slice(None, None)] and data.draw(
                st.integers(0, 7)
            ):
                result.pop()
