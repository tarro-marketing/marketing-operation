...
class AddExamplesCodemod(VisitorBasedCodemodCommand):
    ...
    def __init__(
        self, context, fn_examples, strip_via=(), dec="example", width=88
    ):
        ...
        self.fn_examples = {
            k: tuple(
                d for x in nodes if (d := self.__call_node_to_example_dec(*x))
            )
...
def get_patch_for(func, failing_examples, *, strip_via=()):
    ...
    for ex, via in set(failing_examples):
        with suppress(Exception):
            ...
            if m.matches(
                node, m.Call(args=[m.ZeroOrMore(), data, m.ZeroOrMore()])
            ):
                return None
    ...
    if (
        ...
        and "given"
        not in module.__dict__  # more reliably present than `example`
    ):
