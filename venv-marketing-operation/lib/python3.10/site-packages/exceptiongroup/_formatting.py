    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            HTTPError = getattr(
                sys.modules.get("urllib.error", None), "HTTPError", ()
            )
            if sys.version_info[:2] <= (3, 11) and isinstance(
                exc_value, HTTPError
            ):
                self.__notes__ = None
            else:
    ...
    def format(self, *, chain=True, _ctx=None):
        ...
        for msg, exc in reversed(output):
            ...
            if exc.exceptions is None:
                ...
                yield from _ctx.emit(
                    f"... (max_group_depth is {max_group_depth})\n"
                )
            else:
...
def _compute_suggestion_error(exc_value, tb):
    ...
    for possible_name in d:
        ...
        max_distance = (
            (len(possible_name) + wrong_name_len + 3) * _MOVE_COST // 6
        )
...
def _levenshtein_distance(a, b, max_cost):
    ...
    post = 0
    while (
        a[: post or None] and b[: post or None] and a[post - 1] == b[post - 1]
    ):
        post -= 1
    a = a[: post or None]
    b = b[: post or None]
