...
class _Catcher:
    def __init__(
        self, handler_map: Mapping[tuple[type[BaseException], ...], _Handler]
    ):
    ...
    def handle_exception(self, exc: BaseException) -> BaseException | None:
        ...
        if new_exceptions:
            ...
            excgroup
            and len(excgroup.exceptions) == 1
            and excgroup.exceptions[0] is exc
        ):
            return exc
        else:
            return excgroup
...
def catch(
    __handlers: Mapping[
        type[BaseException] | Iterable[type[BaseException]], _Handler
    ],
