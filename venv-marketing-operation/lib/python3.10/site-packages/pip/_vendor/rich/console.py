...
class Console:
    ...
    def is_terminal(self) -> bool:
        ...
        if hasattr(
                sys.stdin,
                "__module__") and sys.stdin.__module__.startswith("idlelib"):
            # Return False for Idle which claims to be a tty but can't handle
            # ansi codes
            return False
    ...
    def size(self) -> ConsoleDimensions:
        ...
        return ConsoleDimensions(
            width -
            self.legacy_windows if self._width is None else self._width,
    ...
    def measure(self, renderable: RenderableType, *,
                options: Optional[ConsoleOptions] = None) -> Measurement:
    ...
    def render(
            ...
            options: Optional[ConsoleOptions] = None) -> Iterable[Segment]:
        ...
        if _options.max_width < 1:
            # No space to render anything. This prevents potential recursion
            # errors.
            return
        ...
        *objects: Any,
        ...
        with self:
            ...
            render_options = self.options.update(
                ...
                width=min(
                    width,
                    self.width) if width is not None else NO_CHANGE,
                height=height,
                no_wrap=no_wrap,
                markup=markup,
                highlight=highlight,
                ...
                if self.is_jupyter:  # pragma: no cover
                    ...
                    if WINDOWS:
                        ...
                        if use_legacy_windows_render:
                            ...
                            # Either a non-std stream on legacy Windows, or
                            # modern Windows.
                            text = self._render_buffer(self._buffer[:])
                            # https://bugs.python.org/issue37871
                            # https://github.com/python/cpython/issues/82052
                            # We need to avoid writing more than 32Kb in a
                            # single write, due to the above bug
    ...
    {make_tag("rect", x=0, y=offset, width=char_width * width, height=line_height + 0.25)}
        ...
        styles = "\n".join(
            f".{unique_id}-r{rule_no} {{ {css} }}" for css,
            rule_no in classes.items())
