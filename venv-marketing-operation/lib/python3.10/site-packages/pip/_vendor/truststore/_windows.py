...
def _handle_win_error(result: bool, _: Any, args: Any) -> Any:
    if not result:
        # Note, actually raises OSError after calling GetLastError and
        # FormatMessage
        raise WinError()
    return args
...
def _verify_peercerts_impl(
    ...
    if not cert_chain:
        raise ssl.SSLCertVerificationError(
            "Peer sent no certificates to verify")

    pCertContext = None
    hIntermediateCertStore = CertOpenStore(
    ...
    try:
        ...
        cert_enhkey_usage.rgpszUsageIdentifier = (
            c_char_p * 1)(OID_PKIX_KP_SERVER_AUTH)
