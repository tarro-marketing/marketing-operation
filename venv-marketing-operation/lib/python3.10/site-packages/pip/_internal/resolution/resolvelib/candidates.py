...
REQUIRES_PYTHON_IDENTIFIER = cast(
    NormalizedName, "<Python from Requires-Python>"
)
...
class _InstallRequirementBackedCandidate(Candidate):
    ...
    def iter_dependencies(
        self, with_requires: bool
    ) -> Iterable[Optional[Requirement]]:
        requires = self.dist.iter_dependencies() if with_requires else ()
        for r in requires:
            yield from self._factory.make_requirements_from_spec(
                str(r), self._ireq
            )
        yield self._factory.make_requires_python_requirement(
            self.dist.requires_python
        )
...
class ExtrasCandidate(Candidate):
    ...
    def __init__(
        ...
        *,
        ...
        self._comes_from = (
            comes_from if comes_from is not None else self.base._ireq
        )
    ...
    def iter_dependencies(
        ...
        valid_extras = self.extras.intersection(
            self.base.dist.iter_provided_extras()
        )
        invalid_extras = self.extras.difference(
            self.base.dist.iter_provided_extras()
        )
