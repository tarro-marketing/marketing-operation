...
from .core import (MAError, MaskedArray, add, array, asarray, concatenate,
                   ...
                   getmaskarray, make_mask_descr, mask_or, masked,
                   masked_array, nomask, ones, sort, zeros)
...
def _covhelper(x, y=None, rowvar=True, allow_masked=True):
    ...
    if y is None:
        ...
        xnotmask = np.logical_not(
            np.concatenate(
                (xmask, ymask), axis)).astype(int)
        ...
        if rowvar:
            for i in range(n - 1):
                for j in range(i + 1, n):
                    ...
                    _denom[i, j] = _denom[j, i] = ma.sqrt(
                        ma.multiply.reduce(_x))
        else:
            for i in range(n - 1):
                for j in range(i + 1, n):
                    _x = mask_cols(
