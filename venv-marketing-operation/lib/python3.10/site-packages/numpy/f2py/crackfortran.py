...
# block, the Fortran 2008 construct needs special handling in the rest of
# the file
...
parameterpattern = re.compile(
    beforethisafter %
    ('', 'parameter', 'parameter', r'\s*\(.*'), re.I), 'parameter'
...
crackline_bindlang = re.compile(
    r'\s*bind\(\s*(?P<lang>[^,]+)\s*,\s*name\s*=\s*"(?P<lang_name>[^"]+)"\s*\)',
    re.I)
    ...
    if not m:
        ...
        if 'externals' in groupcache[groupcounter]:
            for name in groupcache[groupcounter]['externals']:
                ...
                if m1:
                    ...
                    return
        if verbose > 1 or (
                verbose == 1 and currentfilename.lower().endswith('.pyf')):
            previous_context = None
        ...
        if (m1) and (not m1.group('this') == groupname[groupcounter]):
            raise Exception(
                ...
                (repr(
                    m1.group('this')), repr(
                    groupname[groupcounter]), filepositiontext))
        if skipblocksuntil == groupcounter:
            skipblocksuntil = -1
        grouplist[groupcounter - 1].append(groupcache[groupcounter])
        grouplist[groupcounter - 1][-1]['body'] = grouplist[groupcounter]
...
def _resolvenameargspattern(line):
    ...
    if m1:
        return m1.group('name'), m1.group(
            'args'), m1.group('result'), bind_cname
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if case in ['call', 'callfun']:  # set parents variables
            ...
            try:
                del groupcache[groupcounter]['vars'][name][groupcache[groupcounter]
                                                           ['vars'][name]['attrspec'].index('external')]
            except Exception:
                pass
...
def updatevars(typespec, selector, attrspec, entitydecl):
    ...
    for e in el:
        for e1 in [
            x.strip() for x in markoutercomma(
                removespaces(
                    markinnerspaces(e)),
                comma=' ').split('@ @')]:
            if e1:
    ...
    for e in el1:
        ...
        if ename in groupcache[groupcounter]['vars']:
            ...
            edecl['typename'] = typename
            edecl['attrspec'] = copy.copy(attrspec)
        if 'external' in (edecl.get('attrspec') or []
            ...
            if m1:
                ...
                if 'len' in d1 and 'array' in d1:
                    if d1['len'] == '':
                        ...
                        if ('charselector' not in edecl) or (
                                not edecl['charselector']):
                            edecl['charselector'] = {}
...
def get_useparameters(block, param_map=None):
    ...
    for usename, mapping in list(usedict.items()):
        ...
        if mapping:
            errmess(
                'get_useparameters: mapping for %s not impl.\n' %
                (mapping))
...
def _selected_real_kind_func(p, r=0, radix=0):
    # XXX: This should be processor dependent
    # This is only verified for 0 <= p <= 20, possibly good for p <= 33 and
    # above
    if p < 7:
        return 4
    if p < 16:
            ...
            for a in attr:
                ...
                if intent:
                    if 'intent' not in vars[n]:
                        vars[n]['intent'] = []
                    for c in [x.strip()
                              for x in markoutercomma(intent).split('@,@')]:
                ...
                if check is not None:
                    if 'check' not in vars[n]:
                        vars[n]['check'] = []
                    for c in [x.strip()
                              for x in markoutercomma(check).split('@,@')]:
    ...
    if a_is_expr:
        ...
        if a not in block['vars']:
            if orig_a in block['vars']:
                ...
                block['vars'][a] = {}
        if 'externals' in block and orig_a in block['externals'] + \
                block['interfaced']:
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if blocktype == 'block data' and name == '_BLOCK_DATA_':
        name = ''
    ret = '%s%s%s %s%s%s %s%s%s%s%s%s%send %s %s' % (tab,
                                                     prefix,
                                                     blocktype,
                                                     name,
                                                     args,
