...
def test_gh22819_cli(capfd, gh22819_cli, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        f2pycli()
        gen_paths = [
            item.name for item in ipath.parent.rglob("*") if item.is_file()]
...
def test_untitled_cli(capfd, hello_world_f90, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f"f2py --backend meson -c {ipath}".split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
@pytest.mark.skipif((platform.system() != 'Linux')
                    or (sys.version_info <= (3, 12)), reason='Compiler and 3.12 required')
...
def test_mod_gen_gh25263(capfd, hello_world_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath} -m {MNAME} -h hi.pyf'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
