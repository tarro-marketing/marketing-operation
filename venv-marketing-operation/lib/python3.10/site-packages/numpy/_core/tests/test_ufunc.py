...
class TestUfunc:
    ...
    def test_reduceat_shifting_sum(self):
        ...
        x = np.arange(L)
        idx = np.array(
            list(zip(np.arange(L - 2), np.arange(L - 2) + 2))).ravel()
            ...
            for x, y in itertools.product([aa, -aa], [bb, -bb]):
                ...
                for tcout in 'efdg':
                    ...
                    if tc in 'FDG':
                        ...
                        assert_raises(
                            TypeError, np.true_divide, x, y, dtype=dtout)
                    else:
                        tgt = float(x) / float(y)
    ...
    def test_cross1d(self):
        ...
        assert_raises(ValueError, umt.cross1d, a,
                      np.arange(3.), np.zeros((3, 4)))
    ...
    def test_object_comparison(self):
        ...
        assert_equal(
            np.equal(
                arr1d, arr1d), np.array(
                [True]))  # normal behavior is a cast
    ...
    def test_object_array_accumulate_inplace(self):
        ...
        np.add.accumulate(arr, out=arr)
        assert_array_equal(arr, np.array(
            [[1] * i for i in [1, 3, 6, 10]], dtype=object), )
        ...
        np.add.accumulate(arr, out=arr, axis=-1)
        assert_array_equal(arr[0, :], np.array(
            [[2] * i for i in [1, 3, 6, 10]], dtype=object), )
    ...
    def test_reduce_wrong_dimension_output(
            self, f_reduce, keepdims, out_shape):
...
@pytest.mark.parametrize(
    "method", [
        np.add.accumulate, np.add.reduce, pytest.param(
            lambda x: np.add.reduceat(
                x, [0]), id="reduceat"), pytest.param(
                    lambda x: np.log.at(
                        x, [2]), id="at")])
...
@pytest.mark.parametrize(["dt1", "dt2"], [("S", "U"),
                         ("U", "S"), ("S", "d"), ("S", "V"), ("U", "l")])
