...
class TestNonarrayArgs:
    ...
    def test_std_with_mean_keyword_keepdims_true_masked(self):
        ...
        mean_out = ma.array(
            [[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]]
        )
        std_out = ma.array(
            [[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]]
        )
...
class TestSeterr:
    def test_default(self):
        ...
        assert_equal(
            err,
            dict(divide="warn", invalid="warn", over="warn", under="ignore"),
        )
        ...
        with np.errstate(all="raise"):
            ...
            if not np.isnan(ft_tiny):
                self.assert_raises_fpe(
                    underflow, lambda a, b: a / b, ft_tiny, ft_max
                )
                self.assert_raises_fpe(
                    underflow, lambda a, b: a * b, ft_tiny, ft_tiny
                )
            self.assert_raises_fpe(
                overflow, lambda a, b: a * b, ft_max, ftype(2)
            )
            self.assert_raises_fpe(
                overflow, lambda a, b: a / b, ft_max, ftype(0.5)
            )
            self.assert_raises_fpe(
            ...
            self.assert_raises_fpe(
                overflow, np.power, ftype(2), ftype(2**fi.nexp)
            )
            self.assert_raises_fpe(
                divbyzero, lambda a, b: a / b, ftype(1), ftype(0)
            )
            self.assert_raises_fpe(
                invalid, lambda a, b: a / b, ftype(np.inf), ftype(np.inf)
            )
            self.assert_raises_fpe(
                invalid, lambda a, b: a / b, ftype(0), ftype(0)
            )
            self.assert_raises_fpe(
                invalid, lambda a, b: a - b, ftype(np.inf), ftype(np.inf)
            )
            ...
            self.assert_raises_fpe(
                invalid, lambda a, b: a * b, ftype(0), ftype(np.inf)
            )
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        assert_equal(
            promote_func(np.int32(-1), np.array([u64])), np.dtype(np.float64)
        )
        assert_equal(promote_func(f64, np.array([f32])), np.dtype(np.float64))
        assert_equal(
            promote_func(fld, np.array([f32])), np.dtype(np.longdouble)
        )
        assert_equal(
            promote_func(np.array([f64]), fld), np.dtype(np.longdouble)
        )
        assert_equal(
            promote_func(fld, np.array([c64])), np.dtype(np.clongdouble)
        )
        assert_equal(
            promote_func(c64, np.array([f64])), np.dtype(np.complex128)
        )
        assert_equal(
            promote_func(np.complex64(3j), np.array([f64])),
            np.dtype(np.complex128),
        )
        assert_equal(
            promote_func(np.array([f32]), c128), np.dtype(np.complex128)
        )
    ...
    def test_coercion(self):
        ...
        for a in [np.array([True, False]), np.array([-3, 12], dtype=np.int8)]:
            ...
            assert_equal(
                b.dtype, np.dtype(np.longdouble), "array type %s" % a.dtype
            )
            ...
            assert_equal(
                b.dtype, np.dtype(np.clongdouble), "array type %s" % a.dtype
            )
    ...
    def test_can_cast_structured_to_simple(self):
        ...
        assert_(
            not np.can_cast([("f0", ("i4,i4"), (2,))], "i4", casting="unsafe")
        )

    @pytest.mark.xfail(
        np._get_promotion_state() != "legacy",
...
class TestFromiter:
    ...
    def test_lengths(self):
        ...
        assert_(len(a20) == 20)
        assert_raises(
            ValueError, np.fromiter, self.makegen(), int, len(expected) + 10
        )
...
class TestNonzero:
    ...
    def test_count_nonzero_axis(self):
        ...
        assert_raises(
            TypeError, np.count_nonzero, m, axis=np.array([[1], [2]])
        )
    ...
    def test_count_nonzero_axis_all_dtypes(self):
        ...
        for dt in np.typecodes["All"]:
            ...
            if dt != "V":
                ...
                assert_equal(
                    np.count_nonzero(m, axis=None), expected, err_msg=err_msg
                )
    ...
    def test_countnonzero_keepdims(self):
        ...
        assert_equal(
            np.count_nonzero(a, axis=0, keepdims=True), [[1, 2, 3, 0]]
        )
        assert_equal(
            np.count_nonzero(a, axis=1, keepdims=True), [[1], [2], [3]]
        )
...
class TestIndex:
    def test_boolean(self):
        ...
        assert_(
            (
                np.array([a[0][V > 0], a[1][V > 0], a[2][V > 0]])
                == a[:, V > 0]
            ).all()
        )
...
class TestBinaryRepr:
    ...
    def test_large_neg_int64(self):
        # See gh-14289.
        assert_equal(
            np.binary_repr(np.int64(-(2**62)), width=64), "11" + "0" * 62
        )
...
class TestClip:
    ...
    def test_clip_min_max_args(self):
        ...
        assert_array_equal(
            np.clip(arr, min=None, max=2), np.clip(arr, None, 2)
        )
...
class TestIsclose:
    ...
    def test_timedelta(self):
        ...
        assert np.isclose(
            a, a, atol=np.timedelta64(1, "ns"), equal_nan=True
        ).all()
    ...
    def check_like_function(self, like_function, value, fill_value=False):
        ...
        for d, dtype in self.data:
            ...
            for s in self.shapes:
                for o in "CFA":
                    sz = like_function(
                        d, dtype=dtype, shape=s, order=o, **fill_kwarg
                    )
                ...
                if d.ndim != len(s):
                    assert_equal(
                        ...
                        np.argsort(
                            np.empty(s, dtype=dtype, order="C").strides
                        ),
                    )
                else:
                    assert_equal(
...
class TestArgwhere:
    ...
    def test_2D(self):
        ...
        assert_array_equal(
            np.argwhere(x > 1), [[0, 2], [1, 0], [1, 1], [1, 2]]
        )
...
class TestMoveaxis:
    ...
    def test_errors(self):
        ...
        assert_raises_regex(
            AxisError, "source.*out of bounds", np.moveaxis, x, 3, 0
        )
        assert_raises_regex(
            AxisError, "source.*out of bounds", np.moveaxis, x, -4, 0
        )
        assert_raises_regex(
