...
class TestEinsum:
    ...
    def test_einsum_views(self):
        ...
        for do_opt in [True, False]:
            ...
            # diagonal with various ways of broadcasting an additional
            # dimension
            a = np.arange(27)
            a.shape = (3, 3, 3)
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            if hasattr(b, 'astype'):
                b = b.astype(dtype)
            assert_equal(np.einsum("...i->...", a, optimize=do_opt), b)
            assert_equal(np.einsum(a, [Ellipsis, 0], [
                         Ellipsis], optimize=do_opt), b)
        ...
        for n in range(1, 17):
            ...
            if hasattr(b, 'astype'):
                b = b.astype(dtype)
            assert_equal(np.einsum("i...->...", a, optimize=do_opt), b)
            assert_equal(np.einsum(a, [0, Ellipsis], [
                         Ellipsis], optimize=do_opt), b)
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum(
                    "...i, ...i", a, b, optimize=do_opt), np.inner(
                    a, b))
            assert_equal(np.einsum(a, [Ellipsis, 0], b, [
                         Ellipsis, 0], optimize=do_opt), np.inner(a, b))
        ...
        for n in range(1, 11):
            ...
            assert_equal(np.einsum("i..., i...", a, b, optimize=do_opt),
                         np.inner(a.T, b.T).T)
            assert_equal(np.einsum(a, [0, Ellipsis], b, [
                         0, Ellipsis], optimize=do_opt), np.inner(a.T, b.T).T)
            ...
            for n in range(1, 17):
                if n < 8 or dtype != 'f2':
                    ...
                    assert_equal(np.einsum("ij,jk", a, b, optimize=do_opt),
                                 np.dot(a, b))
                    assert_equal(np.einsum(a, [0, 1], b, [
                                 1, 2], optimize=do_opt), np.dot(a, b))
        ...
        for n in range(1, 25):
            ...
            if np.dtype(dtype).itemsize > 1:
                ...
                assert_equal(
                    np.einsum(
                        "i,i", a, a, optimize=do_opt), np.dot(
                        a, a))
                ...
                assert_equal(np.einsum("i,i", a[1:], a[:-1], optimize=do_opt),
                             np.dot(a[1:], a[:-1]))
                assert_equal(
                    np.einsum("i,->i", a[1:], 2, optimize=do_opt), 2 * a[1:])
                assert_equal(
                    np.einsum(",i->i", 2, a[1:], optimize=do_opt), 2 * a[1:])
    ...
    def test_einsum_misc(self):
        ...
        assert_equal(np.einsum('ij...,j...->i...', a,
                     b, optimize=True), [[[2], [2]]])
    ...
    def test_subscript_range(self):
        ...
        assert_raises(ValueError, lambda: np.einsum(
            a, [0, 52], b, [52, 2], [0, 2], optimize=False))
        assert_raises(ValueError, lambda: np.einsum(
            a, [-1, 5], b, [5, 2], [-1, 2], optimize=False))
