...
IS_AVX = __cpu_features__.get(
    'AVX512F',
    False) or (
        __cpu_features__.get(
            'FMA3',
            False) and __cpu_features__.get(
                'AVX2',
            False))
...
def convert(s, datatype="np.float32"):
    ...
    if (datatype == "np.float64"):
        # make this into a c long long integer
        cp = pointer(c_longlong(i))
        # cast the int pointer to a double pointer
            ...
            for filename in files:
                ...
                with open(filepath) as fid:
                    file_without_comments = (
                        r for r in fid if not r[0] in ('$', '#'))
                    data = np.genfromtxt(
                        file_without_comments, dtype=(
                            '|S39', '|S39', '|S39', int), names=(
                            'type', 'input', 'output', 'ulperr'), delimiter=',', skip_header=1)
