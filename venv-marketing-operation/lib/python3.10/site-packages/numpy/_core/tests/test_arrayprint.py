...
class TestArray2String:
    ...
    def test_format_function(self):
        ...
        assert_(
            np.array2string(
                x,
                formatter={
                    'float_kind': lambda x: "%.2f" % x}) == "[0.00 1.00 2.00]")
        assert_(
            np.array2string(
                x,
                formatter={
                    'float': lambda x: "%.2f" % x}) == "[0.00 1.00 2.00]")
    ...
    def test_unstructured_void_repr(self):
        a = np.array([27, 91, 50, 75, 7, 65, 10, 8, 27, 91, 51,
                     49, 109, 82, 101, 100], dtype='u1').view('V8')
...
class TestPrintOptions:
    ...
    def test_floatmode(self):
        ...
        assert_equal(
            repr(x), "array([0.6104  , 0.922   , 0.457   , 0.0906  , 0.3733  , 0.007244,\n"
        ...
        assert_equal(repr(
            w[::5]), "array([1.0000e+00, 1.0000e+05, 1.0000e+10, 1.0000e+15, 1.0000e+20])")
        assert_equal(
            repr(wp),
        ...
        assert_equal(
            repr(x), "array([0.610352, 0.921875, 0.457031, 0.090576, 0.373291, 0.007244,\n"
...
def test_scalar_repr_numbers(dtype, value):
    ...
    if dtype.char == "g":
        ...
        normalized_name = np.dtype(
            f"{dtype.kind}{dtype.itemsize}").type.__name__
