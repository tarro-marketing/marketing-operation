        ...
        while min(feasible_count, infeasible_count) < min_count:
            ...
            if X is None:
                ...
                if size < 100000:
                    assert_(
                        not any(
                            sum(w) == b for w in itertools.product(
                                *ranges)))
                    infeasible_count += 1
            else:
                # Check the simplified decision problem agrees
...
def check_may_share_memory_exact(a, b):
    ...
    if got != exact:
        err_msg = "    " + "\n    ".join(
            [
                "base_a - base_b = %r" %
                (a.__array_interface__['data'][0] - b.__array_interface__['data'][0],), "shape_a = %r" %
                (a.shape,), "shape_b = %r" %
                (b.shape,), "strides_a = %r" %
                (a.strides,), "strides_b = %r" %
                (b.strides,), "size_a = %r" %
                (a.size,), "size_b = %r" %
                (b.size,)])
    ...
    def test_unary_gufunc_fuzz(self):
        ...
        for ndim in range(2, 6):
            ...
            while overlapping < min_count:
                ...
                if min(a.shape[-2:]) < 2 or min(b.shape[-2:]
                                                ) < 2 or a.shape[-1] < 2:
                    continue
