...
import numpy as np
import pytest
from numpy.testing import (IS_WASM, assert_, assert_array_equal, assert_equal,
...
class TestDateTime:
    ...
    def test_days_creation(self):
        ...
        assert_equal(
            np.array(
                '1967', dtype='M8[D]').astype('i8'), -3 * 365 - 1)
        assert_equal(
            np.array(
                '1968', dtype='M8[D]').astype('i8'), -2 * 365 - 1)
        ...
        assert_equal(np.array('2001-03-22',
                              dtype='M8[D]').astype('i8'),
    ...
    def test_datetime_array_str(self):
        ...
        assert_equal(
            np.array2string(
                ...
                separator=', ',
                formatter={
                    'datetime': lambda x: "'%s'" % np.datetime_as_string(
                        x,
                        timezone='UTC')}),
    ...
    def test_creation_overflow(self):
        ...
        for unit in ['ms', 'us', 'ns']:
            ...
            assert_equal(
                ...
                x[0].astype(
                    np.int64),
                err_msg='Datetime conversion error for unit %s' %
                unit)
    ...
    def test_datetime_busdaycalendar(self):
        ...
        assert_equal(bdd.holidays, np.array(
            ['2011-01-17', '2011-05-30', '2011-12-26'], dtype='M8'))
    ...
    @pytest.mark.parametrize("time_unit",
                             ["Y",
                              "M",
                              pytest.param("W",
                                           marks=pytest.mark.xfail(reason="gh-13197")),
                                 "D",
                                 "h",
                                 "m",
                                 "s",
