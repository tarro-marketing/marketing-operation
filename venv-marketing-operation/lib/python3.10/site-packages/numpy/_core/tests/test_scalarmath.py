...
from numpy.testing import (_SUPPORTS_SVE, IS_PYPY, _gen_alignment_data,
                           assert_, assert_almost_equal, assert_array_equal,
    ...
    def test_type_add(self):
        ...
        for k, atype in enumerate(types):
            ...
            for l, btype in enumerate(types):
                ...
                assert_equal(
                    c_scalar.dtype, c_array.dtype, "error with types (%d/'%c' + %d/'%c')" %
...
class TestConversion:
        ...
        for dt1 in 'bhlqp' + np.typecodes['Float']:
            ...
            for dt2 in 'bhlqp' + np.typecodes['Float']:
                ...
                assert_(not np.array(1, dtype=dt1)[()] < np.array(-1, dtype=dt2)[()],
                        "type %s and %s failed" % (dt1, dt2))
                assert_(np.array(-1,
                                 dtype=dt1)[()] == np.array(-1,
                                                            dtype=dt2)[()],
                        "type %s and %s failed" % (dt1,
                                                   dt2))
    ...
    def test_scalar_comparison_to_none(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert_(not np.float32(1) is None)
            assert_(not np.str_('test') is None)
            # This is dubious (see below):
            assert_(not np.datetime64('NaT') is None)

            assert_(np.float32(1) is not None)
            assert_(np.str_('test') is not None)
            # This is dubious (see below):
            assert_(np.datetime64('NaT') is not None)
        assert_(len(w) == 0)
...
class TestBitShifts:
    ...
    @pytest.mark.parametrize('op', [operator.rshift,
                             operator.lshift], ids=['>>', '<<'])
...
assert isinstance(res, type(res)) == np.bool
...
@pytest.mark.parametrize("op",
                         # TODO: Power is a bit special, but here mostly bools
                         # seem to behave oddly
