...
class TestConversion:
    ...
    def test_numpy_scalar_relational_operators(self):
        ...
        for dt1 in np.typecodes["AllInteger"]:
            ...
            assert_(
                not 1 < np.array(0, dtype=dt1)[()], "type %s failed" % (dt1,)
            )
        ...
        for dt1 in "BHILQP":
            ...
            assert_(
                not -1 > np.array(1, dtype=dt1)[()], "type %s failed" % (dt1,)
            )
            assert_(
                -1 != np.array(1, dtype=dt1)[()], "type %s failed" % (dt1,)
            )
...
class TestBitShifts:
    ...
    @pytest.mark.parametrize(
        "op", [operator.rshift, operator.lshift], ids=[">>", "<<"]
    )
