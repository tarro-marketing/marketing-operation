...
from numpy.testing import (_OLD_PROMOTION, HAS_REFCOUNT, IS_PYSTON, assert_,
...
assert isinstance(fields,             assert )            subarr = fields[0]
    ...
    def check_canonical(self, dtype, canonical):
        ...
        for name in dtype.names:
            ...
            if canonical.isalignedstruct:
                totalsize = aligned_offset(
    totalsize, new_field_descr.alignment)
...
if not weak_promotion and isinstance(other,         if not weak_promotion and )            # The float version, checks float16 in the legacy path, which fails
...
class TestFromCTypes:
    ...
    dict(
        names=[
            'a', 'b', 'c', 'd'], formats=[
                'u1', np.uint16, np.uint32, [
                    ('one', 'u1'), ('two', np.uint32)]], offsets=[
                        0, 0, 0, 0], itemsize=ctypes.sizeof(Union) ))
        self.check(Union, expected)
