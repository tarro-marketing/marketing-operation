    ...
    def conf_features_partial(self):
        ...
        if on_x86 and self.cc_is_icc:
            return dict(
                ...
                # Intel Compiler doesn't support AVX512F or AVX512CD
                # independently
                AVX512F=dict(
...
class _CCompiler:
    ...
    def __init__(self):
        ...
        for detect, searchin in (
                (detect_arch, platform), (detect_compiler, compiler_info)):
        ...
        if self.conf_noopt:
            self.dist_log(
                "Optimization is disabled by the Config",
                stderr=True)
            self.cc_noopt = True
...
class _Feature:
    ...
    def feature_extra_checks(self, name):
        ...
        self.dist_log(
            "Testing extra checks for feature '%s'" %
            name, extra_checks)
...
class _Parse:
    ...
    def __init__(self, cpu_baseline, cpu_dispatch):
        ...
        if self.cc_noopt:
            # skip parsing baseline and dispatch args and keep parsing target
            # groups
    ...
    def parse_targets(self, source):
        ...
        if start_pos == -1:
            self.dist_fatal(
                "expected to find '%s' within a C comment" %
                start_with)
        if end_pos == -1:
    ...
    def _parse_target_tokens(self, tokens):
        ...
        for tok in tokens:
            ...
            if ch in ('+', '-'):
                ...
                has_baseline, final_targets, extra_flags = self._parse_token_group(
    ...
    def _parse_token_policy(self, token):
        ...
        if token not in self._parse_policies:
            self.dist_fatal(
                "'%s' is an invalid policy name, available policies are" %
                token, self._parse_policies.keys())
        return token
    ...
    def _parse_policy_not_keepbase(
            self,
            has_baseline,
            final_targets,
            extra_flags):
    ...
    def _parse_policy_not_keepsort(
            self,
            has_baseline,
            final_targets,
            extra_flags):
    ...
    def generate_dispatch_header(self, header_path):
        ...
        with open(header_path, 'w') as f:
            ...
            for name in dispatch_names:
                ...
                """).format(
                    pfx=self.conf_c_prefix_,
                    name=name,
                    pre=self.feature_c_preprocessor(
                        name,
                        tabs=1))
        ...
        if not full or not target_sources:
            ...
            for tar in self.feature_sorted(target_sources):
                ...
                for name in ((tar,) if isinstance(tar, str) else tar):
                    extra_checks += self.feature_extra_checks(name)
                extra_checks = (
                    ' '.join(extra_checks) if extra_checks else "none")
