...
class SunOSCPUInfo(CPUInfoBase):
    ...
    def _is_ultraenterprice(self):
        return re.match(
            r'.*Ultra-Enterprise',
            self.info['uname_i']) is not None

    def _is_ultraenterprice10k(self):
        return re.match(
            r'.*Ultra-Enterprise-10000',
            self.info['uname_i']) is not None
...
class Win32CPUInfo(CPUInfoBase):
    ...
    def _has_mmx(self):
        if self.is_Intel():
            return (
                self.info[0]['Family'] == 5 and self.info[0]['Model'] == 4) or (
                self.info[0]['Family'] in [
                    6, 15])
        elif self.is_AMD():
...
if sys.platform.startswith(
        'linux'):  # variations: linux2,linux-i386 (any others?)
