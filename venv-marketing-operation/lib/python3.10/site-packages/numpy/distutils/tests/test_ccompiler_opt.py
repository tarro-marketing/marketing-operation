...
class _Test_CCompilerOpt:
    ...
    def get_targets(self, targets, groups, **kwargs):
        ...
        if len(targets) != len(gtargets) or not all(
                t in gtargets for t in targets):
    ...
    def expect_target_flags(self, targets, groups={}, **kwargs):
        ...
        for match_tar, match_flags in match_dict.items():
            ...
            if not match_flags:
                if len(flags) != 0:
                    raise AssertionError(
                        'expected to find empty flags in target "%s"' %
                        match_tar)
            if not re.match(match_flags, flags):
                raise AssertionError(
                    '"%s" flags "%s" not match "%s"' %
                    (match_tar, flags, match_flags))
    ...
    def test_args_validation(self):
        ...
        for baseline, dispatch in (
            ...
            try:
                ...
                raise AssertionError(
                    "excepted an exception for invalid arguments")
