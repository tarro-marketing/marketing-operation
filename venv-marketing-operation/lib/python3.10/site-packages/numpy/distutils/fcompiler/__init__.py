    ...
    def module_options(self, module_dirs, module_build_dir):
        ...
        if self.module_dir_switch is not None:
            if self.module_dir_switch[-1] == ' ':
                ...
                options.append(
                    self.module_dir_switch.strip() +
                    module_build_dir)
        else:
            print(
                'XXX: module_build_dir=%r option ignored' %
                (module_build_dir))
        ...
        if self.module_include_switch is not None:
            ...
            print(
                'XXX: Fix module_include_switch for ',
                self.__class__.__name__)
        return options
                ...
                for alias in klass.compiler_aliases:
                    if alias in fcompiler_aliases:
                        raise ValueError(
                            ...
                            (alias, klass.__name__, fcompiler_aliases[alias][1].__name__))
...
def _find_existing_fcompiler(compiler_types,
    ...
    for compiler_type in compiler_types:
        ...
        try:
            ...
            log.debug(
                "_find_existing_fcompiler: compiler_type='%s' raised DistutilsModuleError",
                compiler_type)
        except CompilerNotFound:
            log.debug(
                "_find_existing_fcompiler: compiler_type='%s' not found",
                compiler_type)
        if v is not None:
