...
class build_src(build_ext.build_ext):
    ...
    user_options = [
        ...
        ('f2pyflags=',
         None,
         "additional flags to f2py (use --f2py-opts= instead)"),
        # obsolete
        ('swigflags=',
         None,
         "additional flags to swig (use --swig-opts= instead)"),
        # obsolete
    ...
    def finalize_options(self):
        ...
        if self.build_src is None:
            ...
            self.build_src = os.path.join(
                self.build_base, 'src' + plat_specifier)
        ...
        for c in ['swig', 'swig_opt']:
            ...
            if v:
                if getattr(self, c):
                    log.warn(
                        'both build_src and build_ext define %s option' %
                        (o))
                else:
                    log.info(
                        'using "%s=%s" option from build_ext command' %
                        (o, v))
                    setattr(self, c, v)
    ...
    def build_npy_pkg_config(self):
        ...
        if build_npkg:
            for pkg, infos in self.distribution.installed_pkg_config.items():
                ...
                d = {'prefix': prefix}
                for info in infos:
                    install_dir, generated = self._build_npy_pkg_config(
                        info, d)
        ...
        for source in sources:
            ...
            if ext == '.src':  # Template file
                if self.inplace:
                    ...
                    target_dir = appendpath(
                        self.build_src, os.path.dirname(base))
                self.mkpath(target_dir)
                target_file = os.path.join(target_dir, os.path.basename(base))
                if (self.force or newer_group(
                        [source] + depends, target_file)):
    ...
    def f2py_sources(self, sources, extension):
                ...
                if os.path.isfile(source):
                    ...
                    if not os.path.isfile(target_file):
                        ...
                        target_file = os.path.join(
                            target_dir, name + 'module.c')
                        if not os.path.isfile(target_file):
                            raise DistutilsSetupError(
                                "%r missing" % (target_file,))
        ...
        if f2py_sources:
            ...
            if (self.force or newer_group(depends, target_file, 'newer')) \
                ...
                log.debug(
                    "  skipping '%s' f2py interface (up-to-date)" %
                    (source))
        else:
        ...
        if not os.path.isfile(target_file):
            raise DistutilsError(
                "f2py target file %r not generated" %
                (target_file,))
        ...
        if not skip_f2py:
            ...
            if not os.path.isfile(target_c):
                raise DistutilsSetupError(
                    "f2py target_c file %r not found" %
                    (target_c,))
            if not os.path.isfile(target_h):
                raise DistutilsSetupError(
                    "f2py target_h file %r not found" %
                    (target_h,))
            ...
            if ext == '.i':  # SWIG interface file
                ...
                if os.path.isfile(source):
                    ...
                    if typ is None:
                        ...
                        if typ2 is None:
                            log.warn(
                                'source %r does not define swig target, assuming %s swig target' %
                                (source, typ))
                        elif typ != typ2:
                            log.warn(
...
_f2py_user_module_name_match = re.compile(
    r'\s*python\s*module\s*(?P<name>[\w_]*?'
