...
class TestPolynomial:
    ...
    def test_polyfit(self):
        ...
        # Without scaling, since reduced chi2 is 1, the result should be the
        # same.
        ...
        mean, cov = np.polyfit(
            np.zeros(
                y.shape[0]), y, w=w, deg=0, cov="unscaled")
        assert_allclose(mean.std(), 0.5, atol=0.01)
    ...
    def test_integ_coeffs(self):
        ...
        assert_((p2.coeffs == [1 / 4. / 5., 1 / 3. /
                               4., 1 / 2. / 3., 9 / 1. / 2., 7, 6]).all())
    ...
    def test_poly_eq(self):
        p = np.poly1d([1, 2, 3])
        p2 = np.poly1d([1, 2, 4])
        assert_equal(p is None, False)
        assert_equal(p is not None, True)
