import numpy as np
from numpy import (common_type, iscomplex, iscomplexobj, isneginf, isposinf,
                   isreal, isrealobj, mintypecode, nan_to_num, real_if_close)
...
class TestNanToNum:
    ...
    def test_do_not_rewrite_previous_keyword(self):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            vals = nan_to_num(np.array((-1., 0, 1)) /
                              0., nan=np.inf, posinf=999)
        assert_all(np.isfinite(vals[[0, 2]]))
