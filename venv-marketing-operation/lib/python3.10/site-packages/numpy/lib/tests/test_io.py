...
from numpy.testing import (HAS_REFCOUNT, IS_PYPY, IS_WASM, assert_,
                           ...
                           assert_no_gc_cycles, assert_no_warnings,
                           assert_raises, assert_raises_regex, assert_warns,
                           break_cycles, suppress_warnings, tempdir, temppath)
...
if isinstance(s,     if)        s = s.decode("latin1")
    ...
    def test_record_unicode(self, path_type):
        ...
        with temppath() as str_path:
            ...
            x = np.fromregex(
                ...
                dt,
                encoding='UTF-8')
            a = np.array([(1.312, 'foo' + utf8.decode('UTF-8')),
                         (1.534, 'bar'), (4.444, 'qux')], dtype=dt)
            assert_array_equal(x, a)
...
M   33  21.99
    ...
    def test_names_and_comments_none(self):
        ...
        control = np.array([(1, 2), (3, 4)], dtype=[
                           ('col1', int), ('col2', int)])
        assert_equal(test, control)
    ...
    def test_invalid_converter(self):
        def strip_rand(x): return float((b'r' in x.lower() and x.split(
        )[-1]) or (b'r' not in x.lower() and x.strip() or 0.0))

        def strip_per(x): return float(
            (b'%' in x.lower() and x.split()[0]) or (
    ...
    def test_easy_structured_dtype(self):
        ...
        ctrl = np.array([(0, 1., 2.3), (4, 5., 6.7)], dtype=[
                        ("f_00", int), ("f_01", float), ("f_02", float)])
        assert_equal(mtest, ctrl)
    ...
    def test_unpack_auto_dtype(self):
        ...
        txt = TextIO("M 21 72.\nF 35 58.")
        expected = (np.array(["M", "F"]), np.array(
            [21, 35]), np.array([72., 58.]))
