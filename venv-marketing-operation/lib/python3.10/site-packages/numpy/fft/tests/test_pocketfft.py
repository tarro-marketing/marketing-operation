...
class TestFFT1D:
    ...
    def test_identity(self):
        ...
        for i in range(1, maxlen):
            assert_allclose(
                np.fft.ifft(np.fft.fft(x[0:i])), x[0:i], atol=1e-12
            )
            assert_allclose(
                np.fft.irfft(np.fft.rfft(xr[0:i]), i), xr[0:i], atol=1e-12
            )
    ...
    def test_fft(self):
        ...
        assert_allclose(
            fft1(x) / np.sqrt(30), np.fft.fft(x, norm="ortho"), atol=1e-6
        )
        assert_allclose(
            fft1(x) / 30.0, np.fft.fft(x, norm="forward"), atol=1e-6
        )
    ...
    def test_ifft(self, norm):
        x = random(30) + 1j * random(30)
        assert_allclose(
            x, np.fft.ifft(np.fft.fft(x, norm=norm), norm=norm), atol=1e-6
        )
    ...
    def test_fft2(self):
        ...
        assert_allclose(
            np.fft.fft(np.fft.fft(x, axis=1), axis=0),
            np.fft.fft2(x),
            atol=1e-6,
        )
        assert_allclose(
            np.fft.fft2(x), np.fft.fft2(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
            np.fft.fft2(x) / np.sqrt(30 * 20),
            np.fft.fft2(x, norm="ortho"),
            atol=1e-6,
        )
        assert_allclose(
            np.fft.fft2(x) / (30.0 * 20.0),
            np.fft.fft2(x, norm="forward"),
            atol=1e-6,
        )
    ...
    def test_ifft2(self):
        x = random((30, 20)) + 1j * random((30, 20))
        assert_allclose(
            np.fft.ifft(np.fft.ifft(x, axis=1), axis=0),
            np.fft.ifft2(x),
            atol=1e-6,
        )
        assert_allclose(
            np.fft.ifft2(x), np.fft.ifft2(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
            np.fft.ifft2(x) * np.sqrt(30 * 20),
            np.fft.ifft2(x, norm="ortho"),
            atol=1e-6,
        )
        assert_allclose(
            np.fft.ifft2(x) * (30.0 * 20.0),
            np.fft.ifft2(x, norm="forward"),
            atol=1e-6,
        )
    ...
    def test_fftn(self):
        ...
        assert_allclose(
            np.fft.fftn(x), np.fft.fftn(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
    ...
    def test_ifftn(self):
        ...
        assert_allclose(
            np.fft.ifftn(x), np.fft.ifftn(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
    ...
    def test_rfft(self):
        ...
        for n in [x.size, 2 * x.size]:
            ...
            assert_allclose(
                np.fft.rfft(x, n=n),
                np.fft.rfft(x, n=n, norm="backward"),
                atol=1e-6,
            )
            assert_allclose(
            ...
            assert_allclose(
                np.fft.rfft(x, n=n) / n,
                np.fft.rfft(x, n=n, norm="forward"),
                atol=1e-6,
            )
    ...
    def test_irfft(self):
        ...
        assert_allclose(
            x,
            np.fft.irfft(np.fft.rfft(x, norm="backward"), norm="backward"),
            atol=1e-6,
        )
        assert_allclose(
            x,
            np.fft.irfft(np.fft.rfft(x, norm="ortho"), norm="ortho"),
            atol=1e-6,
        )
        assert_allclose(
            x,
            np.fft.irfft(np.fft.rfft(x, norm="forward"), norm="forward"),
            atol=1e-6,
        )
    ...
    def test_rfft2(self):
        ...
        assert_allclose(
            np.fft.rfft2(x), np.fft.rfft2(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
        ...
        assert_allclose(
            np.fft.rfft2(x) / (30.0 * 20.0),
            np.fft.rfft2(x, norm="forward"),
            atol=1e-6,
        )
    ...
    def test_irfft2(self):
        ...
        assert_allclose(
            x,
            np.fft.irfft2(np.fft.rfft2(x, norm="ortho"), norm="ortho"),
            atol=1e-6,
        )
        assert_allclose(
            x,
            np.fft.irfft2(np.fft.rfft2(x, norm="forward"), norm="forward"),
            atol=1e-6,
        )
    ...
    def test_rfftn(self):
        ...
        assert_allclose(
            np.fft.rfftn(x), np.fft.rfftn(x, norm="backward"), atol=1e-6
        )
        assert_allclose(
    ...
    def test_irfftn(self):
        ...
        assert_allclose(
            x,
            np.fft.irfftn(np.fft.rfftn(x, norm="ortho"), norm="ortho"),
            atol=1e-6,
        )
        assert_allclose(
            x,
            np.fft.irfftn(np.fft.rfftn(x, norm="forward"), norm="forward"),
            atol=1e-6,
        )
    ...
    def test_hfft(self):
        ...
        assert_allclose(
            np.fft.hfft(x_herm),
            np.fft.hfft(x_herm, norm="backward"),
            atol=1e-6,
        )
        assert_allclose(
        ...
        assert_allclose(
            np.fft.hfft(x_herm) / 30.0,
            np.fft.hfft(x_herm, norm="forward"),
            atol=1e-6,
        )
    ...
    def test_ihfft(self):
        ...
        assert_allclose(
            x_herm,
            np.fft.ihfft(
                np.fft.hfft(x_herm, norm="backward"), norm="backward"
            ),
            atol=1e-6,
        )
        assert_allclose(
...
@pytest.mark.parametrize(
    "fft", [np.fft.fft, np.fft.ifft, np.fft.rfft, np.fft.irfft]
)
@pytest.mark.parametrize(
    "data",
    [
