...
def assert_equal(actual, desired, err_msg="", verbose=True, *, strict=False):
    ...
    if isinstance(desired, dict):
        ...
        for k, i in desired.items():
            ...
            assert_equal(
                actual[k], desired[k], f"key={k!r}\n{err_msg}", verbose
            )
        return
    if isinstance(desired, (list, tuple)) and isinstance(
        actual, (list, tuple)
    ):
        assert_equal(len(actual), len(desired), err_msg, verbose)
        for k in range(len(desired)):
            assert_equal(
                actual[k], desired[k], f"item={k!r}\n{err_msg}", verbose
            )
        return
    ...
    if isinstance(actual, ndarray) or isinstance(desired, ndarray):
        return assert_array_equal(
            actual, desired, err_msg, verbose, strict=strict
        )
...
def assert_almost_equal(actual, desired, decimal=7, err_msg="", verbose=True):
    ...
    def _build_err_msg():
        ...
        return build_err_msg(
            [actual, desired], err_msg, verbose=verbose, header=header
        )
    ...
    try:
        ...
        if isnumber(x) and isnumber(y):
            ...
            if equal_nan and dt == y.dtype and hasattr(dt, "na_object"):
                is_nan = isinstance(dt.na_object, float) and np.isnan(
                    dt.na_object
                )
                bool_errors = 0
                try:
...
def assert_allclose(
    ...
    *,
    ...
    def compare(x, y):
        return np._core.numeric.isclose(
...
def _parse_size(size_str):
    ...
    size_re = re.compile(
        r"^\s*(\d+|\d+\.\d+)\s*({0})\s*$".format("|".join(suffixes.keys())),
        re.I,
    )
