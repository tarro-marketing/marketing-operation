...
def infer_anonymous_param(func):
    def get_returns(value):
        if value.tree_node.annotation is not None:
            ...
            if any(
                v.name.get_qualified_names(include_module_names=True)
                == ("typing", "Generator")
                for v in result
            ):
...
def _is_a_pytest_param_and_inherited(param_name):
    ...
    return (
        _is_pytest_func(funcdef.name.value, decorators),
...
def _find_pytest_plugin_modules() -> List[List[str]]:
    ...
    if sys.version_info >= (3, 8):
        ...
        if sys.version_info >= (3, 9):
            ...
            matches = [
                ep.pattern.match(ep.value) for ep in pytest_entry_points
            ]
        ...
        return [
            ep.module_name.split(".")
            for ep in iter_entry_points(group="pytest11")
        ]
            ...
            if Path(file_io.path) != module_context.py__file__():
                try:
                    m = load_module_from_path(
                        module_context.inference_state, file_io
                    )
                    yield m.as_context()
