...
def argument_clinic(
    ...
    want_value=False,
    want_context=False,
    want_arguments=False,
    want_inference_state=False,
    want_callback=False,
):
    """
...
def builtins_next(iterators, defaults, inference_state):
    ...
    return (
        defaults
        | iterators.py__getattribute__("__next__").execute_with_values()
    )
    ...
    def py__get__(self, instance, class_value):
        return ValueSet(
            [
                ...
                for __get__ in self._wrapped_value.py__getattribute__(
                    "__get__"
                )
            ]
        )
...
class ClassMethodGet(ValueWrapper):
    ...
    def get_signatures(self):
        return [
            sig.bind(self._function) for sig in self._function.get_signatures()
        ]
...
class PartialObject(ValueWrapper):
    ...
    def _get_functions(self, unpacked_arguments):
        ...
        if key is not None or lazy_value is None:
            debug.warning(
                "Partial should have a proper function %s", self._arguments
            )
            return None
        return lazy_value.infer()
...
class PartialSignature(SignatureWrapper):
    ...
    def get_param_names(self, resolve_stars=False):
        names = self._wrapped_signature.get_param_names()[
            self._skipped_arg_count :
        ]
    ...
    def get_signatures(self):
        ...
        for cls in reversed(list(self.py__mro__())):
            if isinstance(cls, DataclassWrapper):
                ...
                for name in sorted(
                    filter_.values(), key=lambda name: name.start_pos
                ):
...
class DataclassParamName(BaseTreeParamName):
    def __init__(
        self, parent_context, tree_name, annotation_node, default_node
    ):
    ...
    def py__call__(self, item_value_set):
        ...
        for args_value in self._args_value_set:
            ...
            if len(lazy_values) == 1:
                ...
                value_set |= item_value_set.get_item(
                    lazy_values[0].infer(), None
                )
            else:
                value_set |= ValueSet(
                    [
            ...
            return ValueSet(
                [
                    compiled.create_simple_object(
                        sequence.inference_state, string
                    )
                ]
            )
    return callback()
...
class EnumInstance(LazyValueWrapper):
    ...
    def _get_wrapped_value(self):
        ...
        if (
            n.startswith("__")
            and n.endswith("__")
            or self._name.api_type == "function"
        ):
