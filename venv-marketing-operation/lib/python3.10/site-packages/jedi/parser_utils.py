...
def get_signature(
    ...
    width=72,
    call_string=None,
    omit_first_param=False,
    omit_return_annotation=False,
):
    """
...
def get_following_comment_same_line(node):
    ...
    try:
        if node.type == "for_stmt":
            ...
            whitespace = (
                node.children[4].get_first_leaf().get_next_leaf().prefix
            )
        else:
        ...
        if is_scope(scope):
            if scope.type in ("classdef", "funcdef", "lambdef"):
                ...
                if scope.children[index].start_pos >= node.start_pos:
                    if (
                        node.parent.type == "param"
                        and node.parent.name == node
                    ):
                        pass
                    elif (
...
def cut_value_at_position(leaf, position):
    ...
    """
    lines = split_lines(leaf.value, keepends=True)[
        : position[0] - leaf.line + 1
    ]
    column = position[1]
