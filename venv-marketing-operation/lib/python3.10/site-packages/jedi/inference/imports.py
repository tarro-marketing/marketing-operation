    ...
    def _sys_path_with_modifications(self, is_completion):
        ...
        return (
            ...
            self._inference_state.get_sys_path(
                add_init_paths=not is_completion
            )
            + [
                str(p)
                for p in sys_path.check_sys_path_modifications(
    ...
    def follow(self):
        if not self.import_path:
            if self._fixed_sys_path:
                ...
                from jedi.inference.value.namespace import (
...
@import_module_decorator
def import_module(
    inference_state, import_names, parent_module_value, sys_path
):
    """
    ...
    if parent_module_value is None:
        ...
        debug.dbg(
            "search_module %s in paths %s: %s", module_name, paths, module
        )
    return ValueSet([module])


def _load_python_module(
    inference_state, file_io, import_names=None, is_package=False
):
    ...
    return ModuleValue(
        ...
        code_lines=get_cached_code_lines(
            inference_state.grammar, file_io.path
        ),
...
def load_module_from_path(
    inference_state, file_io, import_names=None, is_package=None
):
    """
...
def iter_module_names(
    ...
    if add_builtin_modules:
        for (
            name
        ) in inference_state.compiled_subprocess.get_builtin_module_names():
            yield module_cls(module_context, name)

    for name in inference_state.compiled_subprocess.iter_module_names(
        search_path
    ):
        yield module_cls(module_context, name)
