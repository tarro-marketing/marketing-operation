...
from jedi.inference.value.dynamic_arrays import (
    ListModification,
    DictModification,
)
...
def _infer_node(context, element):
    ...
    if typ in (
        ...
        for i, trailer in enumerate(children):
            if trailer == "**":  # has a power operation.
                ...
                value_set = _infer_comparison(
                    context, value_set, trailer, right
                )
                break
        ...
        return context.infer_node(element.children[0]) | context.infer_node(
            element.children[-1]
        )
    elif typ == "operator":
        ...
        for next_name in element.children[2::2]:
            value_set = value_set.py__getattribute__(
                next_name, name_context=context
            )
        return value_set
    elif typ == "eval_input":
        return context.infer_node(element.children[0])
    elif typ == "annassign":
        return annotation.infer_annotation(
            context, element.children[1]
        ).execute_annotation()
    elif typ == "yield_expr":
    ...
    if trailer_op == "[":
        ...
        if trailer_op == ".":
            return atom_values.py__getattribute__(
                name_context=context, name_or_str=node
            )
        else:
            assert trailer_op == "(", "trailer_op is actually %s" % trailer_op
            args = arguments.TreeArguments(
...
def infer_atom(context, atom):
    ...
    if atom.type == "name":
        ...
        stmt = (
            tree.search_ancestor(atom, "expr_stmt", "lambdef", "if_stmt")
            or atom
        )
        if stmt.type == "if_stmt":
            if not any(
                n.start_pos <= atom.start_pos < n.end_pos
        ...
        if (
            c[0] == "("
            and not len(c) == 2
            and not (c[1].type == "testlist_comp" and len(c[1].children) > 1)
        ):
        ...
        try:
            ...
            array_node_c = []
        if c[0] == "{" and (
            array_node == "}" or ":" in array_node_c or "**" in array_node_c
        ):
    ...
    with recursion.execution_allowed(context.inference_state, stmt) as allowed:
        if allowed:
            if seek_name is not None:
                pep0484_values = annotation.find_type_from_comment_hint_assign(
    ...
    if is_annassign or is_setitem:
        ...
        if is_setitem:
            ...
            if (
                ...
                value_set = _infer_comparison(
                    context, left_values, operator, value_set
                )
...
def infer_or_test(context, or_test):
    ...
    for operator in iterator:
        ...
        if operator in ("and", "or"):
            ...
            types = _infer_comparison(
                context, types, operator, context.infer_node(right)
            )
...
def _infer_comparison_part(inference_state, context, left, operator, right):
    ...
    def check(obj):
        ...
        return isinstance(obj, TreeInstance) and obj.name.string_name in (
            "int",
            "float",
        )
    ...
    if module_node is not None:
        ...
        for name in names:
            ...
            if (
                ...
                and expr_stmt.children[1].type == "annassign"
            ):
                correct_scope = (
                    parser_utils.get_parent_scope(name) == context.tree_node
                )
                if correct_scope:
...
def _apply_decorators(context, node):
    ...
    for dec in reversed(node.get_decorators()):
        ...
        with debug.increase_indent_cm():
            ...
            if not len(values):
                debug.warning(
                    "not possible to resolve wrappers found %s", node
                )
                return initial
...
def _infer_subscript_list(context, index):
    ...
    if index == ":":
        ...
        return ValueSet(
            [
                iterable.SequenceLiteralValue(
                    context.inference_state, context, index
                )
            ]
        )
