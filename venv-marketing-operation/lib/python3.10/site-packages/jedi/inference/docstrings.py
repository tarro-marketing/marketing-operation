...
def _search_param_in_numpydocstr(docstr, param_str):
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            params = _get_numpy_doc_string_cls()(docstr)._parsed_data[
                "Parameters"
            ]
        except Exception:
            return []
...
def _execute_array_values(inference_state, array):
    ...
    """
    from jedi.inference.value.iterable import (
...
def infer_param(function_value, param):
    def infer_docstring(docstring):
        return ValueSet(
            p
            for param_str in _search_param_in_docstr(
                docstring, param.name.value
            )
    ...
    if (
        function_value.is_bound_method()
