...
class DirectObjectAccess:
    ...
    def py__getitem__all_values(self):
        ...
        if self.is_instance():
            cls = DirectObjectAccess(
                self._inference_state, self._obj.__class__
            )
        ...
        try:
            ...
            annotation = DirectObjectAccess(
                self._inference_state, getitem
    ...
    def py__iter__list(self):
        try:
            ...
            p = DirectObjectAccess(
                self._inference_state, iter_method
            ).get_return_annotation()
            if p is not None:
                return [p]
    ...
    def is_allowed_getattr(
        self, name, safe=True
    ) -> Tuple[bool, bool, Optional[AccessPath]]:
        ...
        try:
            ...
            if (
                is_get_descriptor
                and type(attr) not in ALLOWED_DESCRIPTOR_ACCESS
            ):
                if isinstance(attr, property):
                    if hasattr(attr.fget, "__annotations__"):
                        a = DirectObjectAccess(
                            self._inference_state, attr.fget
                        )
