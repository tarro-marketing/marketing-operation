...
class CompiledSubprocess:
    ...
    def _get_process(self):
        ...
        self._cleanup_callable = weakref.finalize(
            self, _cleanup_process, process, t
        )
        return process
    ...
    def _send(self, inference_state_id, function, args=(), kwargs={}):
        if self.is_crashed:
            raise InternalError(
                "The subprocess %s has crashed." % self._executable
            )
        ...
        if inference_state_id is None:
            ...
            for i, arg in enumerate(args):
                if isinstance(arg, AccessHandle):
                    args[i] = (
                        inference_state.compiled_subprocess.get_access_handle(
                            arg.id
                        )
                    )
