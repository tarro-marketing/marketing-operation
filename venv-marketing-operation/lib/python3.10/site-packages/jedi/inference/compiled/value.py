    ...
    def execute_annotation(self):
        ...
        if name == "Union":
            return ValueSet.from_sets(
                arg.execute_annotation() for arg in arguments
            )
        elif name:
            ...
            return ValueSet(
                [
                    ...
                    for v in self.inference_state.typing_module.py__getattribute__(
                        name
                    )
                ]
            ).execute_annotation()
...
class CompiledValueFilter(AbstractFilter):
    ...
    def _get(
        ...
        """
        has_attribute, is_descriptor, property_return_annotation = (
            allowed_getattr_callback(
                name,
            )
        )
    ...
    def values(self):
        ...
        names = []
        needs_type_completions, dir_infos = (
...
def create_from_name(inference_state, compiled_value, name):
    access_paths = compiled_value.access_handle.getattr_paths(
        name, default=None
    )
...
def create_cached_compiled_value(
    inference_state, access_handle, parent_context
):
