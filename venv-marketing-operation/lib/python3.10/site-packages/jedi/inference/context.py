...
class AbstractContext:
    ...
    def _check_for_additional_knowledge(
        self, name_or_str, name_context, position
    ):
        ...
        if isinstance(name_or_str, Name) and not name_context.is_instance():
            ...
            if any(
                b.type in ("comp_for", "sync_comp_for") for b in base_nodes
            ):
                return NO_VALUES
...
class TreeContextMixin:
    ...
    def create_context(self, node):
        def from_scope_node(scope_node, is_nested=True):
            ...
            if scope_node.type in ("funcdef", "lambdef", "classdef"):
                ...
                parent_context = from_scope_node(
                    parent_scope(scope_node.parent)
                )
                if node.start_pos >= scope_node.children[-1].start_pos:
                    return parent_context
                return CompForContext(parent_context, scope_node)
            raise Exception(
                "There's a scope that was not managed: %s" % scope_node
            )
    ...
    def create_name(self, tree_name):
        ...
        if (
            definition
            and definition.type == "param"
            and definition.name == tree_name
        ):
...
def _get_global_filters_for_name(context, name_or_none, position):
    ...
    if name_or_none is not None:
        ancestor = search_ancestor(
            name_or_none, "funcdef", "classdef", "lambdef"
        )
        lambdef = None
        if ancestor == "lambdef":
        ...
        if ancestor is not None:
            ...
            if position is not None and position < colon.start_pos:
                if (
                    lambdef is None
                    or position < lambdef.children[-2].start_pos
                ):
