...
class ProxyWithGenerics(BaseTypingClassWithGenerics):
    def execute_annotation(self):
        ...
        if string_name == "Union":
            ...
            # type.
            return (
                self.gather_annotation_classes().execute_annotation()
    ...
    def _create_instance_with_generics(self, generics_manager):
        return ProxyWithGenerics(
            self.parent_context, self._tree_name, generics_manager
        )
...
class TypeAlias(LazyValueWrapper):
    ...
    def _get_wrapped_value(self):
        ...
        (module,) = Importer(
            self.inference_state,
            [module_name],
            self.inference_state.builtins_module,
        ).follow()
    ...
    def py__call__(self, arguments):
        ...
        try:
            ...
            from jedi.inference.gradual.annotation import (
                infer_return_for_callable,
            )

            return infer_return_for_callable(
                arguments, param_values, result_values
            )
...
class Tuple(BaseTypingInstance):
    ...
    def _get_wrapped_value(self):
        (tuple_,) = self.inference_state.builtins_module.py__getattribute__(
            "tuple"
        ).execute_annotation()
        return tuple_
...
class NewType(Value):
    def __init__(
        self, inference_state, parent_context, tree_node, type_value_set
    ):
...
class TypedDict(LazyValueWrapper):
    ...
    def py__simple_getitem__(self, index):
        if isinstance(index, str):
            return ValueSet.from_sets(
                name.infer()
                for filter in self._definition_class.get_filters(
                    is_instance=True
                )
