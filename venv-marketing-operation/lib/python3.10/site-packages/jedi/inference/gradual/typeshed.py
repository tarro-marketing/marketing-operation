...
from jedi.inference.gradual.stub_value import (
    TypingModuleWrapper,
    StubModuleValue,
)
        ...
        for entry in listed:
            ...
            if os.path.isdir(path):
                ...
                if os.path.isfile(init):
                    yield entry, PathInfo(
                        init, directory_path_info.is_third_party
                    )
            elif entry.endswith(".pyi") and os.path.isfile(path):
                name = entry[:-4]
                if name != "__init__":
                    yield name, PathInfo(
                        path, directory_path_info.is_third_party
                    )
...
def _get_typeshed_directories(version_info):
    ...
    for base in ["stdlib", "third_party"]:
        ...
        for check_version in check_version_list:
            ...
            yield PathInfo(
                str(base_path.joinpath(check_version)), is_third_party
            )
...
def import_module_decorator(func):
    ...
    def wrapper(
        ...
        if python_value_set is None:
            if (
                parent_module_value is not None
                and parent_module_value.is_stub()
            ):
...
def _load_from_typeshed(
    ...
    if map_ is not None:
        ...
        if path_info is not None and (
            not path_info.is_third_party or python_value_set
        ):
            return _try_to_load_stub_from_file(
                inference_state,
                python_value_set,
