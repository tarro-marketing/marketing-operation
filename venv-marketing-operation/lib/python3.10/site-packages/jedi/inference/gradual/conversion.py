...
def _stub_to_python_value_set(stub_value, ignore_compiled=False):
    ...
    values = _infer_from_stub(
        stub_module_context, qualified_names, ignore_compiled
    )
    if was_instance:
...
def _infer_from_stub(stub_module_context, qualified_names, ignore_compiled):
    ...
    assert isinstance(
        stub_module, (StubModuleValue, MixedObject)
    ), stub_module_context
...
def convert_names(
    ...
    with debug.increase_indent_cm("convert names"):
        if only_stubs or prefer_stubs:
            return _python_to_stub_names(
                names, fallback_to_python=prefer_stubs
            )
        else:
...
def convert_values(
    values, only_stubs=False, prefer_stubs=False, ignore_compiled=True
):
    ...
    with debug.increase_indent_cm("convert values"):
        if only_stubs or prefer_stubs:
            ...
            return ValueSet.from_sets(
                _stub_to_python_value_set(
                    stub_value, ignore_compiled=ignore_compiled
                )
