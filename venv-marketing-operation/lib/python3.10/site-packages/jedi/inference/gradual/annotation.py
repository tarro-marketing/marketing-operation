...
def infer_annotation(context, annotation):
    ...
    if is_string(inferred_value):
        result = _get_forward_reference_node(
            context, inferred_value.get_safe_value()
        )
...
def infer_return_types(function, arguments):
    ...
    type_var_dict = infer_type_vars_for_execution(
        function, arguments, all_annotations
    )
...
def _infer_type_vars_for_callable(arguments, lazy_params):
    ...
    """
    annotation_variable_results = {}
    for (_, lazy_value), lazy_callable_param in zip(
        arguments.unpack(), lazy_params
    ):
