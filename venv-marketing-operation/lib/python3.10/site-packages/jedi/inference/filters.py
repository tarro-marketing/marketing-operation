        ...
        return self._convert_names(
            self._filter(
                _get_definition_names(
                    self._parso_cache_node, self._used_names, name
                ),
            )
        )
    ...
    def values(self):
        return self._convert_names(
            ...
            for name in self._filter(
                _get_definition_names(
                    self._parso_cache_node, self._used_names, name_key
                ),
            )
        )
...
class ParserTreeFilter(_AbstractUsedNamesFilter):
    ...
    def _is_name_reachable(self, name):
        ...
        return (
            get_cached_parent_scope(self._parso_cache_node, base_node)
            == self._parser_scope
        )

    def _check_flows(self, names):
        for name in sorted(
            names, key=lambda name: name.start_pos, reverse=True
        ):
...
class _FunctionExecutionFilter(ParserTreeFilter):
    def __init__(
        self, parent_context, function_value, until_position, origin_scope
    ):
        super().__init__(
            parent_context,
...
class SpecialMethodFilter(DictFilter):
    ...
    class SpecialMethodName(AbstractNameDefinition):
        ...
        def __init__(
            self, parent_context, string_name, callable_, builtin_value
        ):
            self.parent_context = parent_context
    ...
    def _convert(self, name, value):
        return self.SpecialMethodName(
            self.value, name, value, self._builtin_value
        )
...
class _AttributeOverwriteMixin:
    def get_filters(self, *args, **kwargs):
        yield SpecialMethodFilter(
            self, self.overwritten_methods, self._wrapped_value
        )
