    ...
    for name in names:
        ...
        try:
            ...
            for global_name in method().get(search_name):
                ...
                yield from _add_names_in_same_context(
                    c, global_name.string_name
                )
    ...
    for module_context in potential_modules:
        for name_leaf in module_context.tree_node.get_used_names().get(
            ...
            if any(tree_name in found_names_dct for tree_name in new):
                ...
                for name in new:
                    non_matching_reference_maps.setdefault(name, []).append(
                        new
                    )
...
def expand_relative_ignore_paths(folder_io, relative_paths):
    ...
    return {
        os.path.join(curr_path, p[1])
        for p in relative_paths
        if curr_path.startswith(p[0])
    }
...
def _find_python_files_in_sys_path(inference_state, module_contexts):
    ...
    for module_context in module_contexts:
        ...
        while True:
            path = folder_io.path
            if (
                not any(path.startswith(p) for p in sys_path)
                or path in except_paths
            ):
                break
...
def _find_project_modules(inference_state, module_contexts):
    ...
    yield from recurse_find_python_files(
        FolderIO(inference_state.project.path), except_
    )
...
def search_in_file_ios(
    inference_state, file_io_iterator, name, limit_reduction=1, complete=False
):
