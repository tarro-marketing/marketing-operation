...
def _check_for_exception_catch(
    node_context, jedi_name, exception, payload=None
):
    """
        ...
        for node in obj.get_except_clause_tests():
            if node is None:
                ...
                for cls in except_classes:
                    ...
                    if (
                        isinstance(cls, iterable.Sequence)
                        and cls.array_type == "tuple"
                    ):
                        # multiple exceptions
    ...
    def check_hasattr(node, suite):
        try:
            ...
            args = TreeArguments(
                node_context.inference_state, node_context, arglist
            )
    ...
    while obj is not None and not isinstance(obj, (tree.Function, tree.Class)):
        if isinstance(obj, tree.Flow):
            ...
            if obj.type == "try_stmt" and check_try_for_except(obj, exception):
                return True
            # hasattr check
            if exception == AttributeError and obj.type in (
                "if_stmt",
                "while_stmt",
            ):
