...
def get_executed_param_names_and_issues(function_value, arguments):
            ...
            if argument is not None:
                ...
                for key, argument in var_arg_iterator:
                    ...
                    if key:
                        ...
                        break
                    lazy_value_list.append(argument)
            seq = iterable.FakeTuple(
                function_value.inference_state, lazy_value_list
            )
            ...
            if argument is None:
                ...
                if param.default is None:
                    ...
                    if not keys_only:
                        for (
                            contextualized_node
                        ) in arguments.get_calling_nodes():
                            m = _error_argument_count(
                                funcdef, len(unpacked_va)
                            )
                            issues.append(
                                analysis.add(
                                    contextualized_node.context,
                    ...
                    result_arg = LazyTreeValue(
                        default_param_context, param.default
                    )
                    is_default = True
            else:
                result_arg = argument
    ...
    for key, lazy_value in non_matching_keys.items():
        ...
        issues.append(
            _add_argument_issue(
                "type-error-keyword-argument", lazy_value, message=m
            )
        )
