...
def _paths_from_assignment(module_context, expr_stmt):
    ...
    """
    for assignee, operator in zip(
        expr_stmt.children[::2], expr_stmt.children[1::2]
    ):
        try:
            assert operator in ["=", "+="]
            assert (
                assignee.type in ("power", "atom_expr")
            ...
            assert (
                trailer.children[0] == "."
                and trailer.children[1].value == "path"
            )
...
def _paths_from_list_modifications(module_context, trailer1, trailer2):
    ...
    if name == "insert" and len(arg.children) in (
        3,
        4,
...
def _get_paths_from_buildout_script(inference_state, buildout_script_path):
    ...
    try:
        ...
        debug.warning(
            "Error trying to read buildout_script: %s", buildout_script_path
        )
        return
