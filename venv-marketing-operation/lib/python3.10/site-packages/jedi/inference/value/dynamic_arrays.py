...
def _internal_check_array_additions(context, sequence):
    ...
    def find_additions(context, arglist, add_name):
        params = list(
            arguments.TreeArguments(
                context.inference_state, context, arglist
            ).unpack()
        )
        result = set()
    ...
    temp_param_add, settings.dynamic_params_for_other_modules = (
        settings.dynamic_params_for_other_modules,
        False,
    )

    is_list = sequence.name.string_name == "list"
    search_names = (
        ["append", "extend", "insert"] if is_list else ["add", "update"]
    )
    ...
    for add_name in search_names:
        try:
            ...
            for name in possible_names:
                ...
                if not (
                    value_node.start_pos < name.start_pos < value_node.end_pos
                ):
                    continue
                trailer = name.parent
...
class _Modification(ValueWrapper):
    ...
    def py__getitem__(self, *args, **kwargs):
        return (
            self._wrapped_value.py__getitem__(*args, **kwargs)
...
class DictModification(_Modification):
    ...
    def get_key_values(self):
        return (
            self._wrapped_value.get_key_values()
