...
class FunctionValue(
    FunctionMixin, FunctionAndClassBase, metaclass=CachedMetaClass
):
    @classmethod
...
class BaseFunctionExecutionContext(ValueContext, TreeContextMixin):
        ...
        for_parents = [
            (
                ...
                tree.search_ancestor(
                    y, "for_stmt", "funcdef", "while_stmt", "if_stmt"
                ),
            )
                ...
                for lazy_value in ordered:
                    ...
                    with self.predefine_names(for_stmt, dct):
                        for yield_in_same_for_stmt in yields:
                            yield from self._get_yield_lazy_value(
    ...
    def infer(self):
        ...
        is_coroutine = self.tree_node.parent.type in (
            "async_stmt",
            "async_funcdef",
        )
        ...
        if is_coroutine:
            if self.is_generator():
                async_generator_classes = (
                    inference_state.typing_module.py__getattribute__(
                        "AsyncGenerator"
                    )
                )
...
class OverloadedFunctionValue(FunctionMixin, ValueWrapper):
    ...
    def py__call__(self, arguments):
        debug.dbg(
            "Execute overloaded function %s", self._wrapped_value, color="BLUE"
        )
        function_executions = []
    ...
    def _is_overload_decorated(funcdef):
        if funcdef.parent.type == "decorated":
            ...
            for decorator in decorators:
                ...
                if (
                    dotted_name.type == "name"
                    and dotted_name.value == "overload"
                ):
