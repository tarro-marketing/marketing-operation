...
class IterableMixin:
    ...
    def py__stop_iteration_returns(self):
        return ValueSet(
            [compiled.builtin_from_name(self.inference_state, "None")]
        )
...
class GeneratorBase(LazyAttributeOverwrite, IterableMixin):
    ...
    def _get_cls(self):
        (generator,) = self.inference_state.typing_module.py__getattribute__(
            "Generator"
        )
        return generator
    ...
    def _next(self, arguments):
        return ValueSet.from_sets(
            lazy_value.infer() for lazy_value in self.py__iter__()
        )
...
class _DictMixin:
    def _get_generics(self):
        return tuple(
            c_set.py__class__() for c_set in self.get_mapping_item_values()
        )
...
class Sequence(LazyAttributeOverwrite, IterableMixin):
    ...
    def _get_wrapped_value(self):
        ...
        klass = compiled.builtin_from_name(
            self.inference_state, self.array_type
        )
        (c,) = GenericClass(
            klass, TupleGenericManager(self._cached_generics())
        ).execute_annotation()
        return c
...
class _BaseComprehension(ComprehensionMixin):
    def __init__(
        self, inference_state, defining_context, sync_comp_for_node, entry_node
    ):
        ...
        lazy_values = [
            LazyKnownValue(
                FakeTuple(
                    ...
                    [LazyKnownValues(key), LazyKnownValues(value)],
                )
            )
...
class SequenceLiteralValue(Sequence):
    ...
    def get_tree_entries(self):
        ...
        if array_node.type == "testlist_comp":
            ...
            return [
                value
                for value in array_node.children[::2]
...
class DictLiteralValue(_DictMixin, SequenceLiteralValue, _DictKeyMixin):
    ...
    def py__simple_getitem__(self, index):
        """Here the index is an int/str. Raises IndexError/KeyError."""
        compiled_value_index = compiled.create_simple_object(
            self.inference_state, index
        )
    ...
    def exact_key_items(self):
        ...
        for key_node, value in self.get_tree_entries():
            for key in self._defining_context.infer_node(key_node):
                if is_string(key):
                    yield key.get_safe_value(), LazyTreeValue(
                        self._defining_context, value
                    )
...
class FakeDict(_DictMixin, Sequence, _DictKeyMixin):
    ...
    def _dict_values(self):
        return ValueSet.from_sets(
            lazy_value.infer() for lazy_value in self._dct.values()
        )
        ...
        for lazy_value in types.iterate(ContextualizedNode(context, exprlist)):
            ...
            try:
                ...
                analysis.add(
                    ...
                    part,
                    message="ValueError: too many values to unpack (expected %s)"
                    % n,
                )
            else:
                dct.update(
                    unpack_tuple_to_dict(context, lazy_value.infer(), part)
                )
...
class Slice(LazyValueWrapper):
    ...
    def _get_wrapped_value(self):
        value = compiled.builtin_from_name(
            self._context.inference_state, "slice"
        )
