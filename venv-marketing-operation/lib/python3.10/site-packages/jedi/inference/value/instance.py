...
class CompiledInstance(AbstractInstanceValue):
    ...
    # compiled class.
    def __init__(
        self, inference_state, parent_context, class_value, arguments
    ):
    ...
    @property
    def name(self):
        return compiled.CompiledValueName(
            self, self.class_value.name.string_name
        )
...
class _BaseTreeInstance(AbstractInstanceValue):
    ...
    def py__call__(self, arguments):
        ...
        return ValueSet.from_sets(
            name.infer().execute(arguments) for name in names
        )
    ...
    def py__get__(self, instance, class_value):
        ...
        if names:
            if instance is None:
                instance = compiled.builtin_from_name(
    ...
    def _get_annotated_class_object(self):
        ...
        for signature in self.class_value.py__getattribute__(
            ...
            if type_var_dict:
                ...
                debug.dbg(
                    "Inferred instance value as %s", defined, color="BLUE"
                )
                return defined
        return None
...
class BoundMethod(FunctionMixin, ValueWrapper):
    ...
    def py__class__(self):
        (c,) = values_from_qualified_names(
            self.inference_state, "types", "MethodType"
        )
        return c
    ...
    def py__call__(self, arguments):
        if isinstance(self._wrapped_value, OverloadedFunctionValue):
            return self._wrapped_value.py__call__(
...
class SelfAttributeFilter(ClassFilter):
        ...
        for name in names:
            ...
            if (
                ...
                if name.is_definition() and self._access_possible(name):
                    ...
                    #      filter.
                    if self._is_in_right_scope(
                        trailer.parent.children[0], name
                    ):
                        yield name
    ...
    def _convert_names(self, names):
        return [
            SelfName(self._instance, self._node_context, name)
            for name in names
        ]
