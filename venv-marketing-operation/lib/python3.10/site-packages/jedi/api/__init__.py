...
class Script:
    ...
    def __init__(
        self, code=None, *, path=None, environment=None, project=None
    ):
    ...
    @validate_line_column
    def infer(
        self, line=None, column=None, *, only_stubs=False, prefer_stubs=False
    ):
        self._inference_state.reset_recursion_limitations()
        """
    ...
    def goto(
        ...
        *,
        ...
        if follow_imports:
            names = helpers.filter_follow_imports(
                names, follow_builtin_imports
            )
        names = convert_names(
            names,
            only_stubs=only_stubs,
    ...
    @to_list
    def _search_func(
        self, string, all_scopes=False, complete=False, fuzzy=False
    ):
    ...
    def help(self, line=None, column=None):
        ...
        if (
            leaf is not None
            and leaf.end_pos == (line, column)
            and leaf.type == "newline"
        ):
            next_ = leaf.get_next_leaf()
            if next_ is not None and next_.start_pos == leaf.end_pos:
                leaf = next_

        if leaf is not None and leaf.type in (
            "keyword",
            "operator",
            "error_leaf",
        ):
    ...
    def get_signatures(self, line=None, column=None):
        ...
        return [
            classes.Signature(self._inference_state, signature, call_details)
        ...
        try:
            for node in get_executable_nodes(self._module_node):
                ...
                if node.type in ("funcdef", "classdef"):
                    ...
                    if node.is_nested():
                        import_names |= set(
                            path[-1] for path in node.get_paths()
                        )
            ...
            ana = [
                a
                for a in self._inference_state.analysis
                if self.path == a.path
            ]
    ...
    def get_syntax_errors(self):
        ...
        """
        return parso_to_jedi_errors(
            self._inference_state.grammar, self._module_node
        )
...
class Interpreter(Script):
    ...
    def __init__(self, code, namespaces, *, project=None, **kwds):
        ...
        super().__init__(
            code, environment=environment, project=project, **kwds
        )
