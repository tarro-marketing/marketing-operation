...
def _find_nodes(module_node, pos, until_pos):
    ...
    if until_pos is None:
        ...
        # Find the end
        end_leaf = module_node.get_leaf_for_position(
            until_pos, include_prefixes=True
        )
...
def _expression_nodes_to_string(nodes):
    return "".join(
        n.get_code(include_prefix=i != 0) for i, n in enumerate(nodes)
    )
...
def _remove_unwanted_expression_nodes(parent_node, pos, until_pos):
    ...
    if typ in EXPRESSION_PARTS or is_suite_part:
        ...
        if not is_suite_part:
            nodes[0:1] = _remove_unwanted_expression_nodes(
                nodes[0], pos, until_pos
            )
            nodes[-1:] = _remove_unwanted_expression_nodes(
                nodes[-1], pos, until_pos
            )
        return nodes
    return [parent_node]
...
def extract_function(
    inference_state, path, module_context, name, pos, until_pos
):
...
def _find_needed_output_variables(
    context, search_node, at_least_pos, return_variables
):
    """
