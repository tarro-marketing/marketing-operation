...
def _get_signature_param_names(signatures, positional_count, used_kwargs):
    ...
    for call_sig in signatures:
        for i, p in enumerate(call_sig.params):
            ...
            if (
                i < positional_count
                and kind == Parameter.POSITIONAL_OR_KEYWORD
            ):
                continue
            if (
                kind
...
class Completion:
    ...
    def _complete_python(self, leaf):
        ...
        allowed_transitions = list(
            stack._allowed_transition_names_and_token_types()
        )
        ...
        if any(
            ...
            if nodes and nodes[-1] in ("as", "def", "class"):
                ...
                only_modules = not (
                    "import_from" in nonterminals and "import" in nodes
                )
                ...
                nonterminals[-1] in ("trailer", "dotted_name")
                and nodes[-1] == "."
            ):
                ...
                if nodes[-1] in ["(", ","] and nonterminals[-1] in (
                    ...
                    if signatures:
                        ...
                        positional_count = (
                            call_details.count_positional_arguments()
                        )
        ...
        if not kwargs_only:
            current_line = self._code_lines[self._position[0] - 1][
                : self._position[1]
            ]
    ...
    def _is_parameter_completion(self):
        ...
        return (
            tos.nonterminal in ("typedargslist", "varargslist")
            and tos.nodes[-1] == ","
        )
    ...
    def _complete_params(self, leaf):
        ...
        if stack_node.nonterminal == "funcdef":
            ...
            if node is not None:
                ...
                return complete_param_names(
                    context, function_name.value, decorators
                )
        return []
    ...
    def _complete_global_scope(self):
        ...
        flow_scope_node = get_flow_scope_node(
            self._module_node, self._position
        )
    ...
    def _get_importer_names(self, names, level=0, only_modules=True):
        ...
        i = imports.Importer(
            self._inference_state, names, self._module_context, level
        )
        return i.completion_names(
            self._inference_state, only_modules=only_modules
        )
...
def _extract_string_while_in_string(leaf, position):
    def return_part_of_leaf(leaf):
        ...
        start = match.group(0)
        if (
            leaf.line == position[0]
            and position[1] < leaf.column + match.end()
        ):
            return None, None, None
        return (
            cut_value_at_position(leaf, position)[match.end() :],
            leaf,
            start,
        )
    ...
    while leaf is not None:
        if leaf.type == "error_leaf" and (
            '"' in leaf.value or "'" in leaf.value
        ):
            if len(leaf.value) > 1:
                return return_part_of_leaf(leaf)
            prefix_leaf = None
        ...
        for n in names:
            if s == n.string_name:
                if (
                    ...
                    and n.api_type in ("module", "namespace")
                    and ignore_imports
                ):
                    continue
