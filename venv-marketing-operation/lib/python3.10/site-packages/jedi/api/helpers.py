...
def _get_code_for_stack(code_lines, leaf, position):
    ...
    if leaf.type == "error_leaf" or leaf.type == "string":
        ...
        return _get_code(
            code_lines, user_stmt.get_start_pos_of_prefix(), position
        )
    ...
    def calculate_index(self, param_names):
        ...
        for i, param_name in enumerate(param_names):
            ...
            if (
                ...
                and not star_count == 1
                or star_count == 2
            ):
                if param_name.string_name not in used_names and (
    ...
    for i, node in enumerate(nodes_before):
        if node.type == "argument":
            ...
            if second == "=":
                ...
                if (
                    first_leaf.type == "name"
                    and first_leaf.start_pos >= position
                ):
...
def _get_signature_details_from_error_node(
    node, additional_children, position
):
    for index, element in reversed(list(enumerate(node.children))):
        ...
        if element == "(" and element.end_pos <= position and index > 0:
            ...
            if name.type == "name" or name.parent.type in ("trailer", "atom"):
                return CallDetails(
                    element, children + additional_children, position
                )
...
def cache_signatures(
    inference_state, context, bracket_leaf, code_lines, user_pos
):
...
def validate_line_column(func):
    ...
    def wrapper(self, line=None, column=None, *args, **kwargs):
        ...
        if not (0 <= column <= line_len):
            raise ValueError(
                "`column` parameter (%d) is not in a valid range "
                "(0-%d) for line %d (%r)."
