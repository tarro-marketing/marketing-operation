...
def mangled_name(node: EnhancedAST) -> str:
    ...
    if name.startswith("__") and not name.endswith("__"):
        ...
        while not (
            isinstance(parent, ast.ClassDef) and child not in parent.bases
        ):
...
class PositionNodeFinder(object):
    ...
    def known_issues(
        ...
        if instruction.opname in (
            ...
            "IS_OP",
            "CONTAINS_OP",
        ) and isinstance(node, types_cmp_issue):
            if isinstance(node, types_cmp_issue_fix):
                ...
                if len(comparisons) == 1:
                    ...
                    raise KnownIssue(
                        "multiple chain comparison inside %s can not be fixed"
                        % (node)
                    )
                ...
                raise KnownIssue(
                    "chain comparison inside %s can not be fixed" % (node)
                )
        ...
        if sys.version_info >= (3, 13):
            if instruction.opname in (
                ...
                raise KnownIssue(
                    f"can not map {instruction.opname} to two ast nodes"
                )
    ...
    def verify(self, node: EnhancedAST, instruction: dis.Instruction) -> None:
        ...
        def inst_match(
            opnames: Union[str, Sequence[str]], **kwargs: Any
        ) -> bool:
            """
            match instruction
        ...
        def node_match(
            node_type: Union[Type, Tuple[Type, ...]], **kwargs: Any
        ) -> bool:
            """
            match the ast-node
        ...
        if (
            ...
            and isinstance(
                cast(ast.Constant, cast(ast.BinOp, node).left).value, str
            )
        ):
            # "..."%(...) uses "".join
            return
        ...
        if inst_match(("BEFORE_WITH", "WITH_EXCEPT_START")) and node_match(
            ast.With
        ):
            return

        if inst_match(
            ("STORE_NAME", "STORE_GLOBAL"), argval="__doc__"
        ) and node_match(ast.Constant):
            # store docstrings
            return
        ...
        if (
            inst_match(
                ("STORE_FAST", "STORE_DEREF", "STORE_NAME", "STORE_GLOBAL")
            )
            and (
                node_match(
        ...
        if False:
            ...
            if inst_match("COMPARE_OP", argval="==") and node_match(
                ast.MatchSequence
            ):
                return
        ...
        if inst_match("DELETE_SUBSCR") and node_match(
            ast.Subscript, ctx=ast.Del
        ):
            return
        ...
        if sys.version_info >= (3, 12):
            ...
            if (
                node_match(ast.Yield)
                or isinstance(node.parent, ast.GeneratorExp)
            ) and inst_match(
                "CALL_INTRINSIC_1", argrepr="INTRINSIC_ASYNC_GEN_WRAP"
            ):
                return
            ...
            if node_match(ast.TypeVar) and (
                ...
                or inst_match(
                    ("STORE_FAST", "STORE_DEREF"), argrepr=mangled_name(node)
                )
            ):
                return
            ...
            if node_match(ast.TypeVarTuple) and (
                inst_match(
                    "CALL_INTRINSIC_1", argrepr="INTRINSIC_TYPEVARTUPLE"
                )
            ...
            if node_match(ast.TypeAlias):
                if (
                    inst_match(
                        "CALL_INTRINSIC_1", argrepr="INTRINSIC_TYPEALIAS"
                    )
                    or inst_match(
            ...
            if node_match(ast.ClassDef) and node.type_params:
                ...
                if inst_match(
                    ("STORE_FAST", "LOAD_FAST"), argrepr=".generic_base"
                ):
                    return
            ...
            if (
                node_match((ast.FunctionDef, ast.AsyncFunctionDef))
        ...
        if sys.version_info >= (3, 13):
            ...
            if inst_match("LOAD_FAST") and isinstance(
        ...
        if op_name.startswith(("BINARY_SUBSCR", "SLICE+")):
            ...
            extra_filter = lambda e: isinstance(
                cast(ast.UnaryOp, e).op, op_type
            )
        elif op_name in (
            "LOAD_ATTR",
            "LOAD_METHOD",
