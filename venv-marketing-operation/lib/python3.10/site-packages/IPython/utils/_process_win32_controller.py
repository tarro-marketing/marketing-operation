...
class Win32ShellCommandController(object):
    ...
    def __enter__(self):
        ...
        try:
            p_hstdout, c_hstdout, p_hstderr, c_hstderr, p_hstdin, c_hstdin = [
                None
            ] * 6
            ...
            def create_pipe(uninherit):
                ...
                if not SetHandleInformation(
                    handles[uninherit], HANDLE_FLAG_INHERIT, 0
                ):
                    raise ctypes.WinError()
    ...
    def _stdin_thread(self, handle, hprocess, func, stdout_func):
        ...
        while True:
            ...
            if data is None:
                ...
                if not WriteFile(
                    handle, "", 0, ctypes.byref(bytesWritten), None
                ):
                    raise ctypes.WinError()
                continue
    ...
    def run(self, stdout_func=None, stdin_func=None, stderr_func=None):
        ...
        if (
            WaitForSingleObject(self.piProcInfo.hProcess, INFINITE)
            == WAIT_FAILED
        ):
            raise ctypes.WinError()
