...
for _name in ("stdin", "stdout", "stderr"):
    ...
    try:
        if (
            not _stream
            or not hasattr(_stream, "isatty")
            or not _stream.isatty()
        ):
            _is_tty = False
            break
    except ValueError:
...
class TerminalInteractiveShell(InteractiveShell):
    ...
    def _shortcuts_changed(self, change):
        if self.pt_app:
            self.pt_app.key_bindings = self._merge_shortcuts(
        ...
        for shortcut in user_shortcuts:
            ...
            matching = [
                ...
                if (
                    ...
                    and (
                        old_keys is None
                        or [k for k in binding.keys] == old_keys
                    )
    ...
    def init_prompt_toolkit_cli(self):
        if self.simple_prompt:
            ...
            def prompt():
                prompt_text = "".join(
                    x[1] for x in self.prompts.in_prompt_tokens()
                )
    ...
    def _extra_prompt_options(self):
        ...
        if (
            ...
            and self.prompt_line_number_format == ""
        ):
