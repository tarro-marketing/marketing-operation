...
class EmbeddedSphinxShell(object):
    ...
    def process_input(self, data, input_prompt, lineno):
        ...
        if not is_okexcept and (
            ("Traceback" in processed_output)
            or ("SyntaxError" in processed_output)
        ):
            s = "\n>>>" + ("-" * 73) + "\n"
            ...
            if self.warning_is_error:
                raise RuntimeError(
                    "Unexpected exception in `{}` line {}".format(
                        filename, lineno
                    )
                )
        ...
        if not is_okwarning:
            for w in ws:
                ...
                if self.warning_is_error:
                    raise RuntimeError(
                        "Unexpected warning in `{}` line {}".format(
                            filename, lineno
                        )
                    )
...
class IPythonDirective(Directive):
    ...
    def teardown(self):
        ...
        self.shell.process_input_line(
            "bookmark -d ipy_savedir", store_history=False
        )
        self.shell.clear_cout()
...
def setup(app):
    ...
    app.add_config_value(
        "ipython_rgxin", re.compile(r"In \[(\d+)\]:\s?(.*)\s*"), "env"
    )
    app.add_config_value(
