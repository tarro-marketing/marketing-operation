...
class Fixture(TestCase):
    ...
    def get_module(self):
        module_name = "tmpmod_" + "".join(
            random.sample(self.filename_chars, 20)
        )
...
class TestAutoreload(Fixture):
    ...
    def test_autoload_newly_added_objects(self):
        ...
        ext_int = 2
        """
        new_mod_name, new_mod_fn = self.new_module(
            textwrap.dedent(new_mod_code)
        )
        current_mod_code = f"""
    ...
    def test_verbose_names(self):
        ...
        assert gather_settings("0") == gather_settings(
            "OFF"
        )  # Case insensitive
