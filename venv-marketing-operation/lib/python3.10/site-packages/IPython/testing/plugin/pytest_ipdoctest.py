...
def pytest_addoption(parser: Parser) -> None:
    ...
    parser.addini(
        "ipdoctest_encoding",
        "encoding used for ipdoctest files",
        default="utf-8",
    )
...
def pytest_collect_file(
    ...
    if file_path.suffix == ".py":
        if config.option.ipdoctestmodules and not any(
            ...
            mod: IPDoctestModule = IPDoctestModule.from_parent(
                parent, path=file_path
            )
            return mod
    elif _is_ipdoctest(config, file_path, parent):
        txt: IPDoctestTextfile = IPDoctestTextfile.from_parent(
            parent, path=file_path
        )
        return txt
    return None
...
class ReprFailDoctest(TerminalRepr):
    def __init__(
        self,
        reprlocation_lines: Sequence[Tuple[ReprFileLocation, Sequence[str]]],
    ) -> None:
...
class IPDoctestItem(pytest.Item):
    ...
    def repr_failure(  # type: ignore[override]
        ...
        failures: Optional[
            Sequence[
                Union[doctest.DocTestFailure, doctest.UnexpectedException]
            ]
        ] = None
        if isinstance(
            excinfo.value,
...
def _check_all_skipped(test: "doctest.DocTest") -> None:
    ...
    all_skipped = all(
        x.options.get(doctest.SKIP, False) for x in test.examples
    )
    if all_skipped:
...
def _is_mocked(obj: object) -> bool:
    ...
    return (
        safe_getattr(
            obj, "pytest_mock_example_attribute_that_shouldnt_exist", None
        )
        is not None
    )
    ...
    def _mock_aware_unwrap(
        ...
        *,
        ...
        try:
            ...
            _stop = stop
            return real_unwrap(
                func, stop=lambda obj: _is_mocked(obj) or _stop(func)
            )
    ...
    class LiteralsOutputChecker(IPDoctestOutputChecker):
        ...
        def _remove_unwanted_precision(self, want: str, got: str) -> str:
            ...
            for w, g in zip(wants, gots):
                ...
                if float(w.group()) == approx(
                    float(g.group()), abs=10**-precision
                ):
