...
doctest_deco = (
    skipdoctest.skip_doctest if sys.platform == "win32" else dec.null_deco
)
...
def ipexec_validate(
    fname, expected_out, expected_err="", options=None, commands=()
):
    ...
    if err:
        if expected_err:
            ...
            raise ValueError(
                "Running file %r produced error: %r" % (fname, err)
            )
