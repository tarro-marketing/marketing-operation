...
def catch_corrupt_db(f, self, *a, **kw):
    ...
    try:
        ...
        self.log.error(
            "Failed to open SQLite history %s (%s).", self.hist_file, e
        )
        if self.hist_file != ":memory:":
            if self._corrupt_db_counter > self._corrupt_db_limit:
                ...
                self.log.error(
                    "History file was moved to %s and a new file created.",
                    newpath,
                )
            self.init_db()
            return []
...
class HistoryAccessor(HistoryAccessorBase):
    ...
    def search(
        ...
        * and ?).
        ...
        cur = self._run_sql(
            sqlform, params, raw=raw, output=output, latest=unique
        )
        if n is not None:
            return reversed(list(cur))
        return cur
    ...
    def get_range(self, session, start=1, stop=None, raw=True, output=False):
        ...
        return self._run_sql(
            "WHERE session==? AND %s" % lineclause,
            params,
            raw=raw,
            output=output,
        )
...
class HistoryManager(HistoryAccessor):
    ...
    save_thread = Instance(
        "IPython.core.history.HistorySavingThread", allow_none=True
    )
    ...
    def __init__(self, shell=None, config=None, **traits):
        ...
        super(HistoryManager, self).__init__(
            shell=shell, config=config, **traits
        )
    ...
    def get_range(self, session=0, start=1, stop=None, raw=True, output=False):
        ...
        return super(HistoryManager, self).get_range(
...
def extract_hist_ranges(ranges_str):
    ...
    for range_str in ranges_str.split():
        ...
        assert (
            endsess >= startsess
        ), "start session must be earlier than end session"
