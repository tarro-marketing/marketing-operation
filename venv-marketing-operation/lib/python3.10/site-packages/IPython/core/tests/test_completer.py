...
class TestCompleter(unittest.TestCase):
    ...
    def test_latex_no_results(self):
        ...
        """
        ip = get_ipython()
        text, matches = ip.Completer.latex_matches(
            "\\really_i_should_match_nothing"
        )
    ...
    def test_jedi(self):
        ...
        if jedi_version > (0, 10):
            _test_complete(
                "jedi >0.9 should complete and not crash", "a=1;a.", "real"
            )
        _test_complete("can infer first argument", 'a=(1,"foo");a[0].', "real")
        _test_complete(
            "can infer second argument", 'a=(1,"foo");a[1].', "capitalize"
        )
        _test_complete("cover duplicate completions", "im", "import", 0, 2)

        _test_not_complete(
            "does not mix types", 'a=(1,"foo");a[0].', "capitalize"
        )
    ...
    def test_deduplicate_completions(self):
        ...
        with provisionalcompleter():
            ...
            l = list(
                _deduplicate_completions(
                    "Z.z", ip.Completer.completions("Z.z", 3)
                )
            )
            ip.Completer.use_jedi = False

        assert len(l) == 1, (
            "Completions (Z.z<tab>) correctly deduplicate: %s " % l
        )
    ...
    def test_greedy_completions(self):
        ...
        self.assertFalse(
            ".real" in c, "Shouldn't have completed on a[0]: %s" % c
        )
    ...
    def test_match_dict_keys_tuple(self):
        ...
        assert match(keys, "'foo", extra=("foo1", "foo2")) == (
            "'",
            1,
            ["foo3"],
        )
        assert match(keys, "'foo", extra=("foo1", "foo2", "foo3")) == (
            "'",
            1,
            ["foo4"],
        )
    ...
    def test_fwd_unicode_restricts(self):
        ...
        with provisionalcompleter():
            ...
            completions = [
                completion.text
                for completion in completer.completions(text, len(text))
            ]
    ...
    def test_dict_key_restrict_to_dicts(self):
        ...
        def _():
            with provisionalcompleter():
                ...
                return [
                    completion.text
                    for completion in c.completions(text, len(text))
                ]
    ...
    def test_matcher_suppression(self):
        ...
        with custom_matchers([a_matcher, b_matcher, c_matcher]):
            ...
            _(
                "do not suppress",
                ["completion_a", "completion_b", "completion_c"],
            )
    ...
    def test_matcher_suppression_with_iterator(self):
        ...
        with custom_matchers(
            [matcher_returning_iterator, matcher_returning_list]
        ):
            ip = get_ipython()
            c = ip.Completer
    ...
    def test_matcher_suppression_with_jedi(self):
        ...
        def _():
            with provisionalcompleter():
                matches = [
                    completion.text for completion in c.completions("dict.", 5)
                ]
                self.assertIn("keys", matches)
    ...
    def test_matcher_priority(self):
        ...
        def a_matcher(text):
            return {
                "completions": [SimpleCompletion("completion_a")],
                "suppress": True,
            }
        ...
        def b_matcher(text):
            return {
                "completions": [SimpleCompletion("completion_b")],
                "suppress": True,
            }
