...
def _check_pil_jpeg_bytes():
    ...
    try:
        ...
        raise pytest.skip(
            "PIL can't write JPEG to BytesIO: %s: %s" % (ename, e)
        ) from e
...
def test_figure_to_jpeg():
    ...
    ax.plot([1, 2, 3])
    plt.draw()
    jpeg = pt.print_figure(fig, "jpeg", pil_kwargs={"optimize": 50})[
        :100
    ].lower()
...
def test_backend_unknown():
    with pytest.raises(
        RuntimeError if pt._matplotlib_manages_backends() else KeyError
    ):
