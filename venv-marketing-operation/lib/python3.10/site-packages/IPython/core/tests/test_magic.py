...
def test_time_microseconds_display():
    ...
    with mock.patch(
        "sys.stdout", io.TextIOWrapper(StringIO(), encoding="utf-8")
    ):
        assert execution._format_time(0.000001) == "1 \u03bcs"
    with mock.patch(
        "sys.stdout", io.TextIOWrapper(StringIO(), encoding="ascii")
    ):
...
class TestEnv(TestCase):
    ...
    def test_env_set_bad_input(self):
        self.assertRaises(
            UsageError, lambda: _ip.run_line_magic("set_env", "var")
        )

    def test_env_set_whitespace(self):
        self.assertRaises(
            UsageError, lambda: _ip.run_line_magic("env", "var A=B")
        )
...
def test_script_out():
    ip = get_ipython()
    ip.run_cell_magic(
        "script", f"--out output {sys.executable}", "print('hi')"
    )
...
async def test_script_bg_out():
    ip = get_ipython()
    ip.run_cell_magic(
        "script", f"--bg --out output {sys.executable}", "print('hi')"
    )
    ...
    with TemporaryDirectory() as td:
        try:
            with tt.AssertNotPrints(re.compile("Activating.*")):
                lm.logstart(
                    "-q {}".format(os.path.join(td, "quiet_from_arg.log"))
                )
        finally:
            _ip.logger.logstop()
