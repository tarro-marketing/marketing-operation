...
syntax = dict(
    assign_system=[
        ...
        (
            "a, b = !echo 'a\\nb'",
            "a, b = get_ipython().getoutput(\"echo 'a\\\\nb'\")",
        ),
        ("a,= !echo 'a'", "a, = get_ipython().getoutput(\"echo 'a'\")"),
        (
            "a, *bc = !echo 'a\\nb\\nc'",
            "a, *bc = get_ipython().getoutput(\"echo 'a\\\\nb\\\\nc'\")",
        ),
...
def test_assign_system():
    tt.check_pairs(
        transform_and_reset(ipt.assign_from_system), syntax["assign_system"]
    )


def test_assign_magic():
    tt.check_pairs(
        transform_and_reset(ipt.assign_from_magic), syntax["assign_magic"]
    )
    ...
    tests = [
        ...
        [
            (
                "a = '''",
                None,
            ),  # Test resetting when within a multi-line string
            ("def", None),
            (None, "a = '''\ndef"),
        ],
        [
        ...
        [
            (
                "a = '''",
                None,
            ),  # Test line continuation within a multi-line string
            ("abc\\", None),
            ("def", None),
            ("'''", "a = '''\nabc\\\ndef\n'''"),
        ],
...
def test_escaped_noesc():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_noesc"]
    )


def test_escaped_shell():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_shell"]
    )


def test_escaped_help():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_help"]
    )


def test_escaped_magic():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_magic"]
    )


def test_escaped_quote():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_quote"]
    )


def test_escaped_quote2():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_quote2"]
    )


def test_escaped_paren():
    tt.check_pairs(
        transform_and_reset(ipt.escaped_commands), syntax["escaped_paren"]
    )
...
def decistmt(tokens):
    ...
    for toknum, tokval, _, _, _ in tokens:
        if (
            toknum == tokenize.NUMBER and "." in tokval
