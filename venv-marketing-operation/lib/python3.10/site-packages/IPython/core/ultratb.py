...
def _format_filename(file, ColorFilename, ColorNormal, *, lineno=None):
    ...
    if (
        ...
        if lineno is None:
            ...
            tpl_link = (
                f"{{label}} {ColorFilename}{{name}}:{{lineno}}{ColorNormal}"
            )
...
class TBTools(colorable.Colorable):
    ...
    def text(
        self, etype, value, tb, tb_offset: Optional[int] = None, context=5
    ):
        """Return formatted traceback.

        Subclasses may override this if they add extra arguments.
        """
        tb_list = self.structured_traceback(
            etype, value, tb, tb_offset, context
        )
...
class ListTB(TBTools):
    ...
    def _format_exception_only(self, etype, value):
        ...
        Colors = self.Colors
        output_list = []
        stype = py3compat.cast_unicode(
            Colors.excName + etype.__name__ + Colors.Normal
        )
        if value is None:
...
class VerboseTB(TBTools):
    ...
    def format_record(self, frame_info: FrameInfo):
        ...
        tpl_call = (
            f"in {Colors.vName}{{file}}{Colors.valEm}{{scope}}{ColorsNormal}"
        )
        ...
        if self.include_vars:
            try:
                ...
                for var in fibp:
                    lvals_list.append(
                        tpl_name_val % (var.name, repr(var.value))
                    )
            except Exception:
                lvals_list.append(
    ...
    def format_exception(self, etype, evalue):
        ...
        return [
            ...
            *(
                "{}{}".format(
                    colorsnormal,
                    _safe_string(py3compat.cast_unicode(n), "note"),
                )
                for n in notes
            ),
        ]
    ...
    def format_exception_as_a_whole(
        ...
        records = (
            self.get_records(etb, number_of_lines_of_context, tb_offset)
            if etb
            else []
        )
    ...
    def get_records(
        ...
        if max_len > FAST_THRESHOLD:
            ...
            return FIs
        res = list(stack_data.FrameInfo.stack_data(etb, options=options))[
            tb_offset:
        ]
...
class AutoFormattedTB(FormattedTB):
    ...
    def __call__(
        self, etype=None, evalue=None, etb=None, out=None, tb_offset=None
    ):
