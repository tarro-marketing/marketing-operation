...
def _has_original_dunder(
    ...
    for module_name, *access_path in allowed_external:
        if _has_original_dunder_external(
...
class SelectivePolicy(EvaluationPolicy):
    ...
    allowed_operations_external: Set[Tuple[str, ...]] = field(
        default_factory=set
    )
...
def guarded_eval(code: str, context: EvaluationContext):
    ...
    if context.in_subscript:
        ...
        context = EvaluationContext(
            **{**context._asdict(), **{"locals": locals_}}
        )
...
class _Duck:
    ...
    def __init__(
        self, attributes: Optional[dict] = None, items: Optional[dict] = None
    ):
        ...
        for op, right in zip(node.ops, node.comparators):
            ...
            if dunder:
                ...
                if (
                    dunder == "is_"
                    or dunders
                    and policy.can_operate(dunders, a, b)
                ):
...
def _eval_return_type(
    func: Callable, node: ast.Call, context: EvaluationContext
):
    ...
    if not_empty:
        return _resolve_annotation(
            sig.return_annotation, sig, func, node, context
        )
...
def _resolve_annotation(
    ...
    if annotation is Self and hasattr(func, "__self__"):
        ...
        attributes = [
            ...
            for attr in dir(
                _resolve_annotation(type_arg, sig, func, node, context)
            )
        ]
...
def _eval_node_name(node_id: str, context: EvaluationContext):
    ...
    if not policy.allow_globals_access and not policy.allow_locals_access:
        ...
        raise NameError(
            f"{node_id} not found in locals, globals, nor builtins"
        )


def _eval_or_create_duck(
    duck_type, node: ast.Call, context: EvaluationContext
):
    policy = EVALUATION_POLICIES[context.evaluation]
...
ALLOWED_CALLS = {
    ...
    *[
        method
        for numeric_cls in NUMERICS
        for method in _list_methods(numeric_cls)
    ],
    collections.deque,
