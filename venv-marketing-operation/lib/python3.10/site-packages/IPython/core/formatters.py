    ...
    def format(self, obj, include=None, exclude=None):
        ...
        * image/svg+xml
        ...
        if format_dict or md_dict:
            if include:
                format_dict = {
                    k: v for k, v in format_dict.items() if k in include
                }
                md_dict = {k: v for k, v in md_dict.items() if k in include}
            if exclude:
                format_dict = {
                    k: v for k, v in format_dict.items() if k not in exclude
                }
                md_dict = {
                    k: v for k, v in md_dict.items() if k not in exclude
                }
    ...
    def _check_return(self, r, obj):
        ...
        if (
            ...
            or (
                isinstance(r, tuple)
                and r
                and isinstance(r[0], self._return_type)
            )
        ):
            return r
        else:
...
class PlainTextFormatter(BaseFormatter):
    ...
    def _type_printers_default(self):
        ...
        if "numpy" in sys.modules:
            ...
            d[numpy.float64] = lambda obj, p, cycle: p.text(
