...
class ExecutionInfo(object):
    ...
    def __init__(
        self, raw_cell, store_history, silent, shell_futures, cell_id
    ):
...
class ExecutionResult:
    ...
    @property
    def success(self):
        return (self.error_before_exec is None) and (
            self.error_in_exec is None
        )
...
class InteractiveShell(SingletonConfigurable):
    ...
    ipython_dir = Unicode("").tag(
        config=True
    )  # Set to get_ipython_dir() in __init__
    ...
    history_length = Integer(
        10000, help="Total length of command history"
    ).tag(config=True)
    ...
    alias_manager = Instance(
        "IPython.core.alias.AliasManager", allow_none=True
    )
    ...
    payload_manager = Instance(
        "IPython.core.payload.PayloadManager", allow_none=True
    )
    ...
    profile_dir = Instance(
        "IPython.core.application.ProfileDir", allow_none=True
    )
    ...
    last_execution_succeeded = Bool(
        True, help="Did last executed command succeeded"
    )
    ...
    def init_virtualenv(self):
        ...
        if "VIRTUAL_ENV" not in os.environ:
            ...
            warn(
                "Virtual env path set to '', please check if this is intended."
            )
            return
        ...
        if sys.platform == "win32":
            virtual_env = str(
                Path(os.environ["VIRTUAL_ENV"], "Lib", "site-packages")
            )
        else:
            ...
            if re_m:
                predicted_path = Path(
                    str(virtual_env_path).format(*re_m.groups())
                )
    ...
    def save_sys_module_state(self):
        ...
        self._orig_sys_modules_main_mod = sys.modules.get(
            self.user_module.__name__
        )
    ...
    def restore_sys_module_state(self):
        ...
        if self._orig_sys_modules_main_mod is not None:
            sys.modules[self._orig_sys_modules_main_name] = (
    ...
    def reset_selective(self, regex=None):
        ...
        if regex is not None:
            try:
                ...
                raise TypeError(
                    "regex must be a string or compiled pattern"
                ) from e
    ...
    def _ofind(
        self,
        oname: str,
        namespaces: Optional[Sequence[Tuple[str, AnyType]]] = None,
    ) -> OInfo:
    ...
    def showsyntaxerror(self, filename=None, running_compiled_code=False):
        ...
        elist = (
            traceback.extract_tb(last_traceback)
    ...
    def _find_with_lazy_load(self, /, type_, magic_name: str):
        ...
        """
        finder = {"line": self.find_line_magic, "cell": self.find_cell_magic}[
            type_
        ]
    ...
    def run_cell_magic(self, magic_name, line, cell):
        ...
        if fn is None:
            ...
            message = (
                "%%{0} is a cell magic, but the cell body is empty.".format(
                    magic_name
                )
            )
            if self.find_line_magic(magic_name) is not None:
                message += (
                    " Did you mean the line magic %{0} (single %)?".format(
                        magic_name
                    )
                )
    ...
    def safe_run_module(self, mod_name, where):
        ...
        try:
            try:
                where.update(
                    runpy.run_module(
                        str(mod_name), run_name="__main__", alter_sys=True
                    )
                )
            except SystemExit as status:
                if status.code:
        ...
        *,
        ...
        if store_history:
            ...
            self.history_manager.store_inputs(
                self.execution_count, cell, raw_cell
            )
        if not silent:
        ...
        with self.builtin_trap:
            ...
            with self.display_trap:
                ...
                interactivity = (
                    "none" if silent else self.ast_node_interactivity
                )
    ...
    async def run_ast_nodes(
        ...
        try:
            ...
            def compare(code):
                is_async = (
                    inspect.CO_COROUTINE & code.co_flags
    ...
    def enable_pylab(self, gui=None, import_all=True, welcome_message=False):
        ...
        clobbered = [
            name for name in both if self.user_ns[name] is not ns[name]
        ]
        self.user_ns.update(ns)
        self.user_ns_hidden.update(ns)
    ...
    def mktempfile(self, data=None, prefix="ipython_edit_"):
        ...
        os.close(
            handle
        )  # On Windows, there can only be one open handle on a file
    ...
    def find_user_code(
                ...
                try:
                    ...
                    if not py_only:
                        with io_open(tgt, "r", encoding="latin1") as f:
                            return f.read()
                    raise ValueError(
                        ("'%s' seem to be unreadable.") % target
                    ) from e
        ...
        raise TypeError(
            "%s is neither a string nor a macro." % target, codeobj
        )
