...
class TimeitResult(object):
    ...
    def stdev(self):
        mean = self.average
        return (
            math.fsum([(x - mean) ** 2 for x in self.timings])
            / len(self.timings)
        ) ** 0.5
...
class ExecutionMagics(Magics):
    ...
    def pdb(self, parameter_s=""):
        ...
        if par:
            try:
                ...
                print(
                    "Incorrect argument. Use on/1, off/0, "
    ...
    def _run_with_debugger(
        ...
        if not deb:
            self.shell.InteractiveTB.pdb = (
                self.shell.InteractiveTB.debugger_cls()
            )
            deb = self.shell.InteractiveTB.pdb
    ...
    def timeit(self, line="", cell=None, local_ns=None):
        ...
        default_repeat = (
            7 if timeit.default_repeat < 7 else timeit.default_repeat
        )
