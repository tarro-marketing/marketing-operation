...
class NamespaceMagics(Magics):
    ...
    def pinfo2(self, parameter_s="", namespaces=None):
        ...
        self.shell._inspect(
            "pinfo", parameter_s, detail_level=1, namespaces=namespaces
        )
    ...
    def whos(self, parameter_s=""):
        ...
        for vv in varlist:
            ...
            if tt == "instance":
                typelist.append(
                    abbrevs.get(str(vv.__class__), str(vv.__class__))
                )
            else:
                typelist.append(tt)
        ...
        # table header
        print(
            varlabel.ljust(varwidth)
            + typelabel.ljust(typewidth)
            + " "
            + datalabel
            + "\n"
        ...
        for vname, var, vtype in zip(varnames, varlist, typelist):
            print(
                vformat.format(
                    vname, vtype, varwidth=varwidth, typewidth=typewidth
                ),
                end=" ",
            )
            if vtype in seq_types:
                ...
                vshape = (
                    str(var.shape).replace(",", "").replace(" ", "x")[1:-1]
                )
    ...
    def reset(self, parameter_s=""):
        ...
        opts, args = self.parse_options(
            parameter_s, "sf", "aggressive", mode="list"
        )
        if "f" in opts:
            ans = True
        else:
        ...
        if "s" in opts:  # Soft reset
            ...
            self.shell.reset(
                new_session=False, aggressive=("aggressive" in opts)
            )
        ...
        for target in args:
            ...
            if target == "out":
                print(
                    "Flushing output cache (%d entries)" % len(user_ns["_oh"])
                )
