...
class MagicAssign(TokenTransformBase):
    ...
    def transform(self, lines: List[str]):
        ...
        call = "get_ipython().run_line_magic({!r}, {!r})".format(
            magic_name, args
        )
        new_line = lhs + call + "\n"
...
def _make_help_call(target, esc):
    ...
    (i.e. ? or ??)"""
    method = (
        "pinfo2" if esc == "??" else "psearch" if "*" in target else "pinfo"
    )
    arg = " ".join([method, target])
    # Prepare arguments for get_ipython().run_line_magic(magic_name, magic_args)
    t_magic_name, _, t_magic_arg_s = arg.partition(" ")
    t_magic_name = t_magic_name.lstrip(ESC_MAGIC)
    return "get_ipython().run_line_magic(%r, %r)" % (
        t_magic_name,
        t_magic_arg_s,
    )
...
class EscapedCommand(TokenTransformBase):
    ...
    def find(cls, tokens_by_line):
        ...
        for line in tokens_by_line:
            ...
            ix = 0
            ll = len(line)
            while ll > ix and line[ix].type in {
                tokenize.INDENT,
                tokenize.DEDENT,
            }:
                ix += 1
...
def make_tokens_by_line(lines: List[str]):
    ...
    try:
        for token in tokenutil.generate_tokens_catch_errors(
            ...
            if (token.type == NEWLINE) or (
                (token.type == NL) and (parenlev <= 0)
            ):
...
class TransformerManager:
    ...
    def check_complete(self, cell: str):
        ...
        while (
            tokens_by_line[-1] and tokens_by_line[-1][-1].type in newline_types
        ):
        ...
        if tokens_by_line[-1][-1].string == ":":
            ...
            ix = 0
            while tokens_by_line[-1][ix].type in {
                tokenize.INDENT,
                tokenize.DEDENT,
            }:
                ix += 1
