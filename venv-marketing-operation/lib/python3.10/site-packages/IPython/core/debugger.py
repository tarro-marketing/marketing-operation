...
class Pdb(OldPdb):
    ...
    def __init__(
        self, completekey=None, stdin=None, stdout=None, context=5, **kwargs
    ):
    ...
    def hidden_frames(self, stack):
        ...
        ip_start = [
            i for i, s in enumerate(ip_hide) if s == "__ipython_bottom__"
        ]
        if ip_start and self._predicates["ipython_internal"]:
            ip_hide = [
                h if i > ip_start[0] else True for (i, h) in enumerate(ip_hide)
            ]
        return ip_hide
        ...
        def do_exceptions(self, arg):
            ...
            if not arg:
                for ix, exc in enumerate(self._chained_exceptions):
                    prompt = (
                        ">" if ix == self._chained_exception_index else " "
                    )
                    rep = repr(exc)
                    if len(rep) > 80:
                        rep = rep[:77] + "..."
                ...
                if 0 <= number < len(self._chained_exceptions):
                    ...
                    self.setup(
                        None, self._chained_exceptions[number].__traceback__
                    )
    ...
    def interaction(self, frame, tb_or_exc):
        try:
            if CHAIN_EXCEPTIONS:
                ...
                _chained_exceptions, tb = self._get_tb_and_exceptions(
                    tb_or_exc
                )
                if isinstance(tb_or_exc, BaseException):
                    assert (
                        tb is not None
                    ), "main exception must have a traceback"
    ...
    def print_stack_trace(self, context=None):
        ...
        try:
            skipped = 0
            for hidden, frame_lineno in zip(
                self.hidden_frames(self.stack), self.stack
            ):
    ...
    def format_stack_entry(self, frame_lineno, lprefix=": ", context=None):
        ...
        for i, line in enumerate(lines):
            ...
            linetpl = (
                (frame is self.curframe or show_arrow)
                and tpl_line_em
                or tpl_line
            )
            ret.append(
    ...
    def getsourcelines(self, obj):
        ...
        if inspect.isframe(obj) and obj.f_globals is self._get_frame_locals(
            obj
        ):
    ...
    def break_anywhere(self, frame):
        ...
        if self._predicates["debuggerskip"]:
            if DEBUGGERSKIP in frame.f_code.co_varnames:
                return True
            if frame.f_back and self._get_frame_locals(frame.f_back).get(
                DEBUGGERSKIP
            ):
                return True
        return False
