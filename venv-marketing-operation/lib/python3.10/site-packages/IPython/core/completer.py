...
def match_dict_keys(
    ...
    for key in filtered_keys:
        ...
        rem_repr = (
            repr(rem + '"') if isinstance(rem, str) else repr(rem + b'"')
        )
...
def _convert_matcher_v1_result_to_v2(
    ...
    result = {
        "completions": [
            SimpleCompletion(text=match, type=type) for match in matches
        ],
        "suppress": (
            (True if matches else False) if suppress_if_matches else False
        ),
    }
...
class IPCompleter(Completer):
    ...
    def magic_matches(self, text: str):
        ...
        if not explicit_magic:
            ...
            def matches(magic):
                ...
                """
                return (
                    magic.startswith(bare_text) and magic not in global_matches
                )
    ...
    def magic_config_matches(self, text: str) -> List[str]:
        ...
        if len(texts) > 0 and (texts[0] == "config" or texts[0] == "%config"):
            ...
            classname_matches = [
                c for c in classnames if c.startswith(classname)
            ]
            ...
            if texts[1].find(".") < 0:
                ...
                len(classname_matches) == 1
                and classname_matches[0] == classname
            ):
    ...
    def magic_color_matches(self, text: str) -> List[str]:
        ...
        if len(texts) == 2 and (texts[0] == "colors" or texts[0] == "%colors"):
            ...
            return [
                color
                for color in InspectColors.keys()
    ...
    def _jedi_matches(
        ...
        try:
            ...
            try:
                first_child = next(
                    c
                    for c in interpreter._get_module().tree_node.children
                    if hasattr(c, "value")
                )
            except StopIteration:
                pass
            ...
            if self.debug:
                print(
                    "Error detecting if completing a non-finished string :",
                    e,
                    "|",
                )
        ...
        try:
            return filter(
                ...
                interpreter.complete(
                    column=cursor_column, line=cursor_line + 1
                ),
            )
        except Exception as e:
            if self.debug:
    ...
    def _default_arguments(self, obj):
        ...
        try:
            ...
            ret.extend(
                k for k, v in sig.parameters.items() if v.kind in _keeps
            )
        except ValueError:
            pass
    ...
    def python_func_kw_matches(self, text):
        ...
        try:
            ...
            namedArgs = self._default_arguments(
                eval(callableObj, self.namespace)
            )
    ...
    def _get_keys(obj: Any) -> List[Any]:
        ...
        if isinstance(obj, dict) or _safe_isinstance(
            ...
            try:
                ...
                return []
        elif _safe_isinstance(
            obj, "pandas", "core", "indexing", "_LocIndexer"
        ):
            try:
    ...
    def dict_key_matches(self, text: str) -> List[str]:
        ...
        end_of_tuple_or_item = (
            _DictKeyState.END_OF_TUPLE | _DictKeyState.END_OF_ITEM
        )
    ...
    def unicode_name_matcher(self, context: CompletionContext):
        ...
        fragment, matches = self.unicode_name_matches(
        ...
        try:
            ...
            if profiler is not None:
                ...
                output_path = os.path.join(
                    self.profiler_output_dir, str(uuid.uuid4())
                )
    ...
    def _arrange_and_extract(
        ...
        if not most_recent_fragment:
            most_recent_fragment = (
                ""  # to satisfy typechecker (and just in case)
            )
    ...
    def _complete(
        ...
        *,
        ...
        if (
            not text
        ):  # issue #11508: check line_buffer before calling split_line
            text = (
                self.splitter.split_line(line_buffer, cursor_pos)
                if line_buffer
                else ""
            )
        ...
        for matcher_id, matcher in matchers.items():
            ...
            result["matched_fragment"] = result.get(
                "matched_fragment", context.token
            )
