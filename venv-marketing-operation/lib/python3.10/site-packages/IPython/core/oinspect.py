...
UnformattedBundle: TypeAlias = Dict[
...
def format_argspec(argspec):
    ...
    warnings.warn(
        "`format_argspec` function is deprecated as of IPython 7.10"
        "and will be removed in future versions.",
        DeprecationWarning,
        stacklevel=2,
    )
...
class Inspector(Colorable):
        ...
        if "text/html" in bundle:
            ...
            if isinstance(bundle["text/html"], (list, tuple)):
                ...
                new_b["text/html"] = "\n".join(
                    (
                        f"<h1>{head}</h1>\n{body}"
                        for (head, body) in bundle["text/html"]
                    )
                )
    ...
    def _make_info_unformatted(
        ...
        if info["isalias"]:
            ...
            append_field(
                bundle, "Init signature", "init_definition", code_formatter
            )
            append_field(bundle, "Docstring", "docstring", formatter)
            if detail_level > 0 and info["source"]:
                ...
                append_field(
                    bundle, "Init docstring", "init_docstring", formatter
                )
            ...
            append_field(
                bundle, "Class docstring", "class_docstring", formatter
            )
    ...
    def _get_info(
        ...
        info_dict = self.info(
            obj, oname=oname, info=info, detail_level=detail_level
        )
    ...
    def info(self, obj, oname="", info=None, detail_level=0) -> InfoDict:
        ...
        if (
            info
            and info.parent is not None
            and hasattr(info.parent, HOOK_NAME)
        ):
    ...
    def psearch(
        ...
        *,
        ...
        if len_cmds == 1:
            ...
            raise ValueError(
                "invalid argument string for psearch: <%s>" % pattern
            )
        ...
        for name in ns_search:
            if name not in ns_table:
                raise ValueError(
                    "invalid namespace <%s>. Valid names: %s"
                    % (name, ns_table.keys())
                )
