...
class InteractiveShellApp(Configurable):
    ...
    code_to_run = Unicode("", help="Execute the given command string.").tag(
        config=True
    )
    module_to_run = Unicode("", help="Run the module as a script.").tag(
        config=True
    )
    gui = CaselessStrEnum(
        gui_keys,
    ...
    def init_gui_pylab(self):
        ...
        if isinstance(r, tuple):
            ...
            gui = r
            self.log.info(
                "Enabling GUI event loop integration, " "eventloop=%s", gui
            )
    ...
    def init_extensions(self):
        ...
        try:
            ...
            for ext in extensions:
                try:
                    ...
                    if self.reraise_ipython_extension_failures:
                        raise
                    msg = (
                        "Error in loading extension: {ext}\n"
            ...
            if self.reraise_ipython_extension_failures:
                raise
            self.log.warning(
                "Unknown error in loading extensions:", exc_info=True
            )
    ...
    def _exec_file(self, fname, shell_futures=False):
        ...
        try:
            if os.path.isfile(full_filename):
                self.log.info(
                    "Running file in user namespace: %s" % full_filename
                )
                ...
                with preserve_keys(self.shell.user_ns, "__file__"):
                    ...
                    if full_filename.endswith(
                        ".ipy"
                    ) or full_filename.endswith(".ipynb"):
    ...
    def _run_startup_files(self):
        ...
        if (
            self.exec_PYTHONSTARTUP
            and os.environ.get("PYTHONSTARTUP", False)
            and not (
                self.file_to_run or self.code_to_run or self.module_to_run
            )
        ):
    ...
    def _run_cmd_line_code(self):
        ...
        if self.code_to_run:
            ...
            try:
                self.log.info(
                    "Running code given at command line (c=): %s" % line
                )
                self.shell.run_cell(line, store_history=False)
            except:
                self.log.warning(
        ...
        if self.module_to_run:
            ...
            try:
                self.shell.safe_run_module(
                    self.module_to_run, self.shell.user_ns
                )
            finally:
                sys.argv = save_argv
