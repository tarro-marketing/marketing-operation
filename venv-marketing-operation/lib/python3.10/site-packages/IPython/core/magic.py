...
def _function_magic_marker(magic_kind):
    ...
    def magic_deco(arg):
        ...
        for ns in ["f_locals", "f_globals", "f_builtins"]:
            ...
            raise NameError(
                "Decorator can only run in context where "
                "`get_ipython` exists"
            )
...
class MagicsManager(Configurable):
    ...
    def register_alias(
        ...
        if magic_kind not in magic_kinds:
            raise ValueError(
                "magic_kind must be one of %s, %s given" % magic_kinds,
                magic_kind,
            )
...
class Magics(Configurable):
    ...
    def format_latex(self, strng):
        ...
        # Magic commands
        cmd_re = re.compile(
            r"(?P<cmd>%s.+?\b)(?!\}\}:)" % ESC_MAGIC, re.MULTILINE
        )
        # Paragraph continue
        ...
        if len(args) >= 1:
            ...
            for o, a in opts:
                if mode == "string" and preserve_non_opts:
                    ...
                    remainder_arg_str = remainder_arg_str.replace(
                        o, "", 1
                    ).replace(a, "", 1)
                if o.startswith("--"):
                    o = o[2:]
...
class MagicAlias(object):
    ...
    def __init__(self, shell, magic_name, magic_kind, magic_params=None):
        ...
        self.pretty_target = "%s%s" % (
            magic_escapes[self.magic_kind],
            self.magic_name,
        )
