...
class HistoryTrim(BaseIPythonApplication):
    ...
    backup = Bool(
        False, help="Keep the old history file as history.sqlite.<N>"
    ).tag(config=True)

    keep = Int(
        1000, help="Number of recent lines to keep in the database."
    ).tag(config=True)
    ...
    def start(self):
        ...
        if len(inputs) <= self.keep:
            ...
            print(
                "Not doing anything. Use --keep= argument to keep fewer entries"
            )
            return
        ...
        if inputs:
            with new_db:
                ...
                new_db.executemany(
                    "insert into history values (?,?,?,?)", inputs
                )
                new_db.executemany(
...
class HistoryClear(HistoryTrim):
    ...
    force = Bool(False, help="Don't prompt user for confirmation").tag(
        config=True
    )
