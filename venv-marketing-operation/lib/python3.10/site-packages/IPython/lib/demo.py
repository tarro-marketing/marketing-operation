...
class Demo(object):
    ...
    def __init__(
        ...
        try:
            ip = (
                get_ipython()
            )  # this is in builtins whenever IPython is running
        ...
        try:
            ...
            if self._silent[index]:
                ...
                if self.auto_all or self._auto[index]:
                    ...
                    print(
                        marquee("Press <q> to quit, <Enter> to execute..."),
                        end=" ",
                    )
        ...
        if self.format_rst:
            ...
            for token in tokens:
                if token[0] == Token.String.Doc and len(token[1]) > 6:
                    ...
                    toks += list(pygments.lex(token[1][1:], self.rst_lexer))[
                        :-1
                    ]
                else:
                    toks.append(token)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--style", "-s", default="default", help="pygments style name"
    )
    parser.add_argument(
        "--auto",
        "-a",
        action="store_true",
        help="Run all blocks automatically without" "confirmation",
    )
    parser.add_argument(
        "--delimiter",
        "-d",
    ...
    parser.add_argument("file", nargs=1, help="python demo file")
