...
def test_genelatex_no_wrap():
    ...
    def mock_kpsewhich(filename):
        assert (
            False
        ), "kpsewhich should not be called " "(called with {0})".format(
            filename
        )
...
def test_latex_to_png_color():
    ...
    default_hexblack = latextools.latex_to_png(
        latex_string, wrap=False, color="#000000"
    )
    ...
    dvipng_maroon = latextools.latex_to_png_dvipng(
        latex_string, False, "Maroon"
    )
    ...
    mpl_white_scale = latextools.latex_to_png_mpl(
        latex_string, False, "White", 1.2
    )
