...
class Formatter:
    def __init__(
        ...
        *,
        ...
        if (
            not self.show_linenos
            and self.options.blank_lines == BlankLines.SINGLE
        ):
            raise ValueError(
    ...
    def format_frame(
        self, frame: Union[FrameInfo, FrameType, TracebackType]
    ) -> Iterable[str]:
    ...
    def format_blank_lines_linenumbers(self, blank_line):
        ...
        if blank_line.begin_lineno == blank_line.end_lineno:
            return (
                result
                + self.line_number_format_string.format(
                    blank_line.begin_lineno
                )
                + "\n"
            )
