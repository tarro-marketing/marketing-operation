...
from stack_data.utils import (
    ...
    cached_property,
    is_frame,
    _pygmented_with_ranges,
    assert_,
)

RangeInLine = NamedTuple(
    "RangeInLine", [("start", int), ("end", int), ("data", Any)]
)
RangeInLine.__doc__ = """
...
MarkerInLine = NamedTuple(
    "MarkerInLine", [("position", int), ("is_start", bool), ("string", str)]
)
...
class Variable(
    NamedTuple(
        "_Variable",
        [("name", str), ("nodes", Sequence[ast.AST]), ("value", Any)],
    )
):
    """
    ...
    def tokens_by_lineno(self) -> Mapping[int, List[Token]]:
        if not self.tree:
            raise AttributeError(
                "This file doesn't contain valid Python, so .tokens_by_lineno doesn't exist"
            )
    ...
    def _raw_split_into_pieces(
        ...
        for name, body in ast.iter_fields(stmt):
            if (
                ...
                for rang, group in sorted(
                    group_by_key_func(body, self.line_range).items()
                ):
                    sub_stmt = group[0]
...
class Line(object):
    ...
    def __repr__(self):
        return (
            "<{self.__class__.__name__} {self.lineno} (current={self.is_current}) "
    ...
    def variable_ranges(self) -> List[RangeInLine]:
        ...
        return [
            ...
            for variable, node in self.frame_info.variables_by_lineno[
                self.lineno
            ]
        ]
    ...
    def range_from_node(
        ...
        (start, range_start), (end, range_end) = atext.get_text_positions(
            node, padded=False
        )
    ...
    def render(
        ...
        *,
        ...
        markers = list(markers) + [
            MarkerInLine(position=len(text), is_start=False, string="")
        ]
...
def markers_from_ranges(
    ...
    for rang in ranges:
        ...
        if not (isinstance(start_string, str) and isinstance(end_string, str)):
            raise TypeError(
                "converter should return None or a pair of strings"
            )

        markers += [
            MarkerInLine(
                position=rang.start, is_start=True, string=start_string
            ),
...
class RepeatedFrames:
    ...
    def __repr__(self):
        return "<{self.__class__.__name__} {self.description}>".format(
            self=self
        )
...
class FrameInfo(object):
    ...
    def executing_piece(self) -> range:
        ...
        """
        return only(
            piece for piece in self.scope_pieces if self.lineno in piece
        )
    ...
    def lines(self) -> List[Union[Line, LineGap, BlankLineRange]]:
        ...
        add_empty_lines = self.options.blank_lines in (
            BlankLines.VISIBLE,
            BlankLines.SINGLE,
        )
        prev_piece = None
        ...
        for i, piece in enumerate(pieces):
            if (
                ...
                prev_piece
                and add_empty_lines
                and piece.start > prev_piece.stop
            ):
                if self.options.blank_lines == BlankLines.SINGLE:
                    result.append(
                        BlankLineRange(prev_piece.stop, piece.start - 1)
                    )
        ...
        real_lines = [line for line in result if isinstance(line, Line)]

        text = "\n".join(line.text for line in real_lines)
    ...
    def _pygmented_scope_lines(self) -> Optional[Tuple[int, List[str]]]:
        ...
        scope = self.scope
        assert_(
            formatter, ValueError("Must set a pygments formatter in Options")
        )
        assert_(scope)
    ...
    @cached_property
    def variables_by_lineno(
        self,
    ) -> Mapping[int, List[Tuple[Variable, ast.AST]]]:
        """
